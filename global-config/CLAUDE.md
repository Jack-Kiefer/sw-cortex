...
- **When Jack pushes back on a theory, do NOT re-offer a shallower version of the same theory — dig into the data.** "That doesn't make sense," "still see it," "no," or him pointing at on-screen/Slack/log data is Jack supplying **ground truth that your current picture contradicts** — treat it as evidence your theory is wrong, not as a request to re-explain it. Never respond with another hand-wavy explanation of the same class you just gave (a "cache snapshot," "identical code," "pre-existing condition," "must be a race" story) — that is the single most-corrected failure mode across sessions. Instead: go back to raw sources, reproduce the exact numbers he's seeing, and find the mechanism. The real cause has repeatedly been a layer deeper than the first theory (a darklaunch "Parallel Test" datasource switch producing the negative inventory, a SERPY BOM/product data-integrity bug hiding behind a wrong MO attribution, a Tailscale Funnel path twice insisted "impossible"). If you genuinely can't reproduce it, say **that** and ask for the specific data — don't fill the gap with a guess.
- **Before diagnosing a DB / query / sync bug, pull the CURRENT code and confirm the FULL scope first — never reason from a stale or over-narrowed snapshot.** Get the latest live-branch code for the path in question (a fix "verified" against stale code is worthless), and gather the real scope from logs / `pg_stat_statements` / the audit log **before** proposing a cause — don't over-narrow to the single query or row that's in front of you when the offender set is broader. Reproduce the exact discrepancy numbers from raw source tables before accepting any theory; for inventory-sync specifically, reconcile the audit log + verify MO/BOM/product attribution before ever concluding "the receipt is fine."
- **Prefer surgical, targeted edits over broad `sed`/global find-replace.** A wide replace that looks equivalent will silently hit siblings you didn't mean to touch — one timezone fix patched 15 tests and **broke 6 sibling tests** before being redone as a narrow edit. Scope every replace to the exact occurrences you intend (unique `old_string`, or file-/line-bounded), and after any multi-site edit run the affected test file to confirm you didn't catch collateral.
- **Validate that a verification test actually detects the behavior before declaring success.** A test that passes because it silently measures the wrong thing is worse than no test. Prove the assertion has teeth: `SLEEP()` returns `1` when the query is interrupted (so it can look like it "worked"), whole-second timestamp granularity can mask a sub-second gate (e.g. a ≤1s/order check), a cached read can return the pre-fix value. Confirm the check fails on the un-fixed code (or would) before trusting that it passing means fixed.
- **The `serp_test` account is WRITE-CAPABLE — use it to run migrations; don't stall assuming read-only.** SERP's live darklaunch mirror (MySQL `serp_test` on Hetzner, the `live_darklaunch_db` MCP key) has a write-capable account for running migrations/corrective moves — when a task needs a write there, use it rather than hesitating on a blanket "DB access is read-only" assumption. (Odoo / `laravel_live` / other prod DBs **stay strictly read-only** — this exception is `serp_test` only.)
- **Large output → Write it to a file, don't print it inline.** Reports, CSVs, reconciliation dumps, big result sets: use the Write tool and hand back the path. Printing them inline has truncated whole sessions on the output-token limit. Keep chat responses concise; the deliverable goes to disk.
