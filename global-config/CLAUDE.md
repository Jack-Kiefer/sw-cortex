...
Semantic search over `sw-cortex/DICTIONARY.md`. See "Search the Knowledge Base First" above.
...
Read-only access to production databases. **Never run write queries.** **Always include LIMIT.**
...
`query_database_from_file` reads SQL off disk — use for long/awkward queries. File must live under `~/Desktop/Projects` (override via `MCP_DB_ALLOWED_DIRS`).

### Slack Search (`mcp__slack-search__*`)

Semantic search across Jack's Slack history — past discussions, decisions, who said what. Search first, then pull context for interesting hits.
...
**Always specify the correct branch** (`ref` param) — never assume `main`. Active work lives on the Development/Staging branch.

| Need to...     | Do this                                              |
| -------------- | ---------------------------------------------------- |
| Search code    | `mcp__github__search_code { query, repo? }`          |
| Get file       | `mcp__github__get_file { repo, path, ref? }`         |
| List files     | `mcp__github__list_files { repo, path?, ref? }`      |
...
`global-config/` in `sw-cortex` is the source of truth for `~/.claude` config. `~/CLAUDE.md`, `~/.claude/commands/`, `~/.claude/skills/`, and `~/.claude/scripts/` are symlinked into it — edits apply immediately (commit them in sw-cortex). `mcp.json` is generated from `global-config/mcp.json.template` on push:

```bash
bash scripts/sync-global-config.sh pull   # before editing
bash scripts/sync-global-config.sh push   # after editing
```

**Restart Claude Code after pushing mcp.json changes.**

## When in Doubt, Search

- Uncertain about APIs, library versions, errors, best practices → **WebSearch**
- SugarWish systems/tables/people/rules → **`mcp__knowledge__search_knowledge`**
- Past conversations/decisions → **`mcp__slack-search__search_slack_messages`**
