...
- **`git pull` refuses on "local changes" but `git status` says CLEAN → it's a STALE STAT-CACHE, the file IS genuinely dirty — believe the pull, not `status`.** This is a real trap, not a git bug: an in-place edit that leaves the file's size+mtime unchanged (a rewrite that restored the old mtime, or a `checkout` that touched it) leaves git's cached `stat()` matching the index, so `git status`/`git diff`/`git diff-files`/`git diff-index HEAD` all take the fast path and report **clean without re-hashing** — while the merge's own up-to-date check compares the real blob and correctly refuses. Do NOT spiral: the ONE authoritative test is content, not status — `git hash-object <file>` vs `git rev-parse HEAD:<file>` (differ ⇒ dirty), or `git diff --no-index <(git cat-file -p HEAD:<file>) <file>` to see the actual edit. `git update-index --refresh`/`--really-refresh` only re-STATs and will NOT clear it (size+mtime still match); the thing that forces a re-hash is `git update-index --again <file>` or simply reading the file's bytes. Once you've confirmed via content-hash that the file is dirty, it's the same "real local edit on a main clone" case above: STOP and report, don't stash/reset/force. Never loop re-running `status`/plumbing hoping for a consistent answer — status is CACHED and will keep lying; go straight to the content-hash and decide once. (SWAC session, 2026-07-08: ~10 rounds flip-flopping clean/modified/tracked/untracked on `server/db-config.ts` because `status` reported clean while the pull refused; see [[git-status-clean-but-pull-refuses-stale-stat-cache]].)
