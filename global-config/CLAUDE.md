...
- **Writable from the hub: SERP, SWAC, sw-cortex, `sugarwish-infrastructure`, and `herdr`.** The other repos (`sugarwish-laravel`, `livery`, `sw-design`, `swirl`) are **read-only** — read/search them freely to diagnose, but never edit; when a fix belongs in one of them, print a **hand-off note** (what's wrong + file/line + the owner to ask, from the ownership table above) instead of editing. **`sugarwish-infrastructure` is writable for PRs but is Munyr's ArgoCD-managed prod-of-record repo** (the `SERP/serp-prod/` tree auto-syncs to prod): land changes there as a **reviewed PR Munyr sees, never an auto-merge**, and mind the ArgoCD sequencing (e.g. an HPA needs the Deployment's `replicas:` removed + `ignoreDifferences` on `/spec/replicas`, or it thrashes). Requires the repo cloned at `…/sugarwish-infrastructure`.
- **`herdr` is Jack's PUBLIC fork of the open-source [`herdrdev/herdr`](https://github.com/herdrdev/herdr) (Apache 2.0, Rust) — NOT a SugarWish repo.** It is the terminal workspace manager the hub itself runs. It is writable ONLY for **generic, upstream-worthy Herdr changes** (features/fixes that could go back to `herdrdev/herdr`). **🔒 CRITICAL — the fork is PUBLIC: NEVER commit anything personal or SugarWish-related to it.** No company code, DB names (`serp_*`, `laravel_live`, `odoo`, darklaunch), internal paths, SERP/SWAC/WishDesk/Odoo details, people names, credentials, `.env` values, or references to SugarWish systems — in code, comments, commit messages, branch names, or PR text. Keep every change self-contained to Herdr's own domain (panes/tabs/workspaces/sidebar/config/TUI). If a change would need SugarWish context to make sense, it does NOT belong on the fork. PRs go from the fork → `herdrdev/herdr` upstream (or stay on the fork for a local custom build); `origin` = `Jack-Kiefer/herdr`, `upstream` = `herdrdev/herdr`. Default branch is **`master`**, not `main`.
- **This is mechanically enforced.** A PreToolUse hook (`~/.claude/scripts/repo-write-guard.sh`) hard-DENIES any Edit/Write or `git`/`gh` commit·push·merge·worktree·PR whose resolved repo root (via `git rev-parse --git-common-dir`, so worktrees map to their owner) is not SERP/SWAC/sw-cortex/`sugarwish-infrastructure`/`herdr`. Reads are never blocked. (The guard gates *where* you may write; the public-fork content rule above gates *what* you may write to `herdr` — both apply.)
- **Repo roots:** SERP `/Users/jackkief/Desktop/Projects/SERP` · SWAC `…/SWAC` · sw-cortex `…/sw-cortex` · laravel `…/sugarwish-laravel` · livery `…/livery` · sw-design `…/sw-design` · swirl `…/swirl` · infra `…/sugarwish-infrastructure` · herdr `…/herdr` (public fork).
