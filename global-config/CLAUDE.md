...
- The `serp_test` MCP key (MySQL `serp_test` on Hetzner `5.161.233.240:3306`) is the **REAL live production darklaunch mirror** — the name "test" is a **lie**; it is the most-current copy, not a throwaway/pytest DB. **ALWAYS use `serp_test` when reproducing/diagnosing what the live darklaunch worker or drift monitor actually sees — NEVER a local Docker DB.** The app/`darklaunch_mysql_pool` defaults to a local darklaunch DB that is reseeded from scratch and contains ZERO worker-created rows (`id != odoo_id`) — so reproducing a drift/worker comparison there silently hides the exact rows the bug is about. Compare against live via the `serp_test` MCP key. (The old `live_darklaunch_db` MCP key was removed — it pointed at this same DB; use `serp_test`.)
