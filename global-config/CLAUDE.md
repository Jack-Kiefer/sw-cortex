...
- **AskUserQuestion popups are for INTAKE ONLY — every mid-flow question goes in prose.** The popup is reserved for the options-first intake at task start. Once research/build is underway, ask Jack anything (clarification, approval, a choice) as plain text at the end of the turn — never an AskUserQuestion popup. A rejected popup means "I'll answer in text": never re-fire it, just restate the question in prose and end the turn. (23 popup rejections in 3 days, 19 in SERP sessions that can't see the sw-cortex-only memory `askuserquestion-jack-answers-in-text`.)
