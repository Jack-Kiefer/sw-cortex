...
- **CAP THE MEMORY OF EVERY `tsc`, Jest, `vite build`, and `npm audit` YOU SHELL OUT — an uncapped one can OOM-kill Jack's whole machine.** This is not theoretical: on 2026-07-31 two Jetsam kill events (13:09 + 13:10) took the Mac down. Three `node` processes — each **under 15 seconds old** — hit **5,597 MB / 4,625 MB / 2,373 MB** (12.5 GB in seconds), from an `npx tsc --noEmit -p tsconfig.json` in one SWAC session and a 4-file `npm run test:file` in a second SWAC session running concurrently, while 4 subagents of the first session also shelled out to `npm audit`/`npm ls`. Free memory hit **~91 MB**. **The Mac has only 16 GB of RAM and a 3 GB swap file** — and at crash time a `com.apple.Virtualization.Virtual` VM (Docker/UTM-class) held **8.1 GB**, i.e. *half the machine*, leaving ~8 GB for every Claude session, VS Code, Chrome, Slack, and mysqld combined. Two uncapped `tsc`/Jest runs alone exceed that. **You never have the machine to yourself, and there is far less headroom than a dev box normally implies.** So:
  - **Always prefix these commands with a heap cap and a worker cap.** `NODE_OPTIONS=--max-old-space-size=2048 npx tsc --noEmit …`; Jest/vitest always get `--maxWorkers=2` (never the default one-worker-per-core; do NOT use `--maxWorkers=50%`, which still scales with cores). On 16 GB, 2048 MB is the right ceiling — the crash-causing `tsc` runs peaked at 4.6 GB and 5.6 GB, so a cap this low is what makes them fail loudly instead of taking the machine down. A full `npm run test`/`npm run typecheck` with no cap is the single most dangerous command you can run here.
  - **Serialize, never parallelize, the heavy ones.** Never run typecheck and tests concurrently, never run them in two `run_in_background` calls at once, and never let subagents each fire their own `npm`/`tsc`. If a session already has a dev server up, that's memory too — a build on top of it is additive.
  - **Prefer the narrow command.** `npm run test:file -- <one file>` over the suite; `tsc --noEmit` on the changed project only; `npm ls <pkg>` over `npm audit --json` (which resolves the whole tree). Read `package-lock.json` with `grep`, never by `json.load()`-ing it in Python (it's tens of MB).
  - **Cross-session awareness is on you.** Jack routinely runs 5+ concurrent sessions (2× SWAC, 2× SERP, hub). Before a heavy build, `ps -eo rss,command | grep node | sort -rn | head` — if another session is already mid-`tsc`/Jest, WAIT for it rather than stacking. If free memory is already thin, say so and ask instead of launching the build.
  - **If a machine crash/hang/"my computer froze" is reported, check `/Library/Logs/DiagnosticReports/JetsamEvent-*.ips` first** (root-owned system dir — NOT `~/Library/...`, which has no Jetsam files); `processes[].rpages × 16384` is resident bytes, `lifetimeMax` the peak, and a large `rpages` with a tiny `age` is the smoking gun.
