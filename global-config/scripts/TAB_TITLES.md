...
| Default hook    | `~/.claude/scripts/tab-title-default.sh` | Floor: titles a session that never called the setter (`🔍 <repo> · session`; the sw-cortex hub floor is plain `hub`, no emoji — but see "Hub floor upgrade" below). With `--prompt` (UserPromptSubmit) it ALSO demotes a leading 🙋/❓/✅ → 🔨 — see "Auto-flip" below |
...
**Hub floor upgrade (`hub` → the actual task).** The `hub` floor is picked by **cwd**, but under the hub model the cwd is _always_ `sw-cortex` no matter which repo the work concerns — so a hub tab investigating a SERP bug still read `hub`, and an inline `/go` task that never called the setter stayed that way for its whole life. So on `--prompt`, when the stored title is the **exact bare floor** `hub`, the hook replaces it with a 🔨-prefixed slug derived from Jack's prompt (first meaningful line, ≤52 chars). For a **slash command** the slug comes from `<command-args>` only — never the command's own markdown body — so `/go fix the forecast zeros` titles the tab `🔨 fix the forecast zeros` while a bare `/go` (empty args) correctly stays at `hub`. Only the literal `hub` is eligible: any title the model set, or a previous upgrade, is left alone, so the model's own richer status always wins. To change the extraction, edit `slug_from_prompt()` in `tab-title-default.sh`.
