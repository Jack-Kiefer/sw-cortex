...
- **`serp_app` = LIVE/production SERP; `serp_test` = STAGING SERP** (both MySQL on Hetzner `5.161.233.240:3306`). `serp_app` is prod but does **not** hold the `serp_*` tables yet (arriving via migrations), so the populated `serp_*` tables + darklaunch history live in `serp_test` today. The name "test" is a misnomer (real staging copy, not a throwaway/pytest DB) — but `serp_test` is **NOT production** (prod is `serp_app`). (The old `live_darklaunch_db` MCP key was removed; `serp_test` points at the same DB.)
