...
- ⚠️ **The publish pattern — the concrete tables Laravel reads instead of joining across the DB boundary (from the 2026-08-03 "SERP Cutover Strategy" + "Usage Inventory" artifacts, the most-current operational plan; the in-repo `serp-odoo-cutover.md` is the engineering half).** SERP owns the computation, **publishes** the answer into `laravel_live`, and Laravel reads it locally — no query ever spans two servers. The three published surfaces:
  ...
  - **`serp_sync` columns** on `ec_order`/`preselect_orders` (each with a `ship_date_serp_synchronized` ship-date twin) — replaces the `serp_sale_order` existence test. **Values: `0` placed · `1` ingested/reserved · `5` bypass** (`bypass_odoo` product types). ⚠️ **`items` has NEITHER — only `odoo_sync`** (live-verified `laravel_live` + `manage`, 2026-08-19); item-level sync reads its PARENT order's flag, not a per-item column. `items.serp_sync` is a still-_planned_ mirror (see the `oddo_synchronized` TL;DR line), not yet built.
