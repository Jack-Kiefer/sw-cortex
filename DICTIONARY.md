...
- `inventory_qty` (Englewood/current SA), `mi_inventory_qty` (Taylor), `odoo_inventory` (synced cache, can be NEGATIVE -122/-44/-325, stale — use `inventory_qty` for decisions), `prelist_qty` (int NOT NULL, default 0 — **prelisting**: qty Sophie wants sellable before an MO is packed; added 2026-07-01, all-zero/not yet populated as of 2026-07-02; the Odoo→Laravel inventory sync cron ADDS it into `inventory_qty`, SERPY SUBTRACTS from it — floored at 0 — when it MOs the product; reporting must SUBTRACT `prelist_qty` to avoid double-counting; does NOT touch Odoo, so costing stays correct), `inventory_link` (parent product_id for cascading disables — set NULL on combo-line children in Apr-2026 migration), `location_id` (FK locations; **ALL ~5,150 rows NULL — field unused**), `vendor_id`, `vendor_cost` (decimal 8,2), `product_id` (= `sugarwish_id`, join to `product_template.sugarwish_id`), `product_type`, `tango_utid`, `odoo_id`. **Duplicate SKUs exist** (SA-01-026-A as product_ids 1706+4499; 13 component SKUs duplicated) — use `product_id`/`id` for joins, NOT SKU.
