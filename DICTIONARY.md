...
- ⚠️ **The publish pattern — the concrete tables Laravel reads instead of joining across the DB boundary (from the 2026-08-03 "SERP Cutover Strategy" + "Usage Inventory" artifacts, the most-current operational plan; the in-repo `serp-odoo-cutover.md` is the engineering half).** SERP owns the computation, **publishes** the answer into `laravel_live`, and Laravel reads it locally — no query ever spans two servers. The three published surfaces:
  - **`serp_published_stock`** (in `laravel_live`, ~2,000 rows) — one row per SERP variant, keyed on `serp_product_product.id`, carrying **both** bridge FKs (`receiver_product_id` INT for finished goods, `component_id` BIGINT for raw materials — exactly one set), columns `on_hand_qty`/`reserved_qty`/`unreserved_outbound_qty` (all `DECIMAL(16,4)`, **`on_hand_qty` is SIGNED** — negative is real) + `computed_at`. **No FKs** (referents live in another DB). Replaces the view legs that used to join `serp_stock_quant`/`serp_stock_move` across the boundary. The **internal-location filter (`serp_stock_location` `usage='internal'`) lives in the publisher**, because `serp_stock_location` does not exist in Laravel.
  ...
  - **`serp_sync` columns** on `ec_order`/`preselect_orders` (+ `items.serp_sync`, + `ship_date_serp_synchronized` for the ship-date twin) — replaces the `serp_sale_order` existence test. **Values: `0` placed · `1` ingested/reserved · `5` bypass** (`bypass_odoo` product types).
