...
- **SERP's production data lives on its OWN box, NOT the Laravel cluster** (corrected 2026-07-30; the old "SERP has no dedicated production DB / it runs on the live Laravel/MySQL cluster" line was stale). The **serp-db** box `5.161.233.240` carries two SERP databases: **`serp_test`** (the live darklaunch ERP mirror — `serp_stock_move`, `serp_account_move_line`, SVL, quants) and **`serp_app`** (the app layer — Serpy, forecast config, crons, audit). There is still no server literally named `serp_prod`, but "SERP has no production DB of its own" is now WRONG. The `serp_*` tables inside `laravel_live` remain the near-empty pilot bridge (~19 `serp_sale_order` rows) and are NOT live SERP data.
- **The Odoo→SERP business cutover is `2026-09-26`, executed by ONE flag: `ENVIRONMENT_SET` (`odoo` → `serp`).** `backend/config.py` (line refs re-verified 2026-09-11 — the old `:92`/`:817`/`:829`/`:1072` had all shifted): **`:120`** declares it, **`:1080`** `source_is_serp` is exactly `ENVIRONMENT_SET == "serp"`, **`:1103`** `DATABASE_URL` then routes the whole app ORM onto the darklaunch DB, **`:1179-1185`** hard-fails if it's paired with a local `DARKLAUNCH_DB_ENV` on prod. Flip-day order: **Odoo crons off (freeze a static reference) → pause the SERP worker → halt ops → seed → validate inventory → enable.** **FULL historical Odoo seeding is MANDATORY — no 2026-onward cutoff** (FIFO costing builds cumulatively from zero; 7-year audit window), est. 3+ hrs. ⚠️ Do NOT infer prod seeding policy from the **local** 90-day cap (`SEED_RECENT_DAYS`/`DEFAULT_RECENT_DAYS = 90`) — that cap only exists to keep local Docker disks alive. In-repo plan of record: `docs/plans/serp-odoo-cutover.md` (SERP PR #705). ⚠️ Whether the SERP DBs stay separate from `laravel_live` **permanently** is the working plan but NOT signed off — deferred pending architecture scenarios.
- **`serp_test` and `serp_app` now run INDEPENDENT `_migrations` tracks — `serp_test` is STAGING and legitimately runs AHEAD of prod `serp_app`.** Verified live **2026-08-10**: both `serp_test._migrations` and `serp_app._migrations` at **078** (as of 2026-08-03 they were at 072 / 071 respectively, with 072 present only on `serp_test`) — two separate DBs each with their own `_migrations` table, applying the same `serp_app/migrations/*.sql` files at different times. (This supersedes the 2026-07-30 reading that "both tracks land in `serp_test`"; one-DB-per-environment is now the behaviour, not just the target.) ⚠️ **A version gap between the two is the NORMAL steady state, NOT a failed or mis-targeted migration** — staging is migrated first, and prod catches up on deploy: `deploy-k8s.sh` runs `migrate:serp-app` at phase **[3/6]**, _before_ the ArgoCD rollout, so columns land ahead of the new pods. (serp-dev's deploy deliberately does NOT auto-run serp-app migrations — it shares the prod `serp_app` DB.) ❌ Do not conclude "the migration didn't work / went to the wrong database" from a change being visible in `serp_test` but absent in `serp_app`; establish which DB was _intended_ first. Both DBs contain same-named tables (`serp_ai_turns_live`, `serp_ai_turns_dev`, …), so presence in one says nothing about the other — but the DATA lives in `serp_app` (e.g. 1,462 `serp_ai_turns_live` rows vs 0 in `serp_test`).
- **Live `serp_test` contains ZERO Laravel catalog tables — and the copier that used to mirror them is GONE.** Verified 2026-07-30: none of `components`, `receiver_products`, `items`, `ec_order`, `buyer_products`, `inventory_reservations`, `kits`, `component_kits`, `odoo_inventory` exist there. **`staging_copier_worker.py` was DELETED outright on 2026-07-30 (SERP PR #707, commit `94deba129`)** — the module, the `STAGING_COPIER_ENABLED` flag, its k8s env var, and its tests are all gone, so the Laravel-catalog mirror no longer exists anywhere (not even locally). Any reasoning of the form "the cross-DB `serp_*` ↔ Laravel joins are fine because the copier keeps both sides co-resident" is now **false everywhere**, not just on the live box. The old `_CATALOG_TABLES`/`kits` gap and the `ec_order` dup-bloat bug are moot — do not cite `staging_copier_worker.py:NNN` line numbers; the file does not exist. Laravel-owned data is reached through the Laravel pools (see the pool-routing bullet), not through a mirrored table.
...
- **`serp_app` = LIVE/production SERP; `serp_test` = STAGING SERP** (both MySQL on Hetzner `5.161.233.240:3306`, `LIVE_DARKLAUNCH_DB_*`). This is the cutover target framing (one DB per environment — see the cutover-plan bullet): **`serp_app` is production but does NOT hold the `serp_*` tables yet** — they arrive via migrations (e.g. `069`), so today `serp_app` has the non-SERP app schema and `serp_test` (staging) is where the populated `serp_*` tables live. `serp_test` has historically been the darklaunch mirror (the name "test" is a misnomer — it is a real, most-current copy, not a throwaway/pytest DB), and the darklaunch worker/copier facts below still describe its contents; just do not read "live PRODUCTION" onto `serp_test` — **prod is `serp_app`**. (The old `live_darklaunch_db` MCP key was removed; `serp_test` is that same DB.)
...
- The Odoo sync flag column is intentionally misspelled **`oddo_synchronized`** (double-d, one o) — match it exactly. `5` = error. ⚠️ **Value `3` does NOT exist in prod** — live `laravel_live.ec_order` 2026-09-11 has only `1`=3,973,097, `0`=80,917, `5`=18,727, `2`=**1 row**. Do not write diagnostics that filter on `=3`. (The old "`3` = stuck/archived-SKU" reading conflicted with three other passages in this file; the live query settles it.) **At cutover the flag FREEZES at whatever value it holds** (Odoo was its only writer, advancing 0→2→1 via `/api/odoo` callbacks) — every order placed after the sync crons stop stays at `0` forever, so any code gating on `=1` silently matches nothing and any code reporting `!=1` as "unsynced" silently reports everything; `=5` (bypass_odoo product types, e.g. Gift Cards/Vinebox/Bouqs) is the one value that must survive/be rehomed off the column. **Jack's decided replacement (2026-07-28) is a like-for-like MIRROR, not a re-architecture: add `serp_sync` twins that SERP advances exactly where Odoo advanced the old flags** — `ec_order.serp_sync` (mirrors `oddo_synchronized`; SERP writes it when the order worker creates the `serp_sale_order` row) and `items.serp_sync` (mirrors `items.odoo_sync`, `tinyint default 0 after odoo_sync`, index `(product_id, serp_sync)`) — **`ec_order`'s twins ARE built and backfilled (verified live 2026-08-26: `ec_order.serp_sync` + `ec_order.ship_date_serp_synchronized` on both `laravel_live` and `manage`; ~599.7K/599.7K `store_id=2` orders at `serp_sync=1`, matching `oddo_synchronized`). `items.serp_sync` is still NOT built (`items` has only `odoo_sync`, both DBs). On prod, `serp_sync` is only backfilled, NOT yet live-advanced — the order-queue worker that writes it runs on serp-test only (see the darklaunch order-worker note), so recent prod orders diverge from `oddo_synchronized`**. Downstream stays untouched: the `SynchronizeOdooInventory` Lambda netting is a **one-predicate swap**, `i.odoo_sync IN (0,1)` → `i.serp_sync IN (0,1)` (the `pt.bypass_odoo = 0` filter and `% 10` sharding stay). ⚠️ **The backfill is LOAD-BEARING** — every historical row defaults to `serp_sync = 0`, so on flip day the Lambda would count ~14.6M items as pending and net every product deeply negative, disabling the entire catalog. Backfill BEFORE the flip, batched and ahead of the freeze window: `items` `serp_sync=2` where `odoo_sync=2` / `=5` where `=5`; `ec_order` `serp_sync=1` where `oddo_synchronized=1` / `=5` where `=5` (SERP can self-verify the order backfill — `serp_sale_order.ec_order_id` already covers 2.13M of them). Ship the column EARLY and run both flags in parallel for a few weeks to diff them, so the flip is one predicate change with evidence behind it. Also decided: SERP keeps seeding `receiver_products.odoo_inventory` the way Odoo does today rather than changing readers — "I want the system to work pretty much like it does now."
- **SERPY PO-receipt idempotency guard was demand-blind until 2026-07-30 (SERP PR #710, `38b757528`) — a receipt could report "Fully synced" while never posting the stock move.** `_execute_po_receipt`'s `_already_received_result` summed a SKU's `qty_done` across **ALL** the origin's DONE pickings and compared that cumulative total to only the NEW payload qty, so any SKU that had ever received >= the new amount falsely passed as "already received" — silently skipping a genuinely-new partial receipt against an open backorder. Step 1's open-picking search also filtered on the **non-stored** `picking_type_code`, which can return empty over XML-RPC and mask a real open picking. Trigger scenario: a PO whose SKU was already partially received across prior DONE backorders, receiving again against a still-OPEN picking. Incident: PO **P02123** drafts **#1728/#1748** — 176 metal spreaders (`RM-39-096-A`) reported synced, never posted; ledger sweep found **3 affected POs (P02123 / P02065 / P02083, ~348 units)**, all recoverable via their open pickings after deploy. Fix: the guard is now demand-aware (`_origin_has_open_demand` — it refuses to claim "already received" while an open incoming picking still has un-received demand for a payload SKU), and picking lookups filter on the **stored `picking_type_id`**. ⚠️ **Failed/falsely-synced SERPY ops are never auto-repicked** — recovery is an explicit re-queue via `/api/admin/sync-queue` or a DB update to re-run those drafts.
...
- Pre-cutover, **Odoo is the inventory/accounting source of truth**; SERP/Serpy READ live from Odoo, so discrepancies usually originate in Odoo's data, not SERP's. **At cutover (~2026-09-26) this inverts hard and permanently: SERP becomes master, Odoo freezes read-only and is never written to again — there is no mirror, no shadow leg, no rollback feed.**
- SERP production does **NOT** auto-deploy from CI, and since 2026-08-13 `deploy-k8s.sh` no longer performs the rollout itself — **prod is GitOps via ArgoCD.** Deploy is still kicked off manually on the Hetzner K3s node (`ssh jack@5.161.95.56` then `bash deploy-k8s.sh main`), but that script only PREPARES the release: [1/6] pull → [2/6] build + **push images to ECR** (`150866789693.dkr.ecr.us-east-1.amazonaws.com/serp-{backend,frontend,edge-auth}`) → [3/6] serp-app migrations → [4/6] upsert Secrets + **rewrite image tags in `sugarwish-infrastructure` under `SERP/serp-prod/` and git push**. The actual rollout is done by the **ArgoCD `serp-prod` Application**, which watches that path on `main` — the script explicitly does NOT `kubectl apply` Deployments/Services/Ingress, and ArgoCD reverts manual `kubectl` edits to them. ⚠️ ArgoCD does **not** run on the `5.161.95.56` cluster (no argo CRDs there); it lives on a separate control cluster, UI at **https://deploy.sugarwish.com**, owned by Munyr in the read-only `sugarwish-infrastructure` repo. **`serp-prod` is on AUTOMATED sync today** (`syncPolicy.automated.enabled: true`), so the phase [4/6] tag push auto-rolls — SERP's `/deploy` command stops after [4/6] and prints a "press Sync" handoff, but that handoff only truly blocks once Munyr flips the syncPolicy to manual. (Verified live 2026-08-14: prod pods running ECR image tag `23ecb56de`. The old AWS `deploy.sh` path is frozen legacy.)
...
- **SWAC IS WishDesk** — the GitHub description "SugarWish Activity Coordinator" is misleading. ⚠️ **The `knowledge` MCP also indexes swirl, whose `reference/glossary.md` CONTRADICTS this** — it lists SWAC and WishDesk as two separate systems and calls SWAC "Next.js" and WishDesk "Laravel". **All three are wrong** (verified 2026-09-11: SWAC is React 18.3 + Express + Vite + Drizzle — no Next.js, no Vue; `swirl/architecture/swac/overview.md` itself says "WishDesk (codebase name: SWAC)"). swirl is read-only from the hub, so if a search returns that glossary row, **trust this line instead**.
- Inventory has **no single source of truth**: sellable (SA) = Laravel `receiver_products.inventory_qty` **(until cutover — then the `receiver_product_inventory` view's `sellable_qty`, `serp_stock_quant`-backed)**; raw material (RM) = Odoo only; accounting/valuation = Odoo `stock_quant`/SVL **(all three consolidate into SERP's own `serp_*` tables at cutover — per the 2026-07-29 revision these STAY in `serp_app`, NOT bulk-moved into `laravel_live`; Laravel reads an aggregated stock table SERP publishes into it, since it can't join across the DB boundary)**. Manual inventory corrections go through **SERPY** draft→approve. ⚠️ **Several sellable_qty repoints already SHIPPED AHEAD of the Sept 26 cutover — verified in SWIRL 2026-09-09: WW-2442 (prepick availability), WW-2444, and WW-2810 (receiver availability service) are all `status: released`, `released_date: 2026-09-09 06:56`.** So the reader-side moves to the `receiver_product_inventory` view's `sellable_qty` are partly LIVE now, not cutover-gated, and the Laravel admin inventory-transaction UI has already retired. Older notes on this point say "post-cutover" and are stale — **check what is actually live before citing a cutover-gated date here.**
...
- Single endpoint `POST /api/call_kw` dispatches to `@api.callable` model methods. New logic → fat ORM models in `serp_orm/models/`. **Never** add per-resource REST routers for ORM-backed resources. REST routers (`backend/routers/`) — ⚠️ **the "only for auth, forecast, Odoo sync, inventory_sync, receipts" list is stale** (2026-09-11: **39 router modules**, incl. chatter, locations, `inventory_counts`, `inventory_variance`, `sku_costs`, `shipping_cost_dashboard`, `reports`, `admin`, `odoo_*`). The **rule** still stands: new ORM-backed logic goes in fat `serp_orm/models/`, not a new per-resource REST router — the list was never an exhaustive inventory.
- **Canonical approved divergences** (documented in `docs/ODOO_APPROVED_DIFFERENCES.md`; suppressed in drift monitor): `button_confirm` allows empty PO (test-pinned); ~~no bin-level putaway~~ (**retracted 2026-09-11 — see "`_gather()` DOES expand child locations"; SERP DOES resolve to child bins, and this item's drift-monitor suppression may be hiding a real corruption signal**); byproducts via `serp_mrp_bom_byproduct`; sale-order names `f"S{ec_order_id}"` not via `ir.sequence` (**narrowed 2026-09-11: true of sale orders ONLY — `ir.sequence` itself is fully implemented** (`ir_sequence.py`, `next_by_code`) and IS used for purchase.order, mrp.production, mrp.unbuild, stock.picking, stock.scrap, stock.landed.cost and account.reconcile; don't read this as "SERP has no `ir.sequence`"); no invoicing flow; no `supplier_rank` bumps; SERP uses **CONTINENTAL** accounting (Odoo = Anglo-Saxon); ~~no reconciliation engine~~ (**corrected 2026-09-11 — SERP HAS one**: `account_move_line_reconcile.py` is a 610-line Odoo-15 port with `reconcile()`, partials, `remove_move_reconcile()`, plus `account_full_reconcile`/`account_partial_reconcile`, shipped 2026-07-24. **But it does NOT drive `payment_state`** — move-level `amount_residual` is still a plain field the engine deliberately doesn't update ("lands in PR 2", which never landed), so `action_reverse` still force-writes both, behind a now-stale "SERP has no reconciliation engine" comment at `account_move_actions.py:464`). **Any OTHER divergence = bug.** Fix against Odoo 15 source (`raw.githubusercontent.com/odoo/odoo/15.0`). Slash commands: `/check-odoo-alignment`, `/odoo-fix-divergence`.

**SERP auth (two unrelated layers):**

- **HTTP Basic Auth** — ⚠️ **no longer nginx `/etc/nginx/.htpasswd`** (corrected 2026-09-11). It moved to a dedicated **`serp-edge-auth` K8s Deployment** (`edge_auth/server.py:26-29`: this "is the source of truth for the credential — **nginx no longer consults serp-basic-auth**"; `k8s/edge-auth.yaml:25`, 2 replicas), and `deploy-k8s.sh:276` upserts the `serp-edge-auth-cred` secret on every deploy. The old note that "`deploy.sh` does NOT regenerate `.htpasswd`" is doubly stale — `deploy.sh` itself no longer exists. Still true: the credential is browser-cached (Safari flushes aggressively), so a repeated "sign in" popup = Basic-Auth cache flush, **NOT** a JWT bug.
...
- **Production is the Hetzner K3s cluster** (node `5.161.95.56`, namespace `serp`, app root `/opt/SERP`, live host `serp.sugarwish.com` — health-check from the node, the hostname doesn't resolve from Jack's Mac). Deploy is started with `bash deploy-k8s.sh main` on the node; the node tracks `main` and this is the normal, safe deploy (verified 2026-06-10 — do NOT re-flag "the script only lives on a feature branch"). ⚠️ **Since 2026-08-13 that script only PREPARES the deploy — it builds + pushes images to ECR, runs serp-app migrations, and bumps the image tags in `sugarwish-infrastructure` (`SERP/serp-prod/`); the ArgoCD `serp-prod` Application (separate cluster, UI `deploy.sugarwish.com`, automated sync today) performs the rollout.** The script does not `kubectl apply` Deployments/Services/Ingress, and its `[5/6a]` "waiting for ArgoCD to roll out" phase failing is NOT a failed build — everything is already staged in ECR + the infra repo. Darklaunch prod DB on Hetzner (`5.161.233.240`).
- **serp-dev — auto-deployed dev stack (PR #344, WW-1686, 2026-07-02):** `dev` deploys via **Claude's standing-authorized post-merge step** (Jack's 2026-07-20 change: after merging a PR into `dev`, the session runs the serp-dev deploy itself — the old `.github/workflows/deploy-dev.yml` GitHub auto-deploy was replaced; `deploy-k8s-dev.sh` on the node, serialized) to a parallel stack in the same `serp` namespace at **`serp-test.sugarwish.com`** (a web HOST — do not confuse with the `serp_test` DB): clone `/opt/SERP-dev`, images `serp-dev-backend`/`serp-dev-frontend` (1 replica each; backend mem 1Gi after live-products OOM, PR #363), secret `serp-dev-env`, TLS serves the pre-existing `serp-tls` cert (nginx's default cert broke on HSTS, PR #349). ⚠️ **Corrected 2026-09-11 — "NO workers, NO `migrate:serp-app`" is stale on both halves.** `deploy-k8s-dev.sh:17-20`: **`serp-dev-workers` exists** as a MANUAL-ONLY worker pod (odoo-sync runs; the cron runner drains "Run now" for `env='dev'`). `:30-41`: **`migrate:serp-app` DOES run** (`npm run migrate:serp-app` at `:270`) — but only against **serp-dev's OWN app DB**, with phase [4/7] hard-failing if that DB resolves to `serp_app`. The underlying concern (prod `serp_app` must not get dev-branch migrations early) is enforced by that guard, not by skipping the step. Serpy IS active on serp-dev (verified 2026-07-22: dev drafts write the BASE `serp_draft_operations` table and the dev sync queue `odoo_sync_queue_dev`; the old "blank ANTHROPIC*API_KEY / dev Serpy can't enqueue" claim is stale — the key was added to `serp-dev-env`). It runs a **new third `ENVIRONMENT=dev`** (alongside `local`/`production`, Jack's call — no new env var): `IS_LOCAL` stays false so DB routes production-style through `LIVE_DB*\*` → the **manage cluster** (`5.78.203.128/sugarwish_manage_new_2`), serp_app/Serpy tables resolve to the NON-live variants via `settings.SERP_APP_TABLES_ARE_LIVE`(see Retool suffix convention), and Odoo XML-RPC routes to **staging for ALL non-prod envs** (pre-fix, dev workers fired XML-RPC at PROD Odoo — PRs #352/#354,`TREAT_AS_LOCAL`added). Prod deploy from`main` stays manual; only the dev stack auto-deploys. ⚠️ **ECR shared-repo outage (2026-08-24) — now split:** serp-dev and prod originally pushed to the SAME ECR repos (`serp-backend`/`serp-frontend`) under a "keep 5 newest images (any tag)" lifecycle rule, so frequent serp-dev deploys aged the live prod tag (`serp-backend:f4ea6f7a8`) out of the repo → on the next prod pod restart `ErrImagePull`/`403`→`serp-backend 0/2 ready`→ nginx 503 on everything incl. login (a merge/deploy to prod was NOT involved). Remediation (in`sugarwish-infrastructure`, Munyr's ArgoCD repo): **dev-only repos `serp-backend-dev`/`serp-frontend-dev`** created, serp-dev manifests repointed to them, lifecycle split (prod = untagged-cleanup + keep 10; dev = keep 5); the SERP-side follow-up repointed `deploy-k8s-dev.sh`'s push targets to the `-dev` repos. Takeaway: serp-dev deploys must never share an ECR repo (or a keep-N-any-tag lifecycle) with the live prod image, or dev churn can evict the in-use prod tag.
...
- **Image accumulation:** un-pruned, each deploy leaks ~1.3GB into `/var/lib/containerd` (Docker uses the containerd image store — `/var/lib/docker` looks tiny and misleads `du`); kubelet image GC force-kicks at 85% disk. `deploy-k8s.sh`'s prune phase — ⚠️ **now `[6/6]`, not `[7/7]`** (`deploy-k8s.sh:430`; the script is `[N/6]` per `:12-18`, migrations at `[3/6]`) — keeps current `$TAG` + `$PREV_TAG` + `:latest`. ⚠️ The old parenthetical "captured BEFORE `kubectl apply` rewrites spec images to `:latest`" is **obsolete**: the script no longer `kubectl apply`s Deployments at all (`:20-21` — "This script does NOT kubectl-apply Deployments/Services/Ingress. **ArgoCD is the only writer**"). Plus `docker builder prune --keep-storage=8G plus `docker builder prune --keep-storage=8GB`.
...
- **Deploy does NOT run migrations against the prod `manage` cluster (now on Hetzner, not AWS RDS).** (⚠️ `deploy.sh` itself no longer exists — removed with the AWS/PM2 stack; the current deploy is `deploy-k8s.sh`. The rule below is unchanged.) New `serp_*` tables/columns must be applied to the live `manage` DB manually (by Manish + DBA) **BEFORE** code ships.

**SERP workers** — 🛑 **HISTORICAL: this PM2 table describes the FROZEN AWS EC2 box only** (corrected 2026-09-11). `deploy.sh`, `ecosystem.config.js` and `deployment/` no longer exist in the repo; production runs on Hetzner K3s where these are **Deployments, not PM2 apps** (`serp-backend` replicas 2 per `k8s/backend.yaml:40`; workers gated by `WORKERS_ENABLED` via the `serp-env` secret, `k8s/workers.yaml:73-75`). Kept for reading old runbooks:
...
- **Dual-write / parallel-run validation, NOT a feature-flag library.** ⚠️ **The file is `backend/workers/serp_order_queue_worker.py`** — renamed 2026-07-30 (`67fa02e60`); `serp_order_queue_worker.py` no longer exists on disk, so stop citing it. It **REPLAYS** Odoo's order writes through the SERP ORM into a separate MySQL mirror. **Never** touches live Odoo or the main SERP ORM pool.
...
- **Invariant:** darklaunch must NOT change what the underlying op does. ✅ **FIXED 2026-09-05 (`38dd6ed72`) — the violation below is HISTORY, not current behavior** (corrected 2026-09-11): `handlers/manufacturing.py:640` now documents the live-Odoo write as "deliberately **INDEPENDENT of `SERP_DARKLAUNCH_ENABLED`**" and `:659` uses `payload.components[].quantity` verbatim on both paths, CI-pinned by `test_mo_create_darklaunch_additive.py`. The ~620-MO reconciliation backlog remains valid as history. ⚠️ **What was VIOLATED ~2026-05-28 → 2026-07-29 in the MO-create handler (found + flag turned OFF 2026-08-04):** `backend/workers/handlers/manufacturing.py` branches on `SERP_DARKLAUNCH_ENABLED`: the OFF path (~L733) uses `payload.components[].quantity` verbatim (the counted slip figure = source of truth); the ON path (~L669–682) instead re-reads `mrp.bom.line.product_qty` from Odoo and writes `quantity = bom_line.product_qty × (product_qty / bom_qty)` = **BOM ratio × cup count**, discarding the payload quantity (payload demoted to a `hint` used only for `location_id`/`sku`). The flag doesn't "also-mirror" here — **both branches `xmlrpc.create("mrp.production", …)` against LIVE prod Odoo**, so flipping darklaunch ON silently changed the production MO write. Motive (comment ~L642): make Odoo's MO raw-move row-set match SERP's `bom.explode` output so the stamper stops seeing a row-count mismatch — i.e. it "made the measurement pass by moving the thing measured." **Impact:** ~620 MOs across ~123–135 Serpy drafts (and non-Serpy MOs too), **71 finished-good SKUs stopped recording real slip consumption** (worst: Pistachios back to 02-18); every affected MO matches BOM to `diff=0.0000` (signature of a computed, not typed, value), so any COGS/inventory reading off RM consumption is off by the BOM-vs-actual gap. **Turning the flag OFF stops new bad writes but self-corrects NOTHING** — the ~620 `done` MOs already consumed/valued in Odoo need manual reconciliation. (A follow-up audit — "does darklaunch change any OTHER live functionality" — is separate and not yet resolved.)
- Cutover was historically recorded per-env at `serp_darklaunch_meta.darklaunch_cutover_at` (prod `2026-06-04 09:27:20`, staging `2026-06-03 11:52:27`). ⚠️ **`serp_darklaunch_meta` was DROPPED 2026-08-03 (commit `809a4d8f9`, "the seed is DATA ONLY — drop create-db, migrate, and cutover") — the table no longer exists** (verified live 2026-08-10: absent from `serp_test.information_schema`). Nothing reads `darklaunch_cutover_at` any more; new-order detection is scoped by the per-order SERP flags (`serp_sync`, `ship_date_serp_synchronized`) instead. Validation gate was **<1% drift, stable 2 weeks**. Event log: `serp_darklaunch_processed_events`. Tools: `/compare-darklaunch`, `/compare-orders`, `compare_odoo_replica.py`.
...
- Pipeline: `classify_intent → find_products → propose_operations → DRAFT (serp_draft_operations/_live) → /save-raw-draft` (validates against `OpTypeRegistry` in `serpy/ops/types.py`) `→ /ai-submit` (DRAFT→PENDING_APPROVAL) → human approval → **`serpy_sync_queue` (MySQL `serp_app`)** → odoo-sync worker (⚠️ **MOVED 2026-09-11: the live queue is `serpy_sync_queue` in MySQL `serp_app` (env-routed); the Retool-PG `odoo_sync_queue*` tables are FROZEN since 2026-06-26.**) (~30s poll) → XML-RPC + local mirror + Laravel (`manage` MySQL).
...
- **Op rules live in the DB, not code (SOT flip 2026-07-21, PRs #595/#600, deployed):** `SERPY_OP_RULES_FROM_DB=true` on BOTH prod and serp-test — the loader reads active rule chunks from the DB (**67 active** as of 2026-09-11 — the old "~61" is stale; treat this count as volatile) (`MergedRegistry`; no fail-open — a DB read failure does NOT silently fall back to code). Built-in always-on guards (`frame.base`, `never_guess`, `phantom_bom_routing`, …) still come from code by design; op-scope rules come from the DB. **Editing the prompt/rules in code does nothing for op rules** — edit live via the `/serpy-rules` UI, durable via a `gen_rule_edit` migration (runner-tracked in `_migrations`); a drift guard keeps code↔DB aligned. Per-turn rule-selection telemetry (`SERPY_PERSIST_CHUNK_SELECTION=true`) records the used `chunk_selection` ("Rules used this turn" panel).
...
- ✅ **CONSOLIDATED — the old "two orchestration paths, fixes don't propagate" footgun is GONE** (corrected 2026-09-11). `services/forecast/service.py:1-11`: orchestration "now lives **entirely** in `SupplierForecastPipeline`", which **every** forecast endpoint (`/forecast`, `/forecast/inventory`, `/forecast/suppliers`, `/forecast/export`) routes through; the legacy `ForecastService.generate_forecast()` orchestration was **removed**. The CSV-export-zeros-while-dashboard-correct divergence can no longer occur by this mechanism.
...
| `serp_local_prod`    | MySQL      | Local Docker (serp-mysql, `127.0.0.1:3307`, devuser) — local SERP prod schema                                                                                                                                                                                                                                                                                                                                                                                                                              |
| `serp_local_staging` | MySQL      | Local Docker (serp-mysql, `127.0.0.1:3307`, devuser) — local SERP staging schema                                                                                                                                                                                                                                                                                                                                                                                                                           |
| `laravel_local`      | MySQL      | Local Docker (serp-mysql, `127.0.0.1:3307`, devuser) — the 13 Laravel catalog tables, schema-only; local target for the mixed Laravel pool                                                                                                                                                                                                                                                                                                                                                                 |
| `serp_test`          | MySQL      | **STAGING SERP DB** on Hetzner `5.161.233.240:3306`; holds the populated `serp_*` tables + darklaunch history (old `live_darklaunch_db` key was removed — same DB)                                                                                                                                                                                                                                                                                                                                         |
| `serp_app`           | MySQL      | **LIVE/production SERP DB** — same Hetzner host as `serp_test` (`LIVE_DARKLAUNCH_DB_*`), database `serp_app`. Now **holds the `serp_*` tables** (103 of them; migrations have landed — the earlier "does not hold them yet" is stale). Both DBs sit at the **same `_migrations` head**, timestamp-named (`20260910_161500_widen_stock_move_line_state_enum`, applied 2026-09-10) — ⚠️ the scheme is `YYYYMMDD_HHMMSS_<desc>`, **not** the old numeric `NNN`, so "078" no longer exists as a version string |
...
- **Darklaunch vs replica:** darklaunch DBs historically had `_migrations` + `serp_darklaunch_meta`, and absence of `serp_darklaunch_meta` marked a replica. ⚠️ **`serp_darklaunch_meta` was DROPPED 2026-08-03 (commit `809a4d8f9`) — that fingerprint no longer works.** A DB is now identified as darklaunch by the presence of worker-written rows / the per-order SERP flags, not by this table. ⚠️ **Corrected 2026-09-11 — use `odoo_id IS NULL`, NOT `id != odoo_id`.** The linked-but-renumbered bucket has all but vanished: **138 rows in 2.28M** on `serp_app.serp_sale_order`, **1** on `serp_stock_picking`, and **0** on `serp_test` — a fingerprint that fires on 0.006% of rows, and on zero rows of the staging DB it is meant to identify, does not work. Live signal: `odoo_id IS NULL` (733 on `serp_app`, 3,692 on `serp_test`).
- `serp_test` (live) is **AHEAD of the LOCAL Docker SERP DB** — treat it as canonical **over the local copy**, which is a lagging snapshot. ⚠️ **Corrected 2026-09-11 — do not read this as "`serp_test` is the canonical SERP DB."** Against **prod `serp_app`** it is the sparse one: `serp_app` now carries full Odoo history (**~2.28M `serp_sale_order`, ~16.3M `serp_stock_move`, ~14.8M SVL rows**) vs `serp_test`'s staging slice (**~23.5k SOs**). The old "~35,983 vs ~34,289 moves" figures are three orders of magnitude out of date. Use `serp_test` to reproduce **staging/worker** behavior; use `serp_app` for anything **volume-, drift-, or performance-realistic**. (⚠️ The local DB is no longer named `serp_prod_darklaunch` — SERP PR #46 consolidated local to `serp_local_prod`/`serp_local_staging`/`laravel_local`.) Rebuild the LOCAL copy via `npm run db:pull:prod-replica` (local-build-only); the LIVE `serp_test` reseed is **`npm run db:full-reseed`** (`/reseed-live-darklaunch` — streams full Odoo-prod history straight to Hetzner, never lands on local disk, pauses only `ORDER_QUEUE_WORKER_ENABLED` while the dual-write mirror stays ON; latest full-history reseed, ~102M rows, completed **2026-07-05**). **The old `db:push:prod-darklaunch` build-local-then-restore flow was REMOVED 2026-07-02** — don't cite it (nor its `pm2 stop serp-workers` advice; workers run on K3s). Local Docker darklaunch: `127.0.0.1:3307`, user `devuser`.
- ⚠️ **Hetzner DB box disk vs binlogs (2026-07-02):** the box (`5.161.233.240`) filled its disk and broke prod's forecast API (MySQL errno 28 on temp-table spills) — binary logging with 30-day retention was journaling every reseed's ~35–40GB of `serp_test` rewrites, for a DB whose recovery story is "run the reseed again" (no replica, no PITR). Since PR #353, every session from `seeding/scripts/_db_helpers.py::get_live_darklaunch_connection` (reseed/compare/backfill) runs `SET SESSION sql_log_bin=0` — session-scoped, so the worker/app pools keep journaling normally. Requires the `SESSION_VARIABLES_ADMIN` grant (on `jack-serptest-write-20260428`, added 2026-07-02); without it the connection still succeeds with a loud WARN and journaling intact. Emergency relief = Munyr running `PURGE BINARY LOGS` (79%→58% disk).
- ⚠️ **ALWAYS use the `serp_test` MCP key when reproducing/diagnosing what the live darklaunch worker or drift monitor actually sees — NEVER the local Docker SERP DB.** (⚠️ The local DB is now `serp_local_prod`/`serp_local_staging`/`laravel_local` after SERP PR #46 — the old `serp_*_darklaunch`/`serp_*_replica` local names are retired.) The local SERP DB is reseeded from scratch and contains **ZERO worker-created rows** (`id != odoo_id`); only seed rows where `id == odoo_id`. Both `darklaunch_mysql_pool` (app/worker, `DARKLAUNCH_DB_ENV=local-prod-darklaunch`) and `mcp__serp-orm` default to it. So reproducing a drift-monitor or worker FK comparison against local silently returns **0 of the exact rows the bug is about** — e.g. worker `stock_move` rows carrying a `sale_line_id`: **955 on live `serp_test` vs 0 on local**. The drift monitor compares live `serp_test`; reproduce against the same DB or the local-vs-Odoo id-space overlap masks the bug.
- **`serp_test` vs `serp_app` (same Hetzner host, different DBs):** `serp_test` = **staging** SERP (database `serp_test`) — holds the populated `serp_*` tables and has historically served as the darklaunch shadow/validation mirror; `serp_app` = **live/production** SERP (database `serp_app`), the prod target, and it **now holds the `serp_*` tables** (migrations have landed; the earlier "does not hold them yet" is stale). Both DBs are at the **same `_migrations` head** — timestamp-named `20260910_161500_widen_stock_move_line_state_enum`, applied 2026-09-10 (⚠️ the old numeric "078 as of 2026-08-10" citation is stale; the scheme is now `YYYYMMDD_HHMMSS_<desc>`). ⚠️ **Same head ≠ same data:** `serp_app` carries full history (~2.28M SOs, ~16.3M moves) while `serp_test` is a staging slice (~23.5k SOs), and `serp_mrp_bom` is **no longer identical** on both (438 phantom rows on `serp_app`/`laravel_live` vs 228 on `serp_test`). Both share the `LIVE_DARKLAUNCH_DB_*` connection params (host/port/user/password) — only the database name differs. Do **not** treat `serp_test` as production (**prod is `serp_app`**). **Both keys are wired in `src/services/databases.ts` + the db MCP enum, but the running MCP must be restarted before a newly-added key is queryable.**
...
- Durable origin test: **`odoo_id IS NULL` = SERP-origin** (increasingly common for new receiver-orders). NOT the `id >= 1_000_000_000` scheme — `SERP_ORIGIN_ID_FLOOR = 1_000_000_000` was **reversed the next day**; code assuming a 1B floor is stale (verified 2026-09-11: **0** rows ≥1B). ⚠️ **But do NOT substitute "small ids just above `MAX(odoo_id)`" either** (corrected 2026-09-11) — SERP-origin ids are allocated at a clean **+10,000,000 offset** above the Odoo range: `serp_sale_order` max `odoo_id` 2,418,146 → first SERP-origin id **12,418,257**; `serp_stock_picking` 3,294,515 → **13,293,508**. An `id < 10_000_000` or "just above max(odoo_id)" filter returns wrong rows. **Always test `odoo_id IS NULL`, never an id threshold.**
...
**`mrp_bom`** (~2,194) — `type`: `normal`=real manufacturing BOM; `phantom`=KIT (explodes at SO confirmation, NO MO). **All 813 phantom BOMs are `active=false`.** ~96-98% have `product_id` NULL (linked via `product_tmpl_id` only) — matching on `product_id` alone causes "no BOM found" (⚠️ a caution for **new** queries only: SERP's own `_bom_find` already falls back to `product_tmpl_id`, `mrp_bom.py:170-219`).
...
### The replica/darklaunch matrix — NEVER conflate replica with darklaunch

> ⚠️ **The four DB NAMES below are the RETIRED local Docker layout** (`serp_prod_replica`/`serp_staging_replica`/`serp_prod_darklaunch`/`serp_staging_darklaunch`). **SERP PR #46 consolidated the local Docker DBs 5 → 3** — the current local DBs on `serp-mysql` (`127.0.0.1:3307`, `devuser`) are **`serp_local_prod`**, **`serp_local_staging`**, and **`laravel_local`** (the 13 Laravel catalog tables). The live darklaunch history lives in Hetzner **`serp_test`** (staging) and production is **`serp_app`** — see "The 14-Database Landscape". The replica-vs-darklaunch _concept_ (clean Laravel mirror vs Odoo-overlaid) is still worth understanding, but these are no longer live local DB names.

| DB (RETIRED local name)   | Seed source                | Odoo overlay? | Writer            | Contents (historical)                                                        |
| ------------------------- | -------------------------- | ------------- | ----------------- | ---------------------------------------------------------------------------- |
...
- **`serp_product_template`** (~7,880 on `serp_app`, ~7,372 on `serp_test` — the old "~3,776" is stale as of 2026-09-11) — **NO `default_code` column here.**
...
- **`serp_mrp_bom`** — `type` ENUM(`normal`,`phantom`) (no third value). Phantom-only with `odoo_id` NULL and no normal BOMs/MOs (those live in darklaunch) — ⚠️ **but NOT a frozen April-2026 snapshot** (corrected 2026-09-11): in `laravel_live` (the kit/BOM SoT) it is **actively maintained — 438 phantom rows, 252 active, last written 2026-09-09**. `serp_app` matches at 438; only the stale `serp_test` staging copy still shows the old ~228, so a count near 228 means you queried the wrong DB. Excluded from comparisons (`KNOWN_FILTERED_TABLES` filters `mrp_bom` to `active=true AND type!='phantom'`).
...
- `serp_product_supplierinfo` — dual local FKs + parallel Odoo FKs (`odoo_partner_id`, `odoo_product_id`); `delay`=lead-time days. `components.inventory_source` enum `'odoo'`/`'serp'` = per-component darklaunch switch (**0 of 2,560** are `'serp'` in live `laravel_live` as of 2026-08-10 — the 2026-07-17 pilot of 3 rows (ids 2578/2579/2580, MiiR white drinkware) was reverted to `'odoo'`; the switch does NOT propagate to `component_orders`, which is 100% `'odoo'` — see the `component_orders` entry). `components.odoo_id` is **varchar** and synthetic.

### Declared Data Assumptions registry — SERP's guard against SILENT data breaks

**`backend/data_assumptions/registry.yaml`** (checker: `backend/workers/data_assumptions/`) is a versioned list of invariants SERP's code RELIES ON — an id is never NULL, a join always resolves, a sort key is always set, a state list is exhaustive. A cron runs each entry's `check_sql` on a schedule, compares to `expect`, and posts any BROKEN assumption to **#jack-test**. **If you are planning or reviewing a SERP change, check whether it needs an entry** — this is a real, enforced gate (`creating-pr` step 4c blocks the PR; `pre-pr-review`/`review-pr`/`code-review` flag a missing entry as a CONCERN).

- **The trigger is the FAILURE MODE, not the file extension.** An entry is needed when a change makes correctness newly depend on a data shape that would fail **SILENTLY** — wrong numbers, wrong order, wrong grouping, rows quietly missing — rather than **LOUDLY** (an exception). A break that RAISES needs no entry: the exception is already the alarm. Silent breaks are what nothing else catches — no test, no typecheck, no error in the logs.
- **NOT `.sql`-only — this is the misconception the registry's own history proves.** It was created 2026-07-08 and took **exactly ONE** assumption from a feature PR in its first ~9 days, because `source_query` had to resolve under `backend/sql/` AND every prompt keyed on "does the diff touch `backend/sql/*.sql`". Most SERP data access is ORM/RPC or Python-built SQL, so the registry was structurally capped and the gate passed silently on the majority of PRs. **Fixed 2026-07-17 (SERP PR #531):** `source_query` now ALSO resolves **repo-root-relative** (`backend/serp_orm/models/sale_order.py`, `backend/services/order_management/sql.py`), and all four build-path prompts key on the silent-failure test instead. PR #534 extended the same test to the three review gates.
- **The tell to look for:** a new `ORDER BY` / `GROUP BY` / `CASE` / `JOIN` / filter over a column the code now trusts.
- **Canonical case — WW-1960** (`39c7b6bd`): edited `backend/services/order_management/sql.py` (**Python — no `.sql` anywhere**) to sort orders by gift tier via `size_names.sort_key`. A NULL/0 there doesn't raise; it silently mis-orders the Order Management grid AND the printed slips PDF, so the wrong gift goes in the wrong box. Entry: `size_names_gift_tiers_have_positive_sort_key` (db `laravel_live`).
- **Entry shape:** `name` (unique), `db` (exactly ONE of `odoo`/`odoo_staging`/`laravel_live`/`wishdesk`/`retool`/`live_darklaunch_db`/`serp_app`/`routed` — an assumption never spans DBs), `check_sql` (or `check_sql_file`) returning ONE numeric column aliased **`n`**, `expect` (an int, or `{op, value}`), `source_query`, `description`. Validate with `python scripts/validate_registry.py` (fast, no DB); `tests/unit/test_data_assumptions_registry.py` runs it in `npm run test`, so a dangling path fails CI.
- **Dedupe before adding:** two entries are duplicates if they'd catch the SAME break, even worded differently or on a different `source_query` — **extend** the existing entry, never clone it (a near-copy double-alerts and drifts out of sync when only one is fixed).
- **`check_sql` honors the heavy-query rule:** POSITIVE state list `state IN ('draft','confirmed','waiting','partially_available','assigned')`, never a negated `NOT IN`; add an indexed window + LIMIT. The **5-state list is correct on the SERP MySQL mirror too** — `serp_stock_move.state` includes `partially_available` (verified live on `serp_test` 2026-09-11). ⚠️ **Corrected 2026-09-11:** this previously said to use a **4-state** list on the mirror because "the seeder collapses `partially_available` to `assigned`" — that seeder override has been removed. (`serp_stock_picking.state` is a genuine 6-state enum with no `partially_available` — that is real Odoo parity, not a gap; don't "fix" it.) `text()`/asyncmy landmines apply: `%`→`%%`, no `:word`, no reserved-word aliases.
...
| `receiver_products` | Individual candy/snack **flavors the recipient picks** | ~5,000+; `product_id`/`sugarwish_id` = join to Odoo `product_template.sugarwish_id`; `inventory_qty`/`odoo_inventory` live here — ⚠️ **both retire at cutover**: sellable availability moves to the `laravel_live` VIEW **`receiver_product_inventory.sellable_qty`** (backed by `serp_stock_quant`); `inventory_qty` **freezes** at cutover (writes stop, stale values remain, column dropped later) and all 5 Laravel writers deprecate; `odoo_inventory` ("West Coast Qty") is deprecated with **no replacement feed**. ⚠️ **This instruction was INVERTED — corrected 2026-09-11.** Read the CURRENT view definition from **`laravel_live`**, which now holds the **v2** view (9 columns incl. `sellable_qty`, verified live — no `qty_available`). It does **NOT** exist in `serp_test` or `serp_app` at all (0 rows in `information_schema`), so the old "read it from `serp_test`, not `laravel_live`" sends you to a DB that has no such view. (Related: `laravel_live.component_inventory` is a **BASE TABLE** — `id, component_id, location_id, inventory_qty, created_at, updated_at` — not the matching components _view_, which is not deployed anywhere found.) the current DDL is also in the repo as `migrations/20260721_inventory_views_receiver_and_component_base.sql` (hand-run at cutover only — NOT auto-deployed). **View v2 (2026-07-21, PR #588) is rebased onto a `receiver_products`/components BASE (not quant-driven):** every product gets a row even with zero `serp_stock_quant` internal rows, fixing prelisted/Laravel-only SKUs that read 0 in Parallel Test; a matching components view has the same columns, with **available = sellable for components** (components have no prelists yet — Jack's call). **`product_id` = SugarWish id, NOT Odoo product_id**. **Non-`id` PK = `product_id`** |
...
Not a support-only DB. Full CS+CRM+design+billing platform (internally "WishWorks"). **197** tables (2026-09-11; was 179), primarily a **sales CRM + Gmail-AI assistant**. WishDesk is a **downstream replica** — customer/company/order truth lives in SugarWish (Laravel/Odoo); WishDesk stores ids + cached snapshots.

| Prefix                                    | Subsystem                                                                                                                        |
| ----------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------- |
| `swcrm_*`                                 | Full sales CRM (modeled on Insightly)                                                                                            |
| `swcrm_z_gmail_*`                         | Gmail mirror + AI-draft (SWIM) email assistant (20+ tables, **~525k** messages as of 2026-09-11 — the old "~155k" is 3.4x stale) |
...
- ⚠️ **Corrected 2026-09-11 — this was exactly backwards.** `swcrm_activity` (**2,693 rows**) and `swcrm_campaign` (**63**) are now POPULATED, not empty; `swcrm_field_change_log` + `swcrm_opportunity_stage_history` remain the history tables. Meanwhile **every `dev_*` table is now EMPTY** (`dev_leads`/`dev_opportunities`/`dev_actions`/`dev_activity`/`dev_links` all 0 rows) — the parallel prod CRM pipeline they held is gone. They are still **NOT** dev copies of prod data.
...
- **`orders_ticket_attachments`** — ticket file attachments (join on `ticket_id` VARCHAR; `event_id` links to the event, `stored_name` = disk/S3 filename). **Attachments moved to S3 (WW-2187, released 2026-08-05):** the table gained `s3_key`(varchar 700)/`s3_bucket`(varchar 255) columns; new uploads + the backfill write to the **`sugarwish-chat`** bucket under key prefix **`order-ticket-files/`** (bucket resolved at runtime from `AWS_BUCKET_NAME` on the desk box, NOT the design/SERP buckets). A row with `s3_key` set lives in S3; `s3_key IS NULL` = still local-disk-only. As of 2026-08-10 the backfill is done: **1,836 / 1,887 rows have an `s3_key`; the 51 residual are exactly ids 1–51** (oldest Feb-2026 history whose bytes are gone from every instance — permanently 404, NOT a backfill failure or an "on another instance" split). WishDesk prod runs the desk app on `/opt/desk.sugarwish.com` (the live EC2 desk box, app root holds `uploads/order-ticket-files/`), which held 1,826 of the files — the backfill must run **on the box that holds the local files** (`npx tsx server/scripts/backfill-ticket-attachments-to-s3.ts`; completion = `missing on disk: 0`). A legacy local-disk read fallback still lives in `server/orders-tickets/attachment-storage.ts` (`readAttachment()` reads disk when `s3_key IS NULL`); **WW-2396 (open, related_to WW-2187)** tracks removing it via a SWAC PR.
- **`sw_billing_tickets`** — billing/finance queue (Slack-driven); has `slack_*` columns tying each to its thread. **`proposals`** (**~169k** as of 2026-09-11; was ~99.5k): config is JSON (`details_json`, `recipient_json`, `metadata`); `parent_proposal_id` = revision chain; locked version mirrors to `branding_records` (keyed `proposal_id`).
...
- **`orders`** — mirror of SugarWish/Magento orders. `increment_id = '300' + id`. `path_status` = CS/routing classification (NOT fulfillment): includes `Test Account` (**~107k rows, about a third of the table** as of 2026-09-11 — **filter out**), size buckets.
- 🛑 **The Retool forecast/supplier config layer MIGRATED OFF Retool PG onto `serp_app` MySQL** (verified 2026-09-11 — the `sku_supplier` note below flagged this for one table; the sweep was far wider). These now exist in Retool **only as `_backup_*` snapshots**, so a query against the bare name ERRORS: `sku_projections`, `supplier`, `rm_sku_supplier`, `sku_supplier`, `wine_category_goals`, `sku_costs`, `size_groups`, `supplier_buffer`, `sku_equivalency_ratios`, `mix_predictions`, `projected_orders`, `redemption_distribution`, `season_week_ranges`, `week_settings`, `product_type_min_levels`. Read them from `serp_app` instead.
- **`sku_projections`** (now on `serp_app`) — per-SKU popularity at 3wk/8wk/1yr from **recipient gift-selection survey data**. No active/archive flag — join Odoo `product_product.active` to exclude discontinued.
- **`sku_supplier`** — per-SKU forecast config. `lookback_period` ENUM per-SKU: `8 Week` (default), `3 Week` (fast-moving), `Year` (seasonal). **No `case_qty` column here.** ⚠️ **MIGRATED OFF Retool PG onto `serp_app` MySQL** (config-table cutover, verified 2026-07-20) — read via `execute_serp_app_query` in `backend/crud/crud_forecast_suppliers.py` `get_sa_supplier_metadata()`, NOT Retool; the `supplier` column was dropped in the move. Same for `wine_category_goals` (`backend/crud/crud_wine_goals.py`).
- **`supplier`** (now on `serp_app`; Retool has only `_backup_supplier`) — vendor replenishment config; **`default_case_qty` lives HERE**. Real suppliers: Jack's, Albanese, Blair Candy, Redstone, CJ Dannemiller, Nuts.com, Jerry's, Buckin' Nuts, Sew Many Tails, AG Alchemy, The Pound Bakery.
- **`rm_sku_supplier`** (now on `serp_app`; Retool has only `_backup_rm_sku_supplier`) — RM SKU → supplier for automated RM POs; own `case_qty`. **`rm_weekly_demand_cache`** = authoritative RM purchasing forecast (PG, not SERP MySQL); column **`product_type_keys`** is PLURAL (ARRAY) — no singular `product_type_key`. **`bom_components_cache`** = cached BOM (RM→SA) mirrored from Odoo `mrp.bom` — use instead of querying Odoo live.
...
- **`serp_inventory_counts_live`** — `fulfillment_entry_index` groups independent count sessions. **VERIFIED = 2+ different users, same sku+location+entry_index, matching qty.** Uncounted SKUs get **ZEROED on sync** by design. `location_id` 2001/2002 were TEST ids — **0 such rows remain** as of 2026-09-11. **`serp_beginning_inventory_snapshots`** = pre-computed beginning-of-period on-hand (avoids scanning 8.8M-row `stock_move_line`).
- **`buyer_products` is dual-homed** (Retool copy used by forecast SQL + separate `laravel_live` table). ⚠️ **The Retool copy is badly out of sync — 511 rows vs 1,216 in `laravel_live` (2026-09-11), under half the catalog** — so forecast SQL reading Retool silently misses most buyer products. BI mirrors (NOT transactional sources): `quickbooks_dashboard`, `stripe_dashboard`, `shopifymonthlydata`, `insightly_contact_data`.
...
| `receiver_products`; `component_kits` ⚠️ **frozen** | Laravel-owned | SERP → `manage` MySQL via `laravel_*` ops — ⚠️ **no longer covers `component_kits`** (2026-09-11): `laravel_update_kit` was deleted 2026-09-09, so those rows are read-only inputs |

### Cutover Strategy

- **DUAL-WRITE + PARALLEL TEST is the PRE-cutover posture only** — SERP and Odoo run side-by-side; Serpy writes to BOTH; Odoo → SERP one-way sync keeps SERP populated. Gate: **<1% drift, stable 2 weeks.** The `~2026-06-04` date is the **darklaunch** cutover (per-env, authoritative = `serp_darklaunch_meta.darklaunch_cutover_at`) — **NOT** the Odoo→SERP business cutover, which targets **~2026-09-26** (after the Sept 15 release), **executed via a single cut-over flag (per the 2026-07-28 "SERP Final Phase Discussion"). Also open from that meeting: whether to SPLIT the SERP database off `laravel_live` into a separate DB for microservice scalability — deferred pending Jack presenting architecture scenarios, with weekly alignment meetings established; full historical Odoo data seeding remains mandatory for COGS accuracy.**
- ⚠️ **There is NO post-cutover SERP→Odoo mirror.** At flip day the seed copies Odoo's history **one-way into SERP's OWN databases** (`serp_app`/`serp_test` on the Hetzner serp-db box — **NOT** into `laravel_live`; the cutover is a config flip with **no bulk data movement in the flip window**, per the 2026-07-30 plan revision); after that **nothing writes to Odoo ever again** — it freezes read-only for reference, and decommission is just turning it off (before the ~Oct 2026 v15→v17 forced upgrade). The XML-RPC write service, the `odoo_sync_queue` worker, and the drift monitor **die AT cutover**, not during a wind-down; new records carry **`odoo_id = NULL`**. **⚠️ Cutover mechanics — LANDED IN-REPO 2026-07-30 as `docs/plans/serp-odoo-cutover.md` (SERP PR #705), superseding the earlier "bulk-copy into `laravel_live`" plan: SERP's tables do NOT get bulk-copied into `laravel_live` — they STAY where they already are (a config flip, no ~84.5 GB / ~3 hr data movement), with one database per environment as the target (production on `serp_app`, staging on `serp_test`; migration `069` has started this — see the migration-track bullet in the TL;DR for what's actually true today). Because Laravel then CANNOT join across the DB boundary to `serp_sale_order` / `serp_stock_quant`, the integration surface flips from "join directly" to "SERP publishes into Laravel": SERP writes an aggregated stock table into `laravel_live` that the inventory views read (instead of joining `serp_stock_quant`), and the two `serp_sync` columns are PUBLISHED into Laravel rather than Laravel testing whether a `serp_sale_order` row exists. The `/api/odoo` pull-and-callback endpoints are explicitly retired, replaced by the darklaunch order queue reading Laravel's DB directly.**
- ⚠️ **The publish pattern — the concrete tables Laravel reads instead of joining across the DB boundary (from the 2026-08-03 "SERP Cutover Strategy" + "Usage Inventory" artifacts, the most-current operational plan; the in-repo `serp-odoo-cutover.md` is the engineering half).** SERP owns the computation, **publishes** the answer into `laravel_live`, and Laravel reads it locally — no query ever spans two servers. The three published surfaces:
  - **`serp_published_stock`** (in `laravel_live`) — one row per stocked product, **PK `product_id` = the raw `sugarwish_id`** (NOT `serp_product_product.id` — corrected WW-2529, PR #903 merged 2026-08-20, and live-verified: freshest rows have `product_id == receiver_product_id`). The sugarwish*id is DECODED into **both** bridge FKs (`receiver_product_id` INT for finished goods; for RM a `800`-prefix + len>3 strips to `component_id` BIGINT — exactly one set is filled). Columns `on_hand_qty`/`reserved_qty`/`unreserved_outbound_qty` (all `DECIMAL(16,4)`, **`on_hand_qty` is SIGNED** — negative is real) + `computed_at`. **No FKs** (referents live in another DB). Replaces the view legs that used to join `serp_stock_quant`/`serp_stock_move` across the boundary. **TWO writers converge on the same `product_id`=sugarwish_id key:** (1) the interim Laravel publisher `OdooInventoryService::publishStock` (WW-2496), and (2) the SERP cron **`reconcile-published-stock`, which runs every 10 MINUTES, not hourly** (WW-2529, PR #903; interval changed by migration `20260908_161649_reconcile_published_stock_10min_interval.sql` — verified live 2026-09-11: `active=1, interval_minutes=10`) that computes the table **entirely from Odoo** via `odoo_routed_pool` — env-routed: **Odoo staging on serp-test/dev, live Odoo in prod** — grouped by `product_template.sugarwish_id`, clear+rebuild in one transaction (skip-on-empty, zeros included, sign preserved); the internal-location filter (`stock_quant` over `usage='internal'`, CASE-guarded) lives in that cron. (Note: PR #899's earlier serp*\*-MySQL-sourced version was WRONG — `manage` has `serp_product_product` but zero `serp_stock_quant`, so every `on_hand` published 0; #903 superseded it. Live row count fluctuates with the hourly rebuild — treat the exact count as volatile.)
  - **`serp_published_kits`** — keyed on the **triple** `buyer_product_id` + `recipe_key` + `component_id` (⚠️ corrected 2026-09-11: `recipe_key` was added to the PK by migration `2026_08_13_093633`; the old two-column key and the "~1,039 rows" count are stale). Recomputed hourly, so treat any row count as volatile — ~624 rows / 174 buyer products at 2026-09-11, of which ~495 are default-box (`recipe_key=''`). Replaces Laravel's four `serp_*` Eloquent models + the cross-schema join inside `SerpBomService`.
  - **`serp_sync` columns** on `ec_order`/`preselect_orders` (each with a `ship_date_serp_synchronized` ship-date twin) — replaces the `serp_sale_order` existence test. **Values: `0` placed · `1` ingested/reserved · `2` archived-SKU parity (mirrors `oddo_synchronized=2`) · `4` cancelled-and-synced · `5` bypass** (`bypass_odoo` product types). ⚠️ **`2` and `4` are REAL and were missing from this list** — live-verified on `laravel_live` 2026-09-09 (orders created since 2026-09-01: `0`=329, `1`=4487, `2`=41, `4`=324, `5`=102). `4` means the order was cancelled and the cancellation is itself considered synced, so it drops out of the `serp_sync=0` detection pool; it arrived with WW-3013's historical backfill (`CASE WHEN oddo_synchronized=5 THEN 5 WHEN oddo_synchronized=2 THEN 2 WHEN oddo_synchronized=1 AND status='canceled' THEN 4 WHEN oddo_synchronized=1 THEN 1 ELSE 0 END`). Treating the flag as only `0/1/5` miscounts cancelled orders as still-pending. ⚠️ **`items` has NEITHER — only `odoo_sync`** (live-verified `laravel_live` + `manage`, 2026-08-19); item-level sync reads its PARENT order's flag, not a per-item column. `items.serp_sync` is a still-_planned_ mirror (see the `oddo_synchronized` TL;DR line), not yet built.
  - **The TWO inventory views** are `receiver_product_inventory` (finished goods) **and `component_product_inventory`** (the RM twin — same 9-column contract keyed on `component_id`; **holds & prelist stay literal `0`** so `sellable_qty == available_qty`; demand is real, from `component_orders`). Both live in `laravel_live` (verified 2026-08-03, also in `serp_test`); their bodies are canonical in `EXTRA_VIEWS` in `seeding/scripts/export_manage_schema.py`. ⚠️ **The two environments have DIVERGED — check which one you're reading (verified live 2026-08-14).** `manage` (Laravel staging, the base Manish develops against) filters the demand leg on **`oddo_synchronized = 0`**, while `laravel_live` already filters on **`serp_sync = 0`** — so live is AHEAD on the sync-flag swap even though staging is usually ahead. Conversely the **WW-2496 perf indexes are on `manage` ONLY** — `component_orders (order_id, component_id, quantity)` as `idx_co_order_component_qty` and `preselect_orders (created_at)` as `idx_preselect_created` (Manish shipped them as a migration ~2026-08-14); **neither exists on `laravel_live` yet**, which is the normal staging-first steady state, NOT a failed migration. Both views still use a **30-day** demand window in both environments — a proposed narrowing to 14 days has NOT landed anywhere, so do not describe it as current.
  - ⚠️ **The demand leg tests `ec_order.serp_sync = 0`, NEVER `serp_sale_order`** — a `NOT EXISTS` against that ~17-row stub matches _every_ order, driving `sellable_qty` negative and auto-disabling the whole catalog. This makes `serp_sync` **load-bearing for inventory correctness**, not a reporting flag.
  - ⚠️ **Ordering is load-bearing: publish stock FIRST, then flip `ENVIRONMENT_SET=serp`** — reversed, a crash between the two lets demand count orders whose reservations aren't published yet → reads high → oversell. Publish-then-flag fails safe. Every publish upserts **including zeros** (a skipped drop-to-0 keeps its last non-zero value forever) and **preserves sign**.
  - ⚠️ **`component_orders.order_type` is NOT an order-table selector.** It carries `receiver-order`/`preselect-order`/`merchandise`, but `merchandise` (mug + mug-box, live since Apr 2026) splits across BOTH `ec_order` and `preselect_orders` — so each demand branch joins on the order TABLE and never filters `order_type`; a single-value gate silently drops merchandise demand. Safe because the id spaces are disjoint by construction (`ec_order` 13..19.85M vs `preselect_orders` at a 600,000,001 offset).
  - **Column-deprecation workflow:** no Laravel column is dropped at the flip — it **freezes**, is later **renamed with a `(deleted)` suffix** and left live ~weeks-to-a-month so stray readers error loudly, then hard-dropped in the post-cutover cleanup (already the pattern on `components`). See [[inventory-qty-view-cutover-plan]] and [[taylor-location-retired-at-cutover]].
...
- ⚠️ **Moved + renamed (corrected 2026-09-11):** lives in **MySQL `serp_app`** (env-routed — `serp_app` in prod, `serp_test` on dev), tables **`serpy_sync_queue` / `serpy_sync_circuit_breaker`** — fixed env-agnostic names, environment separated by DB not by suffix (`services/odoo_sync_queue/models.py:17-26`). The old **Retool-PG `odoo_sync_queue_live`/`_dev` tables are FROZEN** (last write 2026-06-26, all 4,395 rows terminal, zero pending); the live queue is at `serp_app.serpy_sync_queue` (verified 2026-09-11: 6,302 `odoo`, 552 `multi`, 465 `laravel`, 421 `serp`). Direction is **SERP → Odoo via XML-RPC**, NOT Odoo → SERP. Worker: asyncio, **15s poll** (`odoo_sync_worker.py:54` — the old "~30s" is stale), **`BATCH_SIZE = 1`** as an unconditional module constant (`:55`), not a prod override of a 50-batch default — "batches 50" appears nowhere. Ordering `priority ASC, created_at ASC`. Statuses (`models.py:29-37`): `pending → processing → synced/partial/failed/cancelled` — ⚠️ **there is no `done` and no `dlq`** (both were stale). ⚠️ **Mirror-failure semantics changed (PR #1186, merged 2026-09-15, per Jack: "darklaunch failures should be warnings not actual failures"):** when a SERPY op's live-Odoo write fully succeeds but ONLY the darklaunch replica **mirror** (the shadow-validation leg) fails, `_with_darklaunch_mirror` now sets a `mirror_failed=True` flag + warning **instead of** `partial` — the row is marked `synced`, the draft completes to `EXECUTED`, and the gap is threaded as a benign FYI (no submitter failure alert). Previously this downgraded a good Odoo write to `partial`, alerted the submitter with "0/1 synced, 1 error", and stranded the draft in `APPROVED`. A handler's OWN internal `partial` (a real op that didn't fully land) still downgrades correctly — only the mirror-swallow path changed.
...
2. **Darklaunch order worker** (`backend/workers/serp_order_queue_worker.py` — live-verified on `dev`, 2026-09-09; the old `serp_order_queue_worker.py` name no longer exists on disk, so stop citing it) — **NO queue**; polls `ec_order WHERE oddo_synchronized=1`; `POLL_INTERVAL=300s`, `BATCH_LIMIT=50`, single-threaded; dual-writes receiver/preselect orders to `serp_prod_darklaunch` only; never pushes back to Odoo. **As of PR #291 (2026-06-25) it queries Odoo ZERO times** — `_resolve_odoo_ids` (id-stamp + the 3 cancel/cold-cancel/ship gates that read live Odoo `sale_order.state`) was removed; the cancel/ship gates now key off a non-Odoo signal / are caught by aggregate drift instead. Consequence: the worker is allowed to run **AHEAD of Odoo** (worker rows are `odoo_id=NULL`, AUTO*INCREMENT `id`). **Detector bug (pre-#291):** `detect_shipped_orders_odoo` had `ORDER BY … DESC LIMIT 50` stranding the oldest tail (observed 883 in-flight) — fix = oldest-first. **As of PR #316 (2026-07-01, deployed to `main`) this worker is gated by `ORDER_QUEUE_WORKER_ENABLED`, NOT `SERP_DARKLAUNCH_ENABLED`** — the two flags were decoupled to fix the reseed drop bug (below). `SERP_DARKLAUNCH_ENABLED` now gates ONLY the dual-write mirror + darklaunch pool init. **Detection now keys off `serp_sync`, not `oddo_synchronized=1`** — after the `serp_sync` migration + the PR #965 unified-detector refactor, one detector pass handles new orders (`serp_sync=0`, not-canceled → `process_new_order`), cold-cancels (`status='canceled' AND serp_sync=0` → `process_cold_cancellation`), warm-cancels, and shipments, claiming `serp_sync=1` as it goes. ⚠️ **Cold-cancel non-termination bug (fixed PR #978, 2026-08-26):** orders cold-cancelled \_before* ever reserving never claimed a flag (`process_cancellation`/`process_cold_cancellation` wrote nothing), so `detect_cold_cancelled_orders` re-surfaced them every cycle, creating ~315 duplicate cancelled pickings per order (~37K junk pickings on `serp_test` for 94 orders) + phantom reservations. Fix = claim `serp_sync=1` on the cold-cancel pass so they leave the `serp_sync=0` pool. **The worker was validated live on serp-test 2026-08-25** (drained real `manage` staging orders into `serp_test`, flipped `ec_order.serp_sync` 0→1) then turned back OFF; prod `serp-workers` still boots with `serp-order-queue` disabled. The enable flag now lives in the Secret/node `.env`, not an inline manifest override — the `SERP/serp-dev/workers-deployment.yaml` inline `ORDER_QUEUE_WORKER_ENABLED` override was removed (PR to `sugarwish-infrastructure`) so it falls through to the Secret. ⚠️ **Its three `serp_app` bookkeeping tables were renamed `darklaunch_*` → `order_queue_*` (PR #1128, merged 2026-09-10):** `order_queue_quarantine` (drives retry behavior), `order_queue_failure` + `order_queue_activity` (the logs behind the Order Queue Stats page; ~49k rows on `serp_app`, ~102k on `serp_test`). These are order-queue worker bookkeeping, NOT part of the darklaunch validation/reconciliation system. The crud module + hook followed: `crud_darklaunch_admin.py` → `crud_order_queue_admin.py`, `useDarklaunchAdmin.ts` → `useOrderQueueAdmin.ts`. **No `serp_` prefix** (deliberate). Index names (`idx_dl_*`) and exported `Darklaunch*` TS type names were intentionally NOT renamed.
...
`button_validate → _action_done` = ~440–650 serial statements per 8–12 move shipment. ⚠️ **Throughput figures corrected 2026-09-11 — the old "~35–72s/shipment, ceiling ≈72 shipments/hr" was a LOCAL BENCH ARTIFACT, wrong by ~40×.** Measured prod is **~1.02s mean / 1.34s p95 per shipment** (`docs/changelog/2026-08-14-ww2472-peak-volume-loadtest.md:250-252`, which states "both halves were wrong"). The serial-by-design rationale below still stands. This is **intentional** — accounting fidelity over speed. **Do NOT batch** — would break `account_move.sequence_number` ordering and `/compare-costing` parity. Fix directions: batch per-move loops into recordset ops, co-locate DB. **Newest-first `LIMIT 50` starves the backlog** — detectors must process **oldest-first** by `MIN(create_date)`.
...
- 🛑 **THE DRIFT MONITOR IS NOT RUNNING (verified 2026-09-11) — the design notes below are accurate, but nothing executes them.** Live `serp_app.serp_cron`: `darklaunch-drift` is **`active=0` in ALL THREE envs** (production/dev/local), production `lastcall` **2026-07-16**; the n8n workflow `HpHN9Reme3L6bNBd` "Darklaunch Compare" is **inactive**. Both paths are dead, so any present-tense "runs hourly / alerts to Slack" reading is false. Treat everything below as **design documentation, not current behavior** — and note that a silent drift monitor is exactly what would hide a live inventory bug. Also ⚠️ `scripts/build_darklaunch_drift_n8n.py` **no longer exists** (deleted 2026-06-11, `63b694219`) — the monitor became an in-repo cron (`backend/workers/crons.py:155`, `workers/darklaunch_drift/worker.py`).
- ⚠️ **STAMPLESS AGGREGATE COMPARE is the design (PR #291, merged + deployed to prod 2026-06-25).** The drift monitor **no longer row-joins on `odoo_id`** — the darklaunch order worker stamps NOTHING (every worker row is `odoo_id=NULL`, AUTO_INCREMENT `id`) and **queries Odoo zero times** (the fragile `_resolve_odoo_ids` call that aborted whole poll cycles is gone). The new comparer (`workers/darklaunch_drift/worker.py`) compares **aggregate signatures** — per-entity `COUNT`, `COUNT GROUP BY state`, `SUM(amount_untaxed)`, quant `SUM(quantity) GROUP BY (SKU, location_id)` — grouped on the **stampless `sw_id` bridge** (`Odoo.sale_order.sw_id = darklaunch.serp_sale_order.sw_id`, 100% populated both sides), windowed on **`sw_datetime`** (the original Laravel timestamp, the only non-diverging time anchor; `date_order`/`date_done` are replay-`now()`-derived). A **15-min settle window** (PR #296) excludes rows younger than 15 min so in-flight orders can't false-flag. Cron `0 6-18 * * 1-5`; Slack to `#jack-test`. **GENERATED** by `scripts/build_darklaunch_drift_n8n.py` — **do NOT hand-edit the JSON**; add suppressions to the Python.
...
- **2026-07-02 drift sweep (PR #361) — fixes + reading newer reports:** (a) **Preselect GMV bug fixed** — `_recompute_sale_order_totals` had excluded packaging/component lines from EVERY preselect (PR #275 over-generalized from an unredeemed exemplar); Odoo's prepick import puts packaging ON the SO for every REDEEMED preselect, so redeemed preselects (the delivery call path) now sum ALL lines, while cold-cancels keep the bare-gift filtered total (this also fixed a $4-vs-$2 over-count on cold-cancelled preselects). Known residual: redeemed-THEN-cancelled preselects under-count by packaging (undecidable from staging; ~$0–4/day, under the $10 threshold). (b) **Orphan quants** — seeded `serp_stock_quant` rows whose Odoo counterpart was later vacuumed were never deleted (upsert-only seed, survives reseeds); the seeder now runs `delete_orphan_quants` (full + recent phases only, never backfill; SKIPs if Odoo returns <1000 ids). (c) **Agreed-cancel picking noise excluded** — the DL worker creates then cancels a delivery picking for orders cancelled before redemption while Odoo creates NO picking, so both-side-cancelled SOs permanently showed DL-only cancel pickings; `_picking_state_counts` now excludes `sp.state='cancel' AND so.state='cancel'` symmetrically (a one-side cancel still flags via the PR #341 classifier). (d) **Paused-worker annotation** — when `ORDER_QUEUE_WORKER_ENABLED=false` (full reseed in flight) the report header warns that artifacts are expected: during a multi-day full reseed EVERY section compares a partial, newest-first-backfilling DL against complete Odoo, so one-directional DL=0 / DL=$0 buckets mean "not yet backfilled", NOT drift. (e) **Known approved-diffs to not chase:** GL `j=2 ±5` = human-posted vendor bills (permanent); the ":information_source: N Odoo-stale (SERP-correct)" cancel lines = Laravel-cancelled orders Odoo never applied (PR #341 classifier) — informational, not drift.
...
| Flag                          | Values                                                                                                                                                                          |
| ----------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `oddo_synchronized`           | `0`=not synced, `1`=pushed, `5`=error, `2`=in-flight/claimed (**1 row live**) — ⚠️ **no `3` in prod** (verified 2026-09-11; the old "`3`=stuck (archived SKUs)" is unsupported) |
...
- **Auto-disable is THREE distinct mechanisms (mapped 2026-07-21) — do not conflate:** (1) **order-time PHP `ReceiverProduct::disableIfCanBe()`** (`app/Models/ReceiverProduct.php:149`) fires synchronously on order placement right after the inventory decrement (receiver/preselect/sweet-shoppe/prepick controllers + `PrepickOrderJob`), disables when `inventory_qty <= drop_level` (own or inventory-link parent's; fallback = global `receiver-inventory-drop-level` 100), cascades parent↔child via `inventory_link`, reads NO prelist/reservations — the most important real-time disabler and the easiest to miss; (2) the **`SynchronizeOdooInventory` Python sidecar** (Odoo→Laravel sync, sharded `product_id % 10`) disables after writing the synced qty when `inventory_qty <= 0 OR inventory_qty < drop_level` (strict `<`, NO cascade — subtly different from PHP's `<=`); (3) the **n8n Disable Unreserved V2** every 1 min querying Odoo directly (the deadlock source; see n8n section). Pre-pick bypasses drop level. "Live choice" rule: `archive != 1` AND `status='enabled'` AND `deleted_at IS NULL` AND within date AND `(odoo_inventory − active reservations) > drop_level`. `drop_level` fallback = `feature_attributes` id=1 (`receiver-inventory-drop-level`, value **100**). Parent SKUs = length 11; children = first 11 chars + suffix. Ops lever: lower `drop_level` (often to 5 or 1), re-enable parent+child.
...
| ------------------------------------------------------- | ------------------------------------------------- | ----------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Laravel `kits`/`component_kits` ⚠️ **FROZEN read-only** | Colored-box packaging                             | B-\*                    | ⚠️ **`laravel_update_kit` DELETED 2026-09-09** (SERP PR #1116) along with the `laravel_kit`/`laravel_kit_composition` sync handlers; `/kits` colored-box table is read-only and rows are frozen (last write 2026-08-20). **All kit editing now goes through `serp_update_kit`.** Still read at order time: `buyer_products.default_kit`→`kits.id`; `components.hide` |
...
- Swap = `remove_component(old)` + `add_component(new)` + `hide=1` old / `hide=0` new — ⚠️ **no longer "across all three"** (corrected 2026-09-11): the Laravel kit arm is retired, so a swap fans out to the remaining systems only and kit edits go through `serp_update_kit`. **Discontinue = ARCHIVE** (`product_product.active=false` / `components.hide=1`), never delete. New Serpy-created `receiver_products` default to `status='disabled'`, `archive=1` — must be manually enabled. Phantom BOMs **cannot** be created via Odoo API (UI only).
- After Apr 2026: Laravel sends items+box, Odoo adds shipper/insert/paper via phantom-BOM explosion; both together define a complete kit. ✅ **The READ path is still live and additive** (verified 2026-09-11) — `ReceiverOrderService.php` still loops `component_kits` into the component map, then **additively** merges SERP components via `SerpBomService::getBomComponents()`, with **`component_kits` taking precedence on conflict**. The Laravel rows are now **immutable inputs, not an editable system**. Note the SERP arm reads the published projection **`serp_published_kits`**, not `serp_mrp_bom` directly (`SerpBomService.php:74`).
...
- **Do NOT fix COGS by adding a BOM or resolving by `product_sku`** — resolve by `product_id` (→ sticker). Popcorn is **vendor-fulfilled** (City Pop / Poppin & Mixin); orders should **NOT** contain merch/sleeves (WW-964). ⚠️ **RETIRED 2026-09-01 (PR #1056, `4dd56cd1f`, on dev+main):** `make_popcorn_boms_phantom()` is DELETED — SA-02 popcorn BOMs now stay `type='normal'` in the seed, matching Odoo (the `feature/popcorn-phantom-explode` `normal→phantom` flip is gone, and the SA-02 carve-outs in `compare_odoo_replica.py` `KNOWN_FILTERED_TABLES` went with it; `ODOO_APPROVED_DIFFERENCES.md` §2.6 marked RETIRED). Open consequence recorded as undecided: popcorn lines now ship as the uncosted flavor SKU instead of exploding into cup+sticker leaves, REOPENING the COGS gap `KNOWN_BUGS.md` once recorded as fixed 2026-06-02. Popcorn forecast splits by vendor **CityPop vs Poppin** (no collapse).
- **Popcorn inventory is Laravel-ONLY (verified 2026-07-22):** Odoo holds **zero internal-location quants** for SA-02 SKUs (only net-zero customer/adjustment bookkeeping pairs — outflow was written off, never received in), so the real on-hand (~250–2,600/SKU) lives solely in `receiver_products.inventory_qty`. Consequence: the quant-backed `receiver_product_inventory` view (and any Odoo/serp_test quant read) shows popcorn as **0/negative** while the live page shows real stock — an open **cutover gap** (fix = book popcorn into an internal location, or special-case the line to the Laravel counter).
...
- **TWO fields, not one: `recipe_snapshot` (singular, legacy) vs `recipe_snapshots[]` (plural array) — at `branding_records.merchandise → $.items[N].recipe_snapshots`.** The scalar is ONE recipe frozen to whichever product was `product_selections[0]` at creation (arbitrary); the array carries one entry per offered option so redemption can pick the right one. By convention the scalar is kept equal to `recipe_snapshots[0]` for back-compat. **The array has TWO producers and ZERO consumers inside SWAC** — every SWAC reader (cart-check, mug-image-processor, previews, the `detect-merch-size-recipe-mismatch.sql` detection query) reads the scalar only. The **sole consumer anywhere** is Laravel `RecipeMerchComponentOrderService::resolveRecipeSnapshot()` at redemption: (1) array present → match entry on `buyer_product_id` → correct; (2) array present but no match → fall back to `largestBoxSnapshot()`; (3) **array absent → returns the frozen scalar regardless of what the recipient picked → wrong box + wrong inventory deducted** (that is WW-1991; Subash's fix adds `resolveFromScalarSnapshot()` to re-derive the key from the actual pick).
- ⚠️ **NEVER zip `recipe_snapshots[]` against `product_selections[]` — index _i_ is NOT index _i_.** SWAC silently drops selections whose recipe doesn't resolve (`console.warn` only), so the array can be _shorter_ than `product_selections` with no error. Always match on `buyer_product_id`, which is what Laravel does. Also: the PATCH-time rebuild uses raw `productSelections` while the builder uses de-duped `cleanedSelections`, so a duplicated bp yields duplicate entries from one producer and not the other.
- **Array presence tracks WHICH CODE PATH created the record, not how many products were offered** — the 2026-06-03 change was a code cutover, never a data cutover. Timeline: **2026-04-11** merch system starts, singular only (Apr–May = 100% scalar-only, correct for single-select); **2026-05-06** multi-select arrives → latent bug born (many products, still one frozen recipe); **2026-06-03** Jason's `94afd9c46` adds `recipe_snapshots[]` to the merch-first builder + `23e9a99b8` adds the PATCH-time rebuild; **2026-06-04** Laravel WW-1066 ships the array-matching consumer; **2026-06-26** Parish's WW-1509 size guard lands _inside_ the `!snapshotMatches` branch and inherits the same blind spot. The **other two writers were never fixed** — the merch-quiz path and the proposal-op widget (`ProposalBuyerMerchandiseSection.tsx` `saveMerchandise` ~`:316` fetches recipes for `buyerProductIds[0]` only and writes the scalar), and the widget additionally _destroys_ an existing array on edit. Result Jun–Jul: only 130 of 181 records carry the array (**June 61%, July 88%** — never 100%; 38–41 bad records, newest 6 days old as of 2026-07-28). **`recipe_snapshots` is untyped on the SWAC client** (`proposal.types.ts:198` — part of why `:360` drops it with no compile error). Filed as **WW-2120** (SWAC producer bug, Parish), bidirectionally linked to WW-1991 (Laravel consumer half). ⚠️ **WW-1991's root-cause section is STALE** — it claims "zero references to `recipe_snapshots`" and "nothing re-resolves the recipe"; both are false against `origin/blue`, WW-1066 shipped that in June.
...
- Production slip batch number = `COALESCE(MAX(production_slip_batch)+1, 900000001)`. ⚠️ **`is_printed` now lives on `receiver_production_slips`, NOT `ec_order`** (confirmed 2026-09-11) — the slip UPDATE sets `receiver_production_slips.is_printed=1` (join via `order_id`) + batch + `batch_date`; an `ec_order.is_printed` reference errors on live `laravel_live`. `preselect_orders` has **NO `batch_date`** column. Only `is_pdf_generated=1` rows participate. `sw_fulfill=0` skips label requirement.
...
| --------------------------------------------- | -------------------------------------------------------------------------------------------------- | ----------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 🛑 Daily Operations Message **INACTIVE**      | `dailyOpsMsg01` / `ZH5i32eGyRE6Zb1f`                                                               | —                                               | ⚠️ **Not running** (verified live 2026-09-11: `active=false`, and its trigger node itself carries `"disabled": true` — flipping `active` alone will NOT restart it). Formerly posted "Daily Inventory Status Report" → `#live-product-warnings` `C084Z9EKDSL`. The daily report migrated off n8n to SERPY. Sibling `93EWEx4SHo9fDQA8` "Live Product Warnings" is also `active=false`.                                                                                                                                                 |
| Operations Slack                              | `opsSlackWorkflow01`                                                                               | Every 20 min                                    | Level-change alerts; **writes** `time_turned_*` to `operation_levels`                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
| Disable Unreserved Products                   | `z3gsFEZ9Z0853Gj1` / `VRdmXlm2XTeRbOyT`                                                            | Every 1 min                                     | Auto-disables SKUs with no real availability; **known deadlock source**; also the 330s "oversell" offender. Posts to **`#ops-and-tech`** (`#jack-test` is only its ERROR branch) — verified 2026-09-11                                                                                                                                                                                                                                                                                                                                |
| 🛑 Cost Tracker Weekly Average **INACTIVE**   | `costTrackerWeekly` / `8tETscs44BzY1f3J`                                                           | —                                               | ⚠️ **Wrong on three counts** (verified live 2026-09-11): that id is `active=false`; the live successor is **`ttKcozL8m1zTQvTT` "Update Scorecards"**; its cron is **`0 9 * * 0` = Sunday 9 AM, NOT Monday 8 AM**; and it posts to **`#jack-test`**, not `#ops-and-tech`.                                                                                                                                                                                                                                                              |
| Odoo Order Sync Integrity Monitor             | ⚠️ **`6iiMjS1arjKpoWmz`** (active) — the documented `bR4rEQjFI3GuwkiY` is a **dead id (HTTP 404)** | hourly `:20`                                    | resets sync flags → `#api-autofix` `C088M68FD47` (schedule + channel verified correct 2026-09-11)                                                                                                                                                                                                                                                                                                                                                                                                                                     |
| Sheets Export (forecast → Retool)             | `pLjFlQDc9kEeA8DG`                                                                                 | hourly                                          | → `#jack-test` `C083M27KU8L`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
| 🛑 Add Retool Incremental Tables **INACTIVE** | `givRuaU8E7jawAGD`                                                                                 | —                                               | ⚠️ `active=false` (2026-09-11). Cron `0 0 * * *` was right, but **the workflow has NO Slack node at all** — the "→ `#jack-test`" destination never existed.                                                                                                                                                                                                                                                                                                                                                                           |
| 🛑 Darklaunch Drift Monitor **INACTIVE**      | `HpHN9Reme3L6bNBd` ("Darklaunch Compare", `active=false`)                                          | `0 6-18 * * 1-5` → `#jack-test` (as configured) | ⚠️ **Not running** — see the drift-monitor entry: the in-repo cron that replaced it is `active=0` in every env too. ⚠️ **The paired id `IalsmpKBKbJM4LXg` is NOT part of this**: it is "Update Projections" (also inactive), a SKU-popularity job writing Retool `sku_projections` with **no Slack node and zero relationship to drift** — the pairing was simply wrong.                                                                                                                                                              |
| 🔴 Duplicate Receiver Orders **BROKEN**       | `EfCt429Vle67ckY8`                                                                                 | every 10 min                                    | ⚠️ **Undocumented and failing on EVERY run** (verified 2026-09-11, latest failures minutes apart): `Unknown column 'eo.is_printed'`. **Cause: the column MOVED tables** — print state now lives on **`receiver_production_slips.is_printed`** (`tinyint(1)`, join via `order_id`), not on `ec_order`. The old `ec_order.is_printed` survives only in the archived `sugarwish_live_cme_temp` / `sugarwish_manage_new_prepicks` schemas, which is why the query used to work. **Fix:** repoint the join at `receiver_production_slips`. |
| Preselect Address Auto-Fixer                  | `Zp59HJx19mly3lQ1`                                                                                 | —                                               | zip/state fixes → `#preselect-address-fixes` `C0A1F8FS6R0`                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |

- **Disable Unreserved — 3-factor formula (live-verified):** `final_available = Actually Available − SW Reserved (inventory_reservations.status='active') − Orders Not Imported (items.odoo_sync IN (0,1))`. SKU scope `default_code LIKE 'FG-%'/'SA-%'/'L-B02%'`, `sugarwish_id` 1–800,000. **NOT two separate ids** — `z3gsFEZ9Z0853Gj1` already contains the 3-factor formula (commit `d7bf45f`); the "2-factor older version" narrative is wrong. Entry gate: only products with `unreserved_demand > 0` (open customer moves Odoo hasn't reserved) are evaluated at all — it's an **oversell tripwire, not a general low-stock disabler**; only enabled, non-archived PARENT products (`inventory_link IS NULL`) are judged, children get disabled via cascade. It is **disable-only with NO cooldown** (every 1 min), so a manual re-enable is reversed within ~60s while the math stays below drop. ⚠️ **Prelist blind spot (found 2026-07-06, live-verified; FIXED 2026-07-06):** the workflow _used to_ read `receiver_products.drop_level` + reservations but NOT `prelist_qty` — and since WW-1667 (2026-07-01) prelisting bumps only that Laravel column and touches NO Odoo inventory, prelisted units were invisible to it while orders sold against the prelist double-counted against availability (as `items.odoo_sync IN (0,1)` AND later as Odoo unreserved demand) → prelisted SKUs (e.g. SA-01-099-A, 100 prelisted) got auto-disabled and stayed disabled. **Fixed 2026-07-06** (n8n workflow `VRdmXlm2XTeRbOyT` activated that day): Node 2's SELECT now includes `COALESCE(rp.prelist_qty,0) AS prelist_qty` and Node 3's formula adds `+ prelistQty` — verified live in the workflow JSON 2026-08-10.
...
- **Kill-switch convention for gated disable workflows (WW-2450/WW-2452, set 2026-09-02):** the enable/disable flag lives **IN the workflow JSON as a Set node** (name `0. Flag: Disable Actions Enabled`, boolean field `disable_actions_enabled`, default `false` = notify-only), wired right after the trigger — **NOT an env var** (a redeploy-to-flip is wrong for a cutover kill-switch) and **NOT a DB row**. Every downstream IF gate and Slack expression reads it via `{{ $('0. Flag: Disable Actions Enabled').first().json.disable_actions_enabled }}`. Dry-run (flag OFF) redirects the Slack alert to `#jack-test` standalone (no threading) instead of the live ops channel, and the wording flips "would disable"/"disabled". Workflow naming: `<base name> WW-<ticket#>`. To flip: open the node in the n8n UI, toggle, save — no redeploy.
...
- `blue` = **integration branch**, NOT production (`main` is). `WW-*` tickets are NOT SWAC-exclusive; `SUG-*` is NOT Laravel-only — both span repos. ⚠️ **Corrected 2026-09-11 — both command names here were wrong.** The laravel promotion helper is **`/merge-to-blue`** (`sugarwish-laravel/.claude/commands/merge-to-blue.md`); there is **no `/pr-to-blue`**, and **no `sw-*` command family exists in any repo** (the claimed June-2026 `ww-*`→`sw-*` rename did not leave any such files). The WW-\* **ticket** prefix is unchanged.
...
| `SERP_SHADOW_WRITES_ENABLED`            | Shadow/prod-traffic validation. ⚠️ **This flag no longer exists in the SERP backend** (verified 2026-08-10 — `git grep` returns nothing); the shadow/parallel-test mechanism was renamed toward darklaunch starting 2026-05-08 (commit `eadd890db`). Treat the `serp_shadow` entries below as predecessor history, not a live flag.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
...
| `ORDER_QUEUE_SHIPMENT_LIMIT` / `ORDER_QUEUE_DETECTION_LIMIT` | Order-queue pacing (PR #1142, merged 2026-09-11). `ORDER_QUEUE_SHIPMENT_LIMIT` (default 50) **replaced** `ORDER_QUEUE_BATCH_LIMIT` — the fairness dial capping the slow shipment arm per cycle; `ORDER_QUEUE_DETECTION_LIMIT` (default 5000) was a hardcoded detection cap now made a flag. ⚠️ **`ORDER_QUEUE_BATCH_LIMIT` is GONE** — the stale prod `=300` (set 7/15 for an obsolete meaning) was reset/removed. |
| `ENVIRONMENT_SET`                       | **Boot-fixed data-source flag** `odoo` (default; prod **until cutover**) \| `serp` — ⚠️ **`serp` is NOT dev-only: at the 2026-09-26 cutover PRODUCTION is set to `serp`.** This one flag IS the cutover (see the TL;DR entry: publish stock FIRST, then flip). → `settings.source_is_serp` → `read_repointable` dispatches every paired `_odoo()/_darklaunch()` read; frontend TopBar badge shows "Live Odoo"/"Parallel Test" (`GlobalSourceToggle` is DISPLAY-ONLY — no runtime flip; value reaches the client via `GET /api/client-config`). **As of 2026-07-22 serp mode is WRITABLE**: the 7 cart pages' read-only locks were lifted, ops stamp `_skip_id_resolution=True` and route to `serp:{entity}` handlers on the darklaunch pool — payload ids must already be SERP-space (serp-source reads only), and approved ops sync `serp_test` through the SERP ORM, **never Odoo**. Cutover plan: this is the "one global source flag" that flips every SERP read AND write — **flip target is `2026-09-26`, NOT the older "Fri Aug 28"** (that date predates the in-repo plan; Aug→Sep is the parallel-run window and the flag flips at the Sept 26 freeze — `docs/plans/serp-odoo-cutover.md` §7). **The last correctness blocker on setting `serp` is the authoritative Laravel write pool (plan item A2):** with the staging copier deleted, any write that lands in SERP's own DB now persists silently in the wrong database instead of being erased next copier cycle — the Laravel route must always resolve to `live_mysql_pool` before the flip |
| ~~`PACK_TOMORROW_ENABLED`~~ ⚠️ **GONE** | **Does not exist** (verified 2026-09-11: 0 hits in `backend/config.py`) — replaced by the DB cron row `pack-tomorrow` whose `active` column is the switch (`serp_app/migrations/006_seed_cron_pack_tomorrow.sql:7`: "`active` replaces the old PACK_TOMORROW_ENABLED env flag"; live prod `active=1, at_time 16:00`)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |     |
| ~~`ODOO_ID_STAMP_ENABLED`~~ ⚠️ **GONE** | **Does not exist** (verified 2026-09-11: 0 hits in `backend/config.py`) — consistent with `odoo_id_stamper` being retired 2026-06-25. Formerly: odoo_id stamping (off → no Odoo DB/XML-RPC during processing)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
| `ODOO_SYNC_WORKER_ENABLED`              | Odoo sync worker gate (`config.py:124`, default **False**)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
| `WORKERS_ENABLED`                       | Master worker switch (`config.py:613`, default True); set per-deployment via the `serp-env` secret                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
| `CRON_RUNNER_ENABLED`                   | In-repo cron runner (`config.py:619`, default True) — the thing that would run `darklaunch-drift` / `pack-tomorrow` / `reconcile-published-stock`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
| `EMAIL_ENABLED`                         | Outbound email (`config.py:591`, default True)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
...
- **2** local DBs — `serp_local_staging` + `serp_local_prod` — rebuilt together via `npm run db:seed` (wipes + reseeds; `seeding/scripts/seed.js:123-133`, whose summary prints `N/2`). ⚠️ **Not 4** (corrected 2026-09-11): `laravel_local` is separate (`npm run db:seed:laravel-local`). ⚠️ **`npm run db:pull` no longer exists** (verified 2026-09-11 — no `pull` script in `package.json`); the old "run it when `manage` schema changes" step is stale. Seeder **caps windowed tables** (`account_move`, `stock_move_line`) at latest 5–10k ids — "nothing compared" in drift reports is **by design**. **`APP_ENV` must be `local`** for safe seeding. Seed order: Odoo first → APPEND SERP-native tables (phantom kits). Each table wiped between seeds.
...
- Personal work-intelligence platform. MCP servers (**8**, verified 2026-09-11 from `~/.mcp.json`): db (read-only), slack-search (Qdrant, encrypted), knowledge (semantic search over `DICTIONARY.md` + `knowledge/COLUMN_MANIFEST.md`), jack-slack (Slack post/read), logs, github, **n8n** (live workflows, read-only), **sessions** (the Session Mesh — `mcp__sessions__*`). ⚠️ The last two were missing from this list; `sessions` in particular is policy-bearing (`~/CLAUDE.md` requires overlap checks through it), so an agent reading the old 6-item list would conclude it doesn't exist. `~/.mcp.json` runs via `npx tsx` — **no build step but requires Claude Code restart after editing `.ts`**.
...
- **`/remind` reminders run LOCALLY on Jack's Mac, NOT on Hetzner.** The Hetzner `sw-cortex-slack.service` / `sw-cortex-reminders.timer` systemd units exist but are **not** the maintained path — the live path is two local processes started/repaired each morning by **`/start-day` Step 0 check #9** (`scripts/reminders-up.sh`): `scripts/slack-handler.ts` (Socket Mode button handler for Done/Snooze/Delete) and a 60s loop over `scripts/check-reminders.ts`. Both DM via **Jack Bot** (`A0AV5GDMHHV`) with **`JACK_SLACK_BOT_TOKEN`** (xoxb) + **`REMINDER_APP_TOKEN`** (xapp) — **NOT** SERPY's `SLACK_BOT_TOKEN`/`SLACK_APP_TOKEN` (PR #43 swapped to SERPY by mistake; PR #44 reverted it). Reminder rows live in local SQLite `tasks/tasks.db`. **`pgrep` finding the handler ≠ healthy:** a corrupt/duplicate `.env` key (dotenv takes the LAST duplicate — a bare team id `T…` in a `*_TOKEN` var is the corruption signature) leaves it running-but-dead, so every button click shows ⚠️ and the reminder re-fires forever as `🔁 Re-reminder`. Fix live with `bash scripts/reminders-up.sh` (auth.test-gated, kills+restarts an unhealthy handler). Re-firing rows are `status='sent' AND interacted=0` and do **not** appear in `manage-reminders.ts list` — delete by id.
...
| `#jack-test`                        | `C083M27KU8L` | SERPY/SERP-500 errors + **automated darklaunch reconciliation reports** — NOT a scratch channel. ⚠️ **As of 2026-09-11 prod `SLACK_ERROR_CHANNEL=#serp-alerts`** — worker/order-queue failure + backlog alerts (everything on `SLACK_ERROR_CHANNEL`, shared by all workers) now post to **`#serp-alerts`**, not here. (`#serp-alerts` channel ID not yet resolved — bot may not be a member; look up before citing an ID.) |
...
| Table                            | Purpose                                                                                                                                                                                                                                                                    |
| -------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `wishdesk.orders_tickets`        | CS support tickets **about** orders (**~681 rows as of 2026-09-11**, overwhelmingly `CLOSED` with a couple `IN_PROGRESS`; no `OPEN`/`WAITING_ON_CLIENT` rows currently exist — actively growing, not a static archive; was ~588 on 2026-08-10) — NOT the orders themselves |
...
- Archiving/disabling a SKU on one side without the other breaks SERPY sync. Inventory added to Odoo **before** the SKU exists in Laravel never gets picked up. ⚠️ The old diagnostic "orders stuck at `oddo_synchronized=3` reference archived/disabled SKUs" **cannot be used as written — there are 0 rows with value 3 in prod** (verified 2026-09-11). The underlying failure (a SKU archived on one side) is real; find it by the SKU mismatch, not by `=3`. Common root cause: matching Odoo `product_product.active=false` (archived variant) while the template stays active.
...
| `0`/`NULL` | No SW mapping (**~2,264 of 7,515 templates**, 2026-09-11; was ~1,371) | —                                                  |
...
- **Popcorn phantom explode — RETIRED 2026-09-01 (PR #1056):** the `normal→phantom` SA-02 seed flip was DELETED; popcorn BOMs now stay `normal` (matches Odoo). Popcorn forecast still splits by vendor **CityPop vs Poppin** (no collapse).
...
9. **SERP does NOT auto-deploy from CI.** GitHub Actions CI runs on push but does NOT deploy. Deploy is a manual SSH step on the Hetzner K3s node: `ssh jack@5.161.95.56 'cd /opt/SERP && bash deploy-k8s.sh main'`, and a merge to `main` is not live until that runs. ⚠️ **But as of 2026-08-13 that command does not itself roll the pods** — it pushes images to ECR and bumps tags in the `sugarwish-infrastructure` repo, and the **ArgoCD `serp-prod` Application** (automated sync today) applies them. So "is it live?" = check the ArgoCD sync state / the running pod image tag, not just whether `deploy-k8s.sh` exited 0. (The old AWS `ssh ubuntu@34.203.231.65 ... bash deploy.sh` path is frozen legacy — not the deploy target.)
...
**Dual nature of the schema — but `serp_orm` was SEVERED from the Laravel half on 2026-07-30.** SERP has ~60 `serp_*` tables mirroring Odoo models in MySQL (stock, purchase, MRP, accounting, products). It historically ALSO reached SugarWish-DOMAIN Laravel tables (`buyer_products`, `receiver_products`, `components`, `kits`, `component_kits`, `ec_order`, `preselect_orders`, `items`, `component_orders`, `locations`) through the ORM — **that is no longer true.** SERP PR #706 (`15c5a5eb9`, "sever serp*orm from the Laravel mirror tables") **deleted the `component` and `receiver.product` models** (their entire storage was a Laravel table) and removed Laravel SQL from eight more ORM files; `serp_orm/` now contains **zero** references to `components`, `receiver_products`, `buyer_products`, `component_kits`, `kits`, or `locations`. Identity resolves through bridge FKs on the variant: `serp_product_product.component_id` / `.receiver_product_id` → `default_code` (SKU) + `serp_product_template.name`, and those two columns are now **`BigInteger`, NOT `Many2one`** — opaque external Laravel keys with no model to traverse into (same shape as `buyer_product_id`). Behavior deliberately removed where SERP has no source: kit membership, finished-good `vendor_cost`, the Laravel `location_id` filter, `get_shipping_address`, and `create_sku`'s INSERT into Laravel tables. Jack's framing: "serp won't have tables like components anymore — those will have to be accessed in laravel now," and inserts that still must happen (e.g. create-product-everywhere) go through Laravel **outside** the ORM. **Naming convention:** every table is prefixed `serp*` and named after the corresponding Odoo model with dots→underscores (`stock.picking`→`serp_stock_picking`, `purchase.order`→`serp_purchase_order`, `mrp.production`→`serp_mrp_production`).
...
- **Staging SERP / darklaunch MySQL DB**: Hetzner at **5.161.233.240**, database `serp_test` (MCP key `serp_test`) — this is the **STAGING SERP DB** (with the darklaunch history), NOT a throwaway test DB and NOT production (**prod is `serp_app`**). ⚠️ **TWO different databases are named `serp_test`, on two different Hetzner boxes — only the serp-db one is live.** The **manage** box `5.78.203.128` (`sw-manage-db`, alongside `sugarwish_manage_new_2`) also carries a `serp_test` schema: it is **DEAD and must never be used** (Jack, 2026-08-18: "serp_test on 5.78.203.128 is dead and never should be used"). Verified live 2026-08-18: the `5.78.203.128` copy is a 101-table fossil last written **2026-06-30**, while the real one on **serp-db** (`5.161.233.240`, the hub's `serp_test` MCP key, `@@hostname='serp-db'`) has 133 tables and was written that same day. When Jack says "serp_test" he means the box co-located with `serp_app` — a `serp_test` query that returns plausible-but-stale rows is the tell you reached the dead twin.
...
- **Identity** ⚠️ **MOVED — lives in MySQL `serp_app` as `serpy_sync_queue` (2026-09-11); the Retool-PG copy is frozen since 2026-06-26.** Formerly lived in Retool PostgreSQL: `serp_users` is a stripped bridge table (4 columns: `id, created_at, updated_at, orm_user_id`) hit on every authenticated request. Old columns (name, email, password, roles, is_admin/finance/ops/internal, supplier_name) moved to `_backup_serp_users`.
...
#### SERP Connection Pools (standardized 2026-07-30 — formerly a known mess)

**As of 2026-07-30 (SERP PR #715, `37679e989`) pool routing is STANDARDIZED through one resolver — do not reason from the old ad-hoc per-pool env-var branching.** Every pool now asks `backend/pools/db_targets.py::resolve_target(pool)` for its target; **no pool branches on env vars itself**, and the resolver's `_TABLE` is the pools spreadsheet encoded once. The ~9 ad-hoc URL builders are gone. **Current pool names (the old ones are dead — grep will not find them):** `odoo_routed_pool` and the new `odoo_live_pool`; `serp_routed_pool`, `serp_app_routed_pool` (renamed from `serp_app_pool`), `serp_live_pool`; `laravel_live_pool` (**always live, read-only — intentionally NOT routed**, Jack: "I don't want laravel*live_pool routed") and `laravel_routed_pool` (**all Laravel WRITES**, so a local edit can never hit production). The environment axis is `DETECTED_ENVIRONMENT × ENVIRONMENT_SET × LOCAL_DB_TWIN`: **`ODOO_ENV`, `SERP_ORM_ENV`, `DARKLAUNCH_DB_ENV`, and `LIVE_DB_SOURCE` are now DERIVED read-only properties, not settable env vars** — don't set them in a `.env` and don't tell anyone to. `DEV_DB*_`, `LOCAL_DB_NAME`, `USE_MANAGE_DB`, and the darklaunch pairing validator (it crashed dev+odoo at startup) were deleted; `LOCAL_DB_TWIN`/`LARAVEL_LOCAL_DB_NAME`were added. Derivation reproduces the pre-change prod values, so prod behavior is unchanged. **serp-test runs`ENVIRONMENT_SET=odoo`and must only ever READ the live DB**; auth on serp-test therefore goes through`routed_mysql_pool`→`manage`, not live. Node env-var changes now ship as a committed `env-changes/_.yaml`applied by the merge step (serp-test) and`/deploy`(live) before the Secret rebuild — secrets themselves are never committed. **Forecast serp-leg reads live`serp*app`in EVERY environment (PR #1054,`ac6df255a`, dev+main):** the repointable forecast reads no longer flow through env-routed `serp_routed_pool`(which resolves the local Docker twin on a laptop) — they pin to`serp_live_pool`→ live`serp_app`, so a local `/forecast`mixes live Odoo/Laravel with live serp data and matches prod (a local`npm run db:seed`is therefore invisible to`/forecast`). ⚠️ Note live `serp_app`'s mirrored `serp*\*`tables have been FROZEN since **2026-08-04** — the darklaunch dual-write mirror is gated OFF in prod — so`serp_app` can itself be stale (e.g. PO lines showing "on order" that Odoo already received). A local-read override (`LOCAL_READ_LIVE_SERP_APP`, read-only, writes still hit the local twin) is IN FLIGHT (PR #1079, not yet on dev/main as of 2026-09-04) — do not cite it as current.

**Still true (capacity footguns, unchanged):** SERP runs **6+ independent connection pools** with heavy duplication. Retool PG alone has THREE pools (asyncpg max=5 + psycopg2 sync max=15 + SQLAlchemy asyncpg 5+5). The SERP ORM pool (`serp_orm/db.py`, custom PyMySQL, size=5/overflow=8 = 13 max, `pool_recycle=60s` aggressive) is held idle for the entire 200-320s forecast request, so only ~13 concurrent requests exhaust it. The `ForecastDatabaseConnector` singleton holds 3 pools (Odoo asyncpg, Live SERP SQLAlchemy MySQL via SSH tunnel, Retool asyncpg; min_size=2, max_size=15, timeout=90s). On the now-frozen AWS EC2 (`i-0a0de24d0845c6341`, `34.203.231.65`) this overwhelmed the box under load (504s, resource exhaustion); production now runs on the Hetzner K3s cluster, but the same pool/index footguns still apply.
...
- `res.partner._increase_rank` supplier_rank bumps on PO confirm; PO receipt/vendor-bill recompute fields — **narrowed 2026-09-11: only `qty_received`/`qty_invoiced` are plain fields (`purchase_order_line.py:38-39`); `qty_to_invoice` IS a faithful Odoo-parity compute (`:113-139`, branching on `purchase_method`) and PO-level `invoice_status` is computed off it (`purchase_order_actions.py:1325-1340`, "Bug #12")**; Odoo crons that move stock_move/quant states post-create — **except the prepick step, which IS replicated** (`_action_assign` fills `qty_done = product_uom_qty` on reserved lines; `ODOO_APPROVED_DIFFERENCES.md` §2.7 explicitly reverses the earlier "SERP does not replicate this cron" framing, because the darklaunch worker mirrors **SugarWish prod**, not vanilla Odoo 15).
- No `procurement.group` model (`group_id` is raw BigInteger only), no lot/serial tracking, no package/owner tracking, chatter only at document level (never on stock.move/move_line/quant). ⚠️ **`ir.model.data`/`env.ref()` removed from this list 2026-09-11 — both are implemented** (`serp_orm/models/ir_model_data.py` on table `serp_ir_model_data`; `env.ref()` at `serp_orm/environment.py:116-159` with Odoo-15 `exists()` re-check), shipped 2026-07-24 with XML IDs seeded for seven models. Residual gap: `mail.template` lookup still keys off the `template_ref` Char rather than `env.ref()`. Note `docs/FUTURE_MIGRATIONS.md:20` and `mail_template.py:52` in the SERP repo still claim it is unbuilt — those are stale in-repo.
...
- ~~Bin-level putaway/removal strategy: SERP writes the parent stock location per standard MO flow; Odoo refines move-lines to specific bins. This is approved divergence, not a bug.~~ **RETRACTED 2026-09-11 — do not rely on this.** Both halves are contradicted by live code: SERP does **not** reliably write the parent location (move lines are stamped at the _gathered quant's bin_, `stock_move_actions.py:780-781`), and the behavior is **not** benign — it is the mechanism behind a parent→child self-cancel that inflates inventory. See "`_gather()` DOES expand child locations (SERP)". Whether a _separate_ putaway-strategy gap still exists is unverified; re-derive it before re-adding anything here.
...
5. **SUPPLIER meta** — `supplier`, `supplier_buffer`, `sku_supplier` (⚠️ **MIGRATED to `serp_app` MySQL** — read via `execute_serp_app_query`, no longer Retool; see the config-table cutover note below), `rm_sku_supplier`, `supplier_permissions`.
6. **COST/WINE/BI** — `sku_costs`, `sku_equivalency_ratios`, `wine_category_goals` (⚠️ **MIGRATED to `serp_app` MySQL** — read via `execute_serp_app_query`, no longer Retool; see the config-table cutover note below), plus mostly-stale mock tables (`quickbooks_dashboard`, `stripe_dashboard`, `shopifymonthlydata`, `mock_*`).

**CRITICAL suffix convention** (environment separation via table-name SUFFIX, not separate DBs): draft-ops/inventory-counts/prompt-logs use BASE locally and `_live` in prod; AI message/turn tables use `_dev` locally and `_live` in prod; sync queue uses plain/`_dev` vs `_live`. Auth/user/forecast tables have NO suffix. `_live` tables hold real prod data with far higher row counts. As noted above, `serp_refresh_tokens`/`serp_password_reset_tokens` are documented as Retool but actually live in MySQL. **Since PR #344 (2026-07-02) the live/non-live table pick is centralized in `settings.SERP_APP_TABLES_ARE_LIVE` — true ONLY for `ENVIRONMENT=production`** (the five env-suffixed sites — `ai_table_name`, `_messages_table`/`_turns_table`, `_get_table_names`, `INVENTORY_COUNTS_TABLE` — all route through it); the serp-dev stack (`ENVIRONMENT=dev`) and local both get the non-live variants (`serp_draft_operations`, `serp_ai_messages_dev`, `serp_ai_turns_dev`, `odoo_sync_queue_dev`, `serp_inventory_counts`, …). ⚠️ Raw `ENVIRONMENT`-string checks (`== "local"` / `== "production"`) are the recurring bug class here: `"dev"` matches neither and silently lands in the PROD branch — it sent serp-dev to the `_live` tables and dev workers' XML-RPC to prod Odoo before PRs #352/#354.

- ⚠️ **In-flight migration (2026-06-17): SERP's operational app tables are moving OFF Retool PostgreSQL INTO the new `serp_app` MySQL DB** (Hetzner, `SERP_APP_LIVE_DB_PARAMS['db']='serp_app'`, pool `backend/pools/serp_app_live.py` → `get_sync_serp_app_connection()` / `serp_app_live_pool`; migrations in `serp_app/migrations/*.sql` auto-run on `deploy-k8s.sh`). **Already hard-swapped (no dual-read/fallback): the auth bridge `serp_users` + `serp_user_counting_locations`** — `_resolve_retool_context` (`backend/dependencies.py`, every-request hot path) now reads serp*app, so **serp_app availability gates logins.** Counting tables (`serp_inventory_counts`/`_live`, `serp_vendor_inventory_counts`, `serp_beginning_inventory_snapshots`) + forecast/config tables are being migrated next. **Config-table cutover — already DONE on serp_app MySQL (verified in SERP code 2026-07-20):** `sku_supplier` (SA supplier metadata; `backend/crud/crud_forecast_suppliers.py` `get_sa_supplier_metadata()`) and `wine_category_goals` (`backend/crud/crud_wine_goals.py`) both read/write via `forecast_db_connector.execute_serp_app_query` against `serp_app` — NOT Retool PG anymore (the `sku_supplier.supplier` column was dropped in the move). So the "AUTH / app config" + "FORECASTING caches" + "SUPPLIER meta" + "COST/WINE/BI" entries above are progressively becoming serp_app-resident, not Retool-resident — check the live code before assuming a `serp*\*`app table is still in Retool. **serp_app SQL gotcha:** the live pool is **pymysql, not SQLAlchemy`text()`** — positional `%s`is correct and the`text()`landmines (bare`%`, `:word`) do NOT apply; but psycopg2→pymysql means every migrated query is a hand-translated PG→MySQL rewrite.
...
**1. Odoo Sync Queue** (`odoo_sync_worker.py`; table **`serpy_sync_queue`** in **MySQL `serp_app`**). ⚠️ **MOVED 2026-09-11: the live queue is `serpy_sync_queue` in MySQL `serp_app` (env-routed); the Retool-PG `odoo_sync_queue*` tables are FROZEN since 2026-06-26.** Triggered when a Serpy draft is approved. Executes warehouse ops (pickings, MOs, BOMs, POs, bills) against live Odoo via **XML-RPC** and mirrors to darklaunch. **PO receipts flow through HERE**, not the darklaunch order worker. Worker polls every **15s** (⚠️ not ~30s — `odoo_sync_worker.py:54`), `BATCH_SIZE=1` in prod, orders priority ASC then created_at ASC. Failures retry exponential backoff `5*2^attempts` capped at 300s; `max_attempts=5` → ⚠️ **no DLQ state exists** (verified 2026-09-11). A **circuit breaker** (CLOSED/OPEN/HALF_OPEN, 5-failure threshold, 30s reset) halts processing when Odoo is failing. Statuses: `pending`/`processing`/`synced`/`partial`/`failed`/`cancelled` — ⚠️ **no `dlq`** (verified in `models.py` 2026-09-11). `ODOO_SYNC_DRY_RUN=True` generates fake odoo_ids (`99990000 + entity_id`). Handlers live in `backend/workers/handlers/` (stock.py, purchasing.py, manufacturing.py, serp_orm/, laravel.py).
...
**2. Darklaunch Order Worker** (`serp_order_queue_worker.py`). **NO queue at all.** It dual-writes receiver-order + preselect-order rows into `serp_prod_darklaunch` ONLY (never the main DB), shadow-processing Odoo-sourced orders (`order_type IN ('receiver-order','preselect-order') AND inventory_source='odoo'`), reading read-only against Odoo/live MySQL for seeding, never pushing back to Odoo. ⚠️ **RETIRED 2026-07-30 (SERP PR #707, commit `94deba129`) — the `staging_copier_worker` and `STAGING_COPIER_ENABLED` were DELETED outright; everything in this bullet about the copier is ARCHIVED HISTORY, not live architecture** (Laravel-owned data is now read on a Laravel pool, not a mirrored copy — see the pool-routing bullet and line 13). Historically: as of 2026-06-17 (PRs #181/#182/#183, deployed) it no longer polled live Laravel MySQL directly for new orders — a `staging_copier_worker.py` ("staging copier", `STAGING_COPIER_ENABLED`, 5-min, `RECENT_WINDOW_DAYS=7`) copied recent Odoo-synced ec*order/preselect/items/component_orders rows from live Laravel (read-only, PII-stripped via an allow-list + `PII_COLUMNS` deny-map) into the local `serp_test` legacy tables — and **as of 2026-07-21 (PR #587) also full-mirrors `receiver_products` itself every cycle** (17-column PII-safe allow-list, keyed on its non-`id` PK `product_id`), closing the reseed-lag gap where Live Products Core/Wine status (`status`/`is_core`) only refreshed at a manual full reseed; `detect_new_orders_odoo` (now SYNC, `store_id=2`) reads those local tables and decides "already synced" by `NOT EXISTS serp_sale_order` (no watermark, no flag). `serp_test` `ec_order.id` is a local AUTO_INCREMENT and detection joins on `ec.order_id`/`odoo_id` (never raw `id`). ⚠️ **Copier dup-bloat bug (found 2026-06-23):** `staging_copier_worker.py` declares `_PK['ec_order']='order_id'` and uses `INSERT … ON DUPLICATE KEY UPDATE`, but the staging `ec_order` table (schema mirrored from `manage_schema.json`) has `order_id` as a **non-unique** index — its only unique key is `PRIMARY(id)`, and `EC_ORDER_COLUMNS` omits `id` — so the conflict never fires and every 300s cycle plain-INSERTs ~5k fresh rows (live `serp_test`: ~950k rows for ~5.1k distinct orders, up to ~209 copies of one `order_id`). Sibling staging tables (`items`/`component_orders`/`preselect_orders`) are clean because their column lists DO include `id`. Detection stays \_correct* (paths use `SELECT DISTINCT ec.order_id` + `NOT EXISTS serp_sale_order`) so this is a latent cost/timeout risk, not a missing-row cause. Fix: add a UNIQUE index on staging `ec_order(order_id)` via a `seeding/migrations/*.sql` migration (+ one-time dedup keeping `MAX(id)` per `order_id`), and/or have the copier write the real Laravel `id`.\*\*
...
**The SERP replica/darklaunch matrix** (⚠️ **the 5-DB local Docker layout below is RETIRED — SERP PR #46 consolidated local to 3 DBs: `serp_local_prod`/`serp_local_staging`/`laravel_local`;** live darklaunch history is Hetzner `serp_test`, prod is `serp_app`): historically `{prod, staging} × {replica, darklaunch}`, all MySQL with `serp_*` tables.

- **REPLICA DBs** (retired names `serp_staging_replica`, `serp_prod_replica`) — clean row-for-row mirrors of `manage`/`laravel_live` with ZERO Odoo overlay; the app reads/writes these; nearly empty shells (frozen ~2026-05-28).
- **DARKLAUNCH DBs** (`serp_staging_darklaunch`, `serp_prod_darklaunch`) — replica PLUS Odoo overlay (normal BOMs, MOs, SVL, POs) where the worker dual-writes Odoo-style ops. `serp_prod_darklaunch` is the future production DB. The Hetzner `serp_test` (staging SERP) holds the darklaunch history and runs consistently ahead (~35K stock_moves vs ~34K in `serp_prod_darklaunch`); production is `serp_app`.
...
- 🛑 **HISTORICAL — `scripts/build_darklaunch_drift_n8n.py` was DELETED 2026-06-11 (`63b694219`) and the n8n workflow is INACTIVE.** The monitor moved in-repo (`backend/workers/crons.py:155`, `workers/darklaunch_drift/worker.py`) and that cron is itself `active=0` (see the drift-monitor entry). Do not try to regenerate from this script — it does not exist. The n8n lessons below are kept only as design history. Formerly: 9 linear nodes, LOOP-FREE (no SplitInBatches — it stalled ~14/42), uses a **Merge barrier** ("Wait For Both") because without it the report fires when only the 1st branch arrived → false-flags. Hard-won lessons: Slack `messageType:text` with mrkdwn (block-only `blocksUi` throws 'no_text'); date-only vs datetime tolerance; NULL↔numeric-zero coercion; float ±0.01; boolean↔0/1.
...
| `laravel_local`                         | MySQL      | Local Docker (serp-mysql, port 3307, devuser) — 13 Laravel catalog tables, schema-only                                               |
| `serp_test`                             | MySQL      | **STAGING SERP DB** (darklaunch history) on Hetzner `5.161.233.240:3306` — prod is `serp_app` (old `live_darklaunch_db` key removed) |

**SERP's replica/darklaunch matrix**: historically `{prod, staging} × {replica, darklaunch}`, all MySQL with `serp_*` tables (⚠️ those 4 local Docker names are RETIRED — SERP PR #46 → local is now `serp_local_prod`/`serp_local_staging`/`laravel_local`; live darklaunch = Hetzner `serp_test`, prod = `serp_app`).
...
- **Darklaunch DBs**: replica PLUS Odoo overlay (normal BOMs, MOs, SVL, POs). `serp_prod_darklaunch` is future production. `serp_test` on Hetzner (staging SERP) holds the darklaunch history and runs consistently ahead (~35K stock_moves vs `serp_prod_darklaunch`'s ~34K); production is `serp_app`.
...
❌ AI assumes `serp_test` is a throwaway test DB → ✅ Reality: it is the **STAGING SERP DB** on Hetzner (with the darklaunch history), not a throwaway — but also NOT production (**prod is `serp_app`**).
...
- **`sugarwish_id` = THE external sync key** (NOT on `product_product`). Decode: prefix `'800'` → strip to get `components.id` (e.g. 800439→component 439); prefix `'500'` → `'500'`+`buyer_products.id`; other int → `receiver_products.product_id` OR `buyer_products.id`; 0/NULL → no mapping (**~2,264 of 7,515** as of 2026-09-11; was ~1,371).
...
- **`serp_stock_move`**: state enum **INCLUDES `partially_available`** — all 7 Odoo states, verified live on `serp_test` 2026-09-11: `enum('draft','waiting','confirmed','partially_available','assigned','done','cancel')`. ⚠️ **Corrected 2026-09-11** — this previously read "MISSING `partially_available` … seeder collapses Odoo's value to `assigned`", and prescribed removing the override. **The override is already gone** (`seeding/scripts/seeders/_seed_stock.py`, now carrying an explicit comment retracting the old "not in the manage enum" premise). A live count still shows **0** `partially_available` rows (7,990 `assigned`) purely because the last full-history reseed predates that fix — that is **reseed lag, not the old bug**. `quantity_done` is **computed store=False** from move_lines, but **has a working inverse** (`_set_quantity_done`, `stock_move.py:155-205`) — the old "writing it silently no-ops" is stale; editing `serp_stock_move_line.qty_done` is the safer explicit route, not the only one. `product_qty` is a **stored compute** converting `product_uom_qty` into the product's native UoM (`stock_move.py:24-28` + `:246-264`, mirroring Odoo's `product_uom._compute_quantity`); the older "ALWAYS NULL (known UoM-normalization bug)" note is **stale**. `purchase_line_id`, `sale_line_id`, `production_id`, `raw_material_production_id`, `odoo_move_id`. No row-level locking → duplicates if run in parallel.
...
- **`serp_mrp_bom`** (**~1,447 rows as of 2026-08-10, create_date 2022-11-08 → 2026-06-25, 100% `odoo_id`-linked** on both `serp_test` and `serp_app` — fully Odoo-synchronized, NO LONGER the frozen ~224–406-row, link-less 2026-04-03 snapshot the older note described; `type='normal'` for BOMs, `type='phantom'` for kits). `/boms` page filters `type='normal'`, `/kits` filters `type='phantom'`.
...
- **`serp_account_move`** (30 fields on the ORM model): ⚠️ **Four claims here were corrected 2026-09-11 — all were stale.** (1) `purchase_order_ids` is a **Many2many through the same junction Odoo uses** (`account_move.py:141-147`, `relation="serp_account_move_purchase_order_rel"`), not a direct FK. (2) `serp_account_move_line` uses **`product_id`** (`account_move.py:340`); the `component_id` `action_reverse` bug was fixed 2026-03-05 and no `component_id` remains in the accounting models. (3) `debit`/`credit`/`balance` are **NOT lossy floats** — SERP's `fields.Float` is **Decimal-backed** and maps to a `DECIMAL(precision, scale)` column (`serp_orm/fields.py:326`, `:344`), declared `digits=(16,4)` with `Decimal` defaults; the "Float not Decimal (precision loss)" note was a misreading of the class name. (4) `payment_state` is now a **stored compute** (`account_move.py:122-131`, `_compute_payment_state` `:265-302`). `move_type`/`state` enums as Odoo. **Still true:** move-level `amount_residual` is a plain field (`account_move.py:119`) that the reconciliation engine deliberately does not update — see the `payment_state` caveat below.
...
- Missing AUTO*INCREMENT → INSERT 1048 errors: **`serp_stock_location` and `serp_stock_picking_type` are now FIXED** (both `id` columns carry `auto_increment` as of 2026-08-10); **`serp_res_currency.id` still lacks it** (verified live 2026-08-10 — `extra` empty), so that one still 1048s on a bare INSERT. M2M rel tables missing `serp*` prefix (`purchase_order_stock_picking_rel`, `stock_move_line_consume_rel`).
...
- **`is_printed`** ⚠️ **(lives on `receiver_production_slips`, join via `order_id` — NOT a column of `ec_order`; confirmed 2026-09-11)** multi-state int: 0=not printed, 1=printed/in-fulfillment (auto-removes from print group), 2=Issue (missing boxcard / no high-res image), 3=address/label-blocked / on-hold, 5=in print group.
...
- `inventory_qty` (Englewood/current SA), `mi_inventory_qty` (Taylor), `odoo_inventory` (synced cache, can be NEGATIVE -122/-44/-325, stale — use `inventory_qty` for decisions), `prelist_qty` (int NOT NULL, default 0 — **prelisting**: qty Sophie wants sellable before an MO is packed; added 2026-07-01; drain wiring LIVE as of 2026-07-13 (SERPY subtracts on MO, logging each drain to the **`prelist_qty_log`** audit table); the Odoo→Laravel inventory sync cron ADDS it into `inventory_qty`, SERPY SUBTRACTS from it — floored at 0 — when it MOs the product; reporting must SUBTRACT `prelist_qty` to avoid double-counting; does NOT touch Odoo, so costing stays correct), `inventory_link` (parent product_id for cascading disables — set NULL on combo-line children in Apr-2026 migration), `location_id` (FK locations; **NO LONGER all-NULL — ~2,431 rows populated as of 2026-08-11**; SERPY create paths now write it and pin new `-E` parents to Englewood=1 via `_force_englewood_for_e_sku`; still NOT the deduction source — that's Odoo `stock_location.sugarwish_id`), `vendor_id`, `vendor_cost` (decimal 8,2), `product_id` (= `sugarwish_id`, join to `product_template.sugarwish_id`), `product_type`, `tango_utid`, `odoo_id`. **Duplicate SKUs exist** (SA-01-026-A as product_ids 1706+4499; 13 component SKUs duplicated) — use `product_id`/`id` for joins, NOT SKU.
...
**WishDesk LIVE DB host — `sugarwish-manage` RDS is GONE (NXDOMAIN); live is on `database-1`.** The old `sugarwish-manage.cqqg1tfyyubp.us-east-1.rds.amazonaws.com` RDS instance was deleted/renamed — the hostname no longer resolves (verified 2026-07-23), so ANY connection attempt to it fails with `Unknown MySQL server host` (direct) or `Lost connection … reading initial communication packet` (through a tunnel — the bastion can't resolve it either; the local forward comes up but collapses on first use). **Do not build tunnels to it; the host is dead, not firewalled.** The live `sugarwish_wishdesk` schema now lives on **`database-1.cqqg1tfyyubp.us-east-1.rds.amazonaws.com`** (the same private RDS as `laravel_live`/`sugarwish` — bastion tunnel required, not directly reachable; migrated ~2026-05-04, 190 tables, actively written). The old read-only user `wishdesk-read-only-for-wd-9182-2` **works on `database-1`** (migrated with the data). SWAC's local `.env` `DB_HOST_LIVE` was repointed 2026-07-23 to `localhost:13307` (SWAC's own `LIVE_SSH` tunnel → `database-1:3306`); if `mcp-db-tool-live` fails to load in a SWAC session, the usual cause is **no 13307 tunnel up yet** (start `npm run dev`, whose tunnel manager binds it) — not bad credentials. Jack's TablePlus "WishDesk Live" confirms the exact same target: `database-1` / `sugarwish_wishdesk` over the `forge@34.194.213.27` bastion — that IS production, no other host. ⚠️ **`branding_records` on live is genuinely EMPTY (0 rows) and that's the real production state, not a bug or lost data** — proposal branding does NOT live there. It lives **inline on `proposals.physical_branding` / `proposals.digital_branding` (JSON columns)** — e.g. proposal 149378 carries ~4 KB of physical_branding (sleeve entries with S3 render/mockup URLs) directly on its row. `preorders` has the same inline pair. So don't hunt for a `branding_records` row by id (a "branding_record_id 7197"-style reference does not resolve to that table on live); query the proposal's own JSON columns. (Do NOT confuse it with `sugarwish_manage_new_2.branding_records` — that is the **manage app's own, separate, actively-written** branding table; see below.)

**Manage (Laravel admin) lives on Hetzner — `sw-manage-db` `5.78.203.128`, NOT AWS.** Per Jack (2026-07-23): manage moved to Hetzner. The hub's `manage` MCP connection points there (`5.78.203.128` / `sugarwish_manage_new_2`) — that is the correct, current manage DB (actively written; e.g. `branding_records` ids ≤ 3332 updated 2026-07-22, `manage_audit_logs` ~3.07M rows). A `sugarwish_manage_new_3` schema also exists on the same box (sparse — appears new/in-progress; `new_2` is the live one). Manage's `branding_records` is the **manage app's** table and is unrelated to WishDesk's `branding_records` despite the identical name. (Separate Hetzner box: `sw-wishdesk-db` `5.78.187.176` carries `sugarwish_wishdesk_dev` — WishDesk DEV, which SWAC's confusingly-named `DB_*_MANAGE` env vars actually point at.)
...
**CRM slow-route perf gotcha (WishDesk `crm-api`):** `rev_by_company_by_date_detailed` is a **heavy revenue-by-company-by-date MySQL VIEW** (`laravel_live`) whose unbounded, un-date-windowed use is the root cause of the multi-second-to-minute CRM slow routes (e.g. `GET /client-profile/company/:companyId/domain-insights`, med ~49s / max ~78s — EXPLAIN shows it materializing the full ~512M-row derived set). The fix pattern is to **decompose the view into scoped base-table queries** (`buildDateKeyedComponent` / date-keyed component), not `FROM rev_by_company_by_date_detailed`; most interactive `crm-api` routes were migrated off it as of 2026-07-15, with `clientProfileService.ts` domain-insights the last hot-path straggler and `holidayReportingService.ts` remaining as offline/report-only usage. A second slow pattern in the same routes: the **`ecard_summary` uncorrelated aggregate** (`GROUP BY gc.order_id` with no `order_id` filter — e.g. `/orders/company/:companyId`); the fix is to scope it with `WHERE gc.order_id IN (...)`. Underlying anchor: the company table has **no index on `domain`, `name`, or `magento_name`**, so both the domain-insights scan and the CRM search-route `LIKE` full-scan.
...
#### `swcrm_z_gmail_messages` (**~525k** as of 2026-09-11; the old ~155K is stale)
...
- `orders_tickets` (**~681** as of 2026-09-11; the ~94 below is stale, as is its priority split): SEPARATE system, `priority` varchar(20) HIGH(6)/MEDIUM(42)/LOW(46); `company_id` = SugarWish corporate account.
...
- **`serpy_sync_queue`** (was `odoo_sync_queue`/`_dev`/`_live` in Retool PG — ⚠️ **MOVED 2026-09-11: the live queue is `serpy_sync_queue` in MySQL `serp_app` (env-routed); the Retool-PG `odoo_sync_queue*` tables are FROZEN since 2026-06-26.**): SERP↔Odoo sync engine. Columns: `entity_type`, `entity_id`, `operation`, `payload`, `status` (`pending`/`processing`/`synced`/`partial`/`failed`/`cancelled` — ⚠️ **no `dlq`**, verified in `models.py` 2026-09-11), `attempts`/`max_attempts`(=5), `priority`, `odoo_id`, `error_message`, `next_attempt_at`, `origin`. Worker polls **15s** (⚠️ not ~30s), BATCH_SIZE=1 prod, exp backoff capped 300s. `odoo_sync_circuit_breaker` (`state` CLOSED/OPEN/HALF_OPEN, 5-failure threshold). `odoo_sync_stats`.
...
**SERP Bot = the superuser's own partner, renamed (PR #1052, `53f24679e`, dev+main).** The seed no longer INSERTs a separate `__serp__` user+partner at id `2_000_000_001` as the chatter author; `_seed_serp_bot` now RENAMEs the superuser's partner (login `__system__`, partner id=2) to "SERP Bot" via UPDATE. So there is ONE bot identity (system chatter authors to it), NO SERP-origin bot row, and the old `2_000_000_001` id (plus its #453 AUTO_INCREMENT-floor side effect) is GONE — do not reason from "SERP seeds a bot at 2_000_000_001".
...
- Decode by prefix: `'800'` prefix → strip to get `components.id` (~2238 templates); other integer → `receiver_products.product_id` OR `buyer_products.id`; `0`/`NULL` → no mapping (**~2,264 of 7,515** as of 2026-09-11; was ~1,371).
...
- **Status enum:** `pending` → `processing` → `synced` | `partial` (created but not finalized) | `failed` (retrying) | `cancelled`. ⚠️ **There is no `dlq` state** (corrected 2026-09-11 — `SyncStatus` in `services/odoo_sync_queue/models.py` has no such member).
...
⚠️ **AI lumps replica/darklaunch/shadow/serp_test together** → ✅ **Reality:** The replica-vs-darklaunch _concept_ is DISTINCT (clean Laravel mirror vs Odoo-overlaid). ⚠️ **But the four DB NAMES below are the RETIRED local Docker layout** — SERP PR #46 consolidated the local Docker DBs 5 → 3, so the current local DBs are **`serp_local_prod`** / **`serp_local_staging`** / **`laravel_local`** (on `serp-mysql`, `127.0.0.1:3307`); the live darklaunch history is Hetzner **`serp_test`** (staging) and prod is **`serp_app`**. The old matrix was **{prod, staging} × {replica, darklaunch}**, all MySQL with `serp_*` tables:

| DB (RETIRED local name)                                            | Contents                                                                                                                                                                                                               | Fingerprint (historical)                                                             |
| ------------------------------------------------------------------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------ |
...
| **DARKLAUNCH** (`serp_staging_darklaunch`, `serp_prod_darklaunch`) | Replica PLUS Odoo overlay (normal BOMs, MOs, SVL, POs). Worker dual-writes Odoo-style ops here. `serp_prod_darklaunch` is the future production DB (~10.2k SOs, ~2k POs, ~5.5k pickings, ~34k stock_moves at cutover). | HAS `_migrations` (⚠️ `serp_darklaunch_meta` DROPPED 2026-08-03, commit `809a4d8f9`) |

- **`serp_test`** (MCP key) = MySQL `serp_test` on **Hetzner `5.161.233.240:3306`** — the **STAGING SERP DB** (NOT a throwaway test DB, NOT production — **prod is `serp_app`**), holding the darklaunch history the worker writes. Consistently _ahead_ of `serp_prod_darklaunch` (~35K moves vs ~34K). TZ = `America/Denver`. (The old `live_darklaunch_db` MCP key was removed — same DB.)
- **"compare-replica" tooling actually compares darklaunch.**
- ⚠️ **The darklaunch drift monitor is no longer a decision input (Jack, 2026-08-17: "dont use teh drift montior anymore").** It is deliberately **left RUNNING, not disabled** ("dont disable the darklaunch drift just dont worry about it") — so its Slack alerts to `#jack-test` still fire and should be treated as **ignorable noise**, not as a cutover-readiness signal or a reason to open work. The `<1% drift` gate and the suppression pipeline documented below describe how the monitor behaves, **not** a gate anyone is still watching; the drift monitor dies at cutover regardless. Do not propose re-tuning it, and do not cite a drift finding as evidence that something is broken without checking with Jack first.
...
- **Per-env reseeds:** prod darklaunch `2026-06-04 09:27:20`; staging `2026-06-03 11:52:27` (each env's `serp_darklaunch_meta.darklaunch_cutover_at` is authoritative; the `serp_test` MCP key shares the prod key).

#### Pre/post cutover behavior

- **Pre-cutover (seed):** seed all tables from Odoo (`INSERT...ON DUPLICATE KEY UPDATE`); high-volume tables (sale_order, sale_order_line, stock_picking, stock_move*, stock_move_line*, SVL, account_move\*, mrp_production) seeded with only the **latest ~5,000 ids** as of seed-start snapshot. Whole-table row-count delta vs live Odoo is EXPECTED. **Odoo chatter is seeded too (2026-07-22, `--chatter-only` flag):** `serp_mail_message` + tracking values + `serp_ir_attachment`(+rel) twins per model (~415k messages / 2.6k attachments on live `serp_test`); attachment BINARIES upload to S3 **`s3://sw-serp/serp-attachments/`** keyed by checksum (re-seeds skip already-present objects); `sale.order` chatter is WINDOWED to the seeded order window — an old SO showing "No messages yet" is by design, not a sync bug; a collision guard skips incoming rows colliding with runtime (`odoo_id IS NULL`) rows and AUTO_INCREMENT floors are bumped to `odoo_max + 10,000,000` on all shared chatter/catalog tables (worker-collision guard).
...
⚠️ **AI tries to hand-edit the n8n drift workflow JSON** → ✅ **Reality (updated 2026-09-11):** don't hand-edit it, but **don't regenerate it either** — `scripts/build_darklaunch_drift_n8n.py` was deleted 2026-06-11 and the n8n workflow is inactive. The drift monitor is now an in-repo cron (`workers/darklaunch_drift/worker.py`), and that cron is `active=0` in every env.
...
- **Bin-level `location_id` divergence** (3 of 9 in-window worker stock*move_lines write parent loc 8 where Odoo refines to a bin like 1285/1358/1366/3927). ⚠️ **Cause clause retracted 2026-09-11:** this previously read "SERP doesn't replicate Odoo putaway/removal strategy", which is no longer supported — `_gather` \_does* expand to child bins and stamps the line at the gathered quant's bin. That phrase also conflated two separate things: **removal strategy IS implemented** (`stock_quant.py:79-90` + `:141-144`, with the Python re-sort at `:148-166`; only the _resolver_ at `:66-77` hardcodes `"fifo"` pending `removal_strategy_id` columns, which matches Odoo's own default anyway), while **putaway is a genuine stub** (`stock_location.py:292-308` — `get_putaway_strategy` is `return self`). Note the observation here (writing the **parent**, loc 8) is the _opposite_ of the expansion symptom, so it may be a distinct phenomenon; **the cause is currently unknown — re-derive it.** Do **not** add `location_id` to `WORKER_ROW_DIVERGENT_COLUMNS['stock_move_line']` until then: suppressing it could mask the parent→child self-cancel signal. (`stock_move_line.move_id` FK id-space and `reference`=NULL are separate and still fine to suppress.)
...
#### Darklaunch Order Worker (`serp_order_queue_worker.py`)

- **NO queue.** ⚠️ **The `staging_copier_worker` was RETIRED 2026-07-30 (commit `94deba129`)** — new-order detection no longer reads a locally-staged `serp_test` copy (nor `ec_order WHERE oddo_synchronized=1` against live Laravel); Laravel-owned rows are read on a Laravel pool and detection gates on the per-order SERP flags. Historically (2026-06-17→07-30) it read the copier-populated staging copy; `POLL_INTERVAL=300s`, `BATCH_LIMIT=50`, single-threaded. Dual-writes receiver-order + preselect-order rows into `serp_prod_darklaunch` ONLY (never the main DB). SHADOW-processes Odoo-sourced orders (`order_type IN ('receiver-order','preselect-order') AND inventory_source='odoo'`), reading read-only against Odoo/live MySQL for seeding, never pushing back to Odoo. Replaces Odoo's Update-Orders/Prepicks crons by design. ~80ms AWS→Hetzner RTT dominates ~25–27s/shipment.
...
- **Prelisting (`receiver_products.prelist_qty`, added 2026-07-01):** lets Sophie make MOs sellable on the site _before_ they're packed, WITHOUT touching Odoo — deliberately avoiding the placeholder-MO/unbuild trick that corrupts product costing. Design (Jack + Anna, decided in `receiver_products` not a separate table): the Odoo→Laravel inventory sync cron ADDS `prelist_qty` into `inventory_qty` (so `inventory_qty` may go negative — expected/fine), and SERPY SUBTRACTS from `prelist_qty` (floored at 0) when it MOs that product. Any inventory reporting must SUBTRACT `prelist_qty` or it double-counts. As of 2026-07-13 the wiring is **LIVE and firing** (drain rows confirmed 2026-07-11). SERPY writes an audit ledger **`prelist_qty_log`** (per-row `product_id`, `source` e.g. `serpy`, δ/delta, `qty_after`, `changed_at`, draft id) each time it drains; a drain does `qty_after = max(0, prelist_qty − produced_qty)`. **Wrong-key decrement bug (#392/#430):** SERPY drained the _wrong_ product's row → ~9 SKUs needed backfill rows ("wrong-key decrement drained wrong row; prelist reconciled"). Caveats when auditing: the ledger can be **incomplete** — a live `prelist_qty` can be correctly 0 with **no** matching decrement row logged (one observed case, net-harmless); a SKU with **zero rows in the whole log** means prelist was never added → a produced MO correctly drains nothing (no-op, not a bug). `receiver_products.prelist_qty` is the live truth; `prelist_qty_log` is the (imperfect) audit trail. (Table name is singular `prelist_qty_log` — `prelist_qty_logs` does NOT exist.)
...
- Odoo `stock_location.sugarwish_id` controls Laravel sync: **`>0` syncs to Laravel** (only `EW/Stock/Fulfillment` id=2008 has `sugarwish_id=1`), **`=0`** internal-only, **`NULL`** system/virtual. **⚠️ `receiver_products.location_id` is NO LONGER all-NULL** (the old "unused" note is stale — live-verified 2026-08-11: ~2,431 rows now carry a non-NULL `location_id`). It is now populated by SERPY create paths, and new `-E` parent SKUs are pinned to Englewood (`location_id=1`) by `_force_englewood_for_e_sku` (`backend/workers/handlers/multi_target.py`, shipped 2026-07-31 commit `5838a1e1e`; the `laravel_update_receiver_product` **update** op canNOT write `location_id` — it's not in `RECEIVER_PRODUCT_WRITABLE`, so unarchive/enable ops leave it untouched). A data-assumptions invariant (`active_e_parent_skus_are_englewood`, `backend/data_assumptions/registry.yaml`) asserts every **active** `-E` parent (`archive=0`, `deleted_at IS NULL`) sits on `location_id=1`; pre-guard legacy rows created before 2026-07-31 can violate it, and un-archiving one (e.g. product_id 5101 SA-03-045-E un-archived by draft #1861 on 2026-08-11) trips the alarm — a known/anticipated false-alert, corrected by an ops `UPDATE ... SET location_id=1`, not a create-path bug. **The physical inventory-deduction location is still resolved via Odoo `stock_location.sugarwish_id`** (EW=2008, TY=2006), NOT this column.
...
- **Bakery & Cafe** → normally TY, but **OVERRIDE to EW if a sleeve/custom merch is attached**. ⚠️ **The override is implemented as `laravel_live.buyer_products.custom_location_id` (nullable), NOT by any suffix logic** — `ReceiverProductService::resolveLocationForReceiverFilter()` (`app/Services/Receiver/ReceiverProductService.php`, prod branch `blue`) returns `custom_location_id` when the giftcard has a branding record with `physical_branding` or `merchandise`, else `location_id`; the caller then applies `if ($locationFilterId !== null) { $query->where('location_id', …) }`. **A NULL `custom_location_id` does NOT default to EW — it silently SKIPS the location filter entirely**, so the receiver is offered the unfiltered catalog (both `-A` and `-E`) and picks whichever. The column has no backfill in its migration (`2026_05_08_120000_add_custom_location_id_to_buyer_products_table.php` only adds it nullable) — it is hand-populated, so a missed row is a silent routing bug.
...
2. **SERP `serp_mrp_bom` phantom BOMs** (`type='phantom'`): shippers `S-*`, inserts `I-*`, production paper `P-*`, cups `E-*`/`C-*`, merch recipe_keys. Edited via Serpy `serp_update_kit` / "Update SERP Kit". Actively maintained in `laravel_live` (the kit/BOM SoT) and `serp_app` — 438 phantom rows, 252 active, last written 2026-09-09. Only the stale `serp_test` staging copy still shows the old ~224-row April-2026 snapshot, so a count near that means the wrong DB was queried.
...
- **Phantom BOM** = a kit that EXPLODES into component moves **at SALE/delivery time** (no separate MO). It creates ZERO `mrp_production` records. ~265-775 active phantom BOMs in Odoo. ~97.9% of Odoo BOMs have `product_id=NULL` (linked via `product_tmpl_id` only) → causes "no BOM found" in `serp_find_kits` which matches by `product_id`. ⚠️ **Corrected 2026-09-11 — SERP handles this.** `_bom_find` (`mrp_bom.py:170-219`) builds Odoo 15's combined domain (product-specific **OR** template-level `product_id IS NULL`) and resolves in two passes so the product-specific BOM wins regardless of MySQL's NULLs-first ordering. The old "only searches `product_id`, no fallback (docstring claims one; code doesn't implement it)" is exactly backwards and predates the 2026-05-28 `_bom_find` single-domain rewrite.
...
- **SERP phantom flip** (`feature/popcorn-phantom-explode`) — ⚠️ **RETIRED 2026-09-01 (PR #1056, `4dd56cd1f`, dev+main).** Historically the SERP seeder `make_popcorn_boms_phantom()` flipped `SA-02-*` to phantom in the LOCAL/darklaunch seed only (never in Odoo), auto-exploding the kit into costed cup+sticker leaf moves — an approved divergence (`ODOO_APPROVED_DIFFERENCES.md` §2.6). That function is now DELETED; SA-02 popcorn BOMs stay `type='normal'` matching Odoo, so popcorn lines ship as the uncosted flavor SKU (reopening the ~$0.17/flavor COGS gap once recorded fixed 2026-06-02 — resolution undecided).
...
**The ONLY signal that matters** is the per-table `values:` line: "K rows, C columns — clean" = faithful; "X/Y rows diverge" = real drift; ":red_circle: N rows 30m+ old in Odoo MISSING from darklaunch" = genuine sync gap. Real bugs are a minority: dropped order lines when `product.default_code` is NULL, seeder collapsing `partially_available`→`assigned`, bin-level `location_id` divergence (cause **unknown** — the old "SERP doesn't replicate Odoo putaway/removal strategy" explanation was retracted 2026-09-11; see "The minority that ARE real bugs").

#### Drift workflow is code-generated — do NOT hand-edit

🛑 **HISTORICAL (updated 2026-09-11):** the n8n "Darklaunch Drift Monitor" JSON was code-generated by `scripts/build_darklaunch_drift_n8n.py`, **but that script was deleted 2026-06-11 and the workflow is inactive** — the monitor moved to an in-repo cron (`workers/darklaunch_drift/worker.py`), itself `active=0`. Neither hand-edit nor regenerate; the notes below are design history. It's loop-free (no SplitInBatches; SplitInBatches stalled ~14/42 tables and never fired Slack), uses a **Merge barrier** (Wait For Both — without it, fires on the first branch only → false flags), `_equiv` normalization (date-only vs datetime tolerance, NULL↔'0.0000', float ±0.01, bool↔0/1), runs hourly cron `0 6-18 * * 1-5` (6am–6pm MT Mon–Fri), windows latest ~1000 ids per table across ~42 tables, alerts to `#jack-test` (`C083M27KU8L`) for rows 30m+ old that are missing or settled-drift (`GRACE_MINUTES=30`, `WINDOW=1000`). Slack `messageType:block` blocksUi-only throws a `no_text` error — use `messageType:text` with `mrkdwn`.

#### Drift monitor 4-layer noise-suppression pipeline (deployed 2026-07-09)

⚠️ The drift monitor now has a **4-layer noise-suppression pipeline** (PR #457, migration 056, deployed to prod 2026-07-09). In `backend/workers/darklaunch_drift/worker.py`: **L1** boundary-safe windowing (GL/account_move jitter → structurally ~0); **L2** age/recency grace (today + 6h); **L3** a **persistence gate + delta-trend** backed by a new `serp_app.drift_sweep_history` table (migration 056) — a finding must persist and NOT be shrinking before it alerts; **L4** a benign-classifier that collapses known-benign items into an `ℹ️ known-benign` footer. **Critical safety nets** so suppression can never hide a real drift: a **material-shrink floor (≥10%/sweep)** — a slow-trickling real drift (e.g. Δ360→359→358) no longer reads as "converging" forever — plus **per-section hard ceilings** that always alert at N regardless of trend. Because L3 needs history to accumulate, **the first ~2 post-deploy sweeps still look un-suppressed** (expected). This layers ON TOP of the standing "neither LAG nor DRIFT is ever an acceptable answer" rule — the suppression only silences the benign-artifact categories, never a confirmed real divergence.
...
| `serp_test`(Hetzner`5.161.233.240`) | **STAGING SERP DB** (darklaunch history, not a throwaway; prod is `serp_app`), MCP key `serp_test`, consistently ~ahead of `serp_prod_darklaunch` | —                            |

- **Fingerprints:** darklaunch has `_migrations` (⚠️ `serp_darklaunch_meta` was DROPPED 2026-08-03, commit `809a4d8f9` — no longer a fingerprint; use worker-written rows / per-order SERP flags); replicas have neither and no Odoo-owned/manufacturing tables.
...
✅ **Reality:** Slowness is **deliberate** for costing parity. ⚠️ **Timings corrected 2026-09-11: prod is ~1.02s mean / 1.34s p95 per shipment** — the old "~27s local / 47–66s prod" was a local bench artifact, off by ~40x. Still serial, serial, ~130 per-move UPDATEs, to match Odoo's `account_move.sequence_number` ordering. Batching breaks `/compare-costing` parity. Throughput ceiling ~72 shipments/hr (~1,728/day) — **6–7× below** the Dec-2025 peak (peak day shipments 11,433 on 2025-12-29; peak hour 1,877 orders).
...
⚠️ SERP **intentionally does NOT replicate** (documented in `docs/ODOO_APPROVED_DIFFERENCES.md` / `KNOWN_BUGS.md`): invoicing flow (`invoice_status`/`commitment_date`/`effective_date` — worker creates SO with default `invoice_status='to invoice'`, never advances); `res.partner._increase_rank` supplier_rank bumps on PO confirm; PO-receipt/vendor-bill recompute fields; Odoo crons (reservation/ship) that move state post-create — **but NOT prepick, which IS replicated** (`_action_assign` pre-fills `qty_done`; see `ODOO_APPROVED_DIFFERENCES.md` §2.7, which reverses the older framing); `procurement.group` model; lot/serial tracking; package/owner tracking; `ir.model.data`/`env.ref()`; line-level chatter; `sale.order.action_confirm` deliberately does NOT trigger procurement or create a delivery picking (the darklaunch worker is the **SINGLE** picking creator via `stock.picking.action_process_new_order` — adding a second creator yields two pickings per order).
...
- **Darklaunch Order Worker** (`serp_order_queue_worker.py`): **no queue** — polls live MySQL directly (`ec_order WHERE oddo_synchronized=1`, `POLL_INTERVAL 300s`, `BATCH_LIMIT 50`, single-threaded), dual-writes receiver/preselect rows into `serp_prod_darklaunch` ONLY.
...
⚠️ `product_template.sugarwish_id` (lives on TEMPLATE, **not** `product_product`): prefix `'800'` → strip → `components.id`; prefix `'500'` → strip → `buyer_products.id`; other int → `receiver_products.product_id` OR `buyer_products.id`; `0`/`NULL` → no mapping (**~2,264 of 7,515** as of 2026-09-11; was ~1,371). Don't assume non-800 means receiver — could be buyer. `sugarwish-odoo` `sale.py` filters `startswith('500')`/`startswith('800')`.
...
⚠️ Only `stock_move.state='done'` actually changes inventory. In SERP, `quantity_done` is a **computed, store=False** field summed from `stock_move_line.qty_done` — ⚠️ **but it DOES have an inverse** (corrected 2026-09-11): `_set_quantity_done` (`stock_move.py:155-205`, mirroring Odoo's `_quantity_done_set`) distributes a written value across move lines and creates one if none exist, and is used in production (`mrp_unbuild_actions.py:236`). The old "writing it silently no-ops (no inverse method)" is stale. The misleading error "Set quantity_done on at least one move" still means no line carries a `qty_done`. Editing `stock_move_line.qty_done` remains the **safer explicit** route — the inverse deliberately creates lines without `product_uom_qty` (`stock_move.py:181-190`, Odoo L350-353: copying it "used to plant a phantom reservation") — it just is not the only route. `_action_done` historically used `product_uom_qty` (ordered) instead of actual completed qty (a bug).
...
#### `_gather()` DOES expand child locations (SERP) — and a parent→child transfer self-cancels

⚠️ **Corrected 2026-09-11 — the previous entry here said the opposite and was stale.** It claimed SERP's `_gather()` used an exact `location_id` match "while Odoo uses `child_of`", framed as an intentional divergence. That predates the "Bug #4" fix and is **wrong**. Verified live in `backend/serp_orm/models/stock_quant.py:124-139`: when `strict=False` (**the default**) SERP expands the location to all descendants via `get_child_locations` / `parent_path`, with the comment `Odoo L506: child_of expansion`. `strict=True` is the exact-match branch (`Odoo L510`). So child-bin stock IS visible to a parent-location caller.

⚠️ **The live consequence: a parent→child internal transfer silently self-cancels, inflating inventory.** `stock_move_actions.py:726` calls `_update_reserved_quantity` **without passing `strict`**, so it defaults to `False` and the source gather expands _into the destination subtree_. The move line is then stamped at the gathered quant's bin (`stock_move_actions.py:780-781` — "The gathered quant's bin, NOT self.location_id") while `location_dest_id` keeps the move's dest, and `_action_done` decrements at the line's bin (`decrement_loc = ml_loc`, `stock_move_actions.py:1332`). On a parent→child move both ends resolve to the **same** child bin: debit and credit cancel, the parent is never debited, and a phantom unit is left behind.

Neither field is individually wrong — **the corruption is emergent**, which is why it survived review. The trigger is **direction, not entry point**: verified 2026-09-11 on `serp_test` with product 29324 (`SA-51-042-E`), where `EW/Stock` (8) → `EW/Stock/Fulfillment` (2008, a child of 8) failed identically from **two different front doors** — the Products page UI (draft #3005, move `EW/INT/05200`) and SERPY (draft #3006, move `EW/INT/05201`). Both produced a move line with `location_id = location_dest_id = 2008`. The reverse direction (child→parent, draft #3004) worked, because 2008 is a leaf with no descendants to expand into. The phantom `1.0000` was still sitting on quant `10166775` at `EW/Stock` (write_date 12:31:47), untouched by the later transfers.

The two entry points above are what has been **observed** failing; the full blast radius across other flows that reserve at a parent location has **not** been surveyed yet, and what Odoo 15 itself does in this case (whether it guards against a destination inside the source subtree) is **unverified** — do not assume parity either way from this entry.

**Practical rule:** pass `strict=True` when the operation must not pull from child bins — `stock.scrap.action_validate` already does (`stock_scrap.py:203`, with the comment "scrap is AT a specific location, not 'this or descendants'"). Before trusting any "SERP doesn't expand child locations" claim, re-read `_gather` directly.
...
#### `qty_received` / `qty_invoiced` DO auto-update in SERP — imperatively, not via `@api.depends`

⚠️ **Corrected 2026-09-11 — the previous entry said these were "static columns that never update". Both halves are false.** `qty_received` is recomputed inside `stock.move._action_done` (`stock_move_actions.py:1510-1520`, compute at `:1728`) for incoming receipts **and** refundable purchase returns (which subtract); `qty_invoiced` is recomputed by `account.move._update_qty_invoiced` (`account_move_billing.py:803`, write at `:843`) called from `action_post`/`button_cancel`/`button_draft` (`account_move_actions.py:225`, `:335`, `:356`). **The real caveat:** SERP has no `@api.depends` machinery, so these are **imperative calls at the state transition** — a write that bypasses those methods will NOT refresh the columns. In **Odoo** they are stored computes. (Odoo data quality: 20 over-received lines where `qty_received > product_qty`, e.g. P01654 +30K; 266 partially received; 10 cancelled POs with 2–18 active moves.)
...
2. **SERP `serp_mrp_bom` phantom BOMs** (shippers `S-*`, inserts `I-*`, production paper `P-*`, cups `E-*`/`C-*`, merch) — edited via `serp_update_kit` / "Update SERP Kit". Actively maintained in `laravel_live` (the kit/BOM SoT) and `serp_app` — 438 phantom rows, last written 2026-09-09; only the stale `serp_test` staging copy still shows the old ~224-row April-2026 snapshot.
...
⚠️ `type='phantom'` BOMs do NOT create manufacturing orders (zero `mrp_production` rows reference a phantom `bom_id`) — they **explode into delivery-order component moves at SALE time**, not MO time. **~96-98% of Odoo BOMs have `product_id=NULL`** (linked via `product_tmpl_id` only) — ⚠️ **SERP's `_bom_find()` handles this** (corrected 2026-09-11): `mrp_bom.py:170-219` mirrors Odoo 15's combined product-specific-OR-template-level domain, two-pass so the product-specific BOM wins. The old "no fallback, causing 'No active BOM found'" claim predates the 2026-05-28 rewrite. `/boms` must filter `type='normal'`; `/kits` filters `type='phantom'`. **Safe to archive Odoo phantom BOMs** — all SERP runtime paths explicitly exclude phantoms (`boms.py`, `manufacturing.py` raises, `expander/odoo.py` raises); `/compare-darklaunch` excludes phantoms (`KNOWN_FILTERED_TABLES`).
...
⚠️ **Both halves corrected 2026-09-11 — these gaps are CLOSED.** (1) The "~62 of 121 phantom-BOM component product_ids missing from `serp_product_product` → BOM creation FK-fails" precondition is gone: verified **0 missing** of 149 distinct components on `serp_app` (and 0 of 69 on `serp_test`) — the products were imported. (2) `serp_mrp_bom.product_id` is **NOT** `NOT NULL` — it is nullable (`mrp_bom.py:60` has no `required=True`; live column `nullable: true`; **1,402 of 1,675** rows on `serp_test` are NULL), so the claimed "schema divergence blocks sync-back" does not exist. Odoo BOM `product_id` being ~96-99% NULL (template-level) is still true, and SERP now mirrors it.
...
⚠️ `ec_order.product_type` classifies the **order context**; `receiver_products.product_type` classifies the **physical product**. The same cookie can be `567` (Bakery & Cafe) on one order and `47` (Custom Mug & Treats) on another. Route by `ec_order.product_type`. SA SKUs are context-dependent in cost reporting — a SKU may show blank for a product-type week even though it shipped under a different type. **The two legitimately disagree all the time** — of stacked legs shipped since Jan 2026, **327 of 1,236** have an item whose `product_type` differs from the order's, and the common pairs are all valid cross-line picks. So an item/order type mismatch is NOT by itself a bug (verified live 2026-07-28).
⚠️ **Gift cards (type `43`) are the exception that DOES have to match** — of 6,895 gift-card-item orders since Jan 2025, **6,894** carry `ec_order.product_type = 43` (one outlier in 14 months). Five fields move together on a real gift-card order and must be judged as a set, never restamped individually: `product_type=43` · `size` = a GC `buyer_products.id` (921/923/924/928…) · `sw_fulfill=0` (vendor/Tango) · `gift_total_picks=0` · `items.odoo_sync=5`. The `-43-` segment in `SA-43-0NN-A` SKUs IS the product-type number. **`44` is a sibling gift-card type** — Laravel special-cases 43 and 44 together on the stacked leg; 43 is the main one. **Do NOT confuse with `13`** — `giftcards_card.product_type = 13` is "Customize a Sugarwish", the _sender-side_ card product (that's what WW-1409's 567→13 fix was about; different table, different meaning). A gift card as the SECOND gift of a stacked card (`is_stacked=2`, `product_type=43`) is a **routine, supported flow**, not corruption. Because type 43 has `bypass_odoo=1`, mis-stamping a gift-card order with a non-GC type (e.g. 567) sends it into the Odoo importer, which can never find it by `sugarwish_id` → permanent failed-products log + daily email loop (WW-1727). ⚠️ **Fixing such a row by setting `product_type=43` ALONE produces a shape matching neither pattern** (43 with a Bakery `size`, `sw_fulfill=1`, `odoo_sync=0`) and books a real Bakery sale as a gift-card sale in every report keyed on `ec_order.product_type` — either correct all five fields or leave it and annotate.
...
⚠️ `-A` = standard/Taylor (location*id 2); `-E` = branded Englewood (EW, location_id 1). Branded (sleeve/merch) orders must use `-E`; non-branded must use `-A` (an hourly n8n workflow flags mismatches). Forecast rolls `-A` and `-E` as the **SAME** product. "Move quantity -A to -E" = a PAIR of inventory adjustments, never a stock_transfer. Fulfillment overrides: Bakery & Cafe is normally TY but **OVERRIDES to EW if sleeve/custom merch**; Mini Popcorn moves from Poppin & Mixin to CityPop if sleeved; Custom Mug & Treats is normally TY but overrides to EW if sleeve/merch (causing split EW/TY shipments, double shipping, manual tracking emails). ⚠️ **The suffix is a CONSEQUENCE, not a decision — no code anywhere reads or writes `-A`/`-E` to route an order.** Routing resolves a \_location id* (`buyer_products.custom_location_id` for branded, `location_id` otherwise) and filters the catalog on `receiver_products.location_id`; branded orders get `-E` only because `-E` products happen to live on `location_id=1`. So "merch → `-E`" holds only while `custom_location_id` is populated — when it is NULL the filter is skipped and the receiver sees both suffixes. Forecast location attribution reads the FIRST item's `location_id`; physical mug production is in Taylor but forecast usage **rolls up to Englewood**. Only EW/TY are valid for production-slip printing (`validate_slip_rows` ALLOWED_LOCATIONS raises "Print blocked" otherwise).

⚠️ **OPEN (live-verified 2026-08-17): `buyer_products.custom_location_id` is NULL on `bakery-cafe-nano` (id 7030) and `onepick-brownies` (id 6994)** — all ten sibling Bakery & Cafe rows carry `custom_location_id=1`; these two were missed. Consequence on 7030: **1,041 branded Nano orders since 2026-06-03, still accruing daily (latest 2026-08-17)**, offered the unfiltered catalog and mostly picking wrong `-A` SKUs. 6994 has no branded orders yet — latent, not firing. **Not yet fixed** as of this note; the fix is Laravel-side data (`UPDATE buyer_products SET custom_location_id=1 WHERE id IN (7030,6994)`) and is the Laravel team's write, not SERP's — `custom_location_id` appears in SERP only in `seeding/snapshots/manage_schema.json`, no SERP logic reads it. Note the affected `-A` SKUs have `mi_inventory_qty=0` (no Taylor stock behind the pick). Also: **no data-assumption covers this** — `shipping_buyer_product_location_resolves` guards only non-NULL `location_id`, and `active_e_parent_skus_are_englewood` guards the `-E` catalog rows (which are configured correctly); nothing watches the routing _input_.
...
⚠️ A single oversized/malformed inbound email can take **all of desk.sugarwish.com down**. On 2026-06-29 a 624 KB Yahoo HTML email hit `server/services/email-parser.ts` **HTML Thread Extraction** (~line 483), whose quote-stripping `On … wrote:` regexes catastrophically backtrack (O(n²)) → 100% CPU → app hang → ALB unhealthy → site down. Two SendGrid **Inbound Parse** routes feed it: **`POST /api/email-webhook`** (`server/livechat/email-webhook-route.ts`, livechat replies, ~150 chat sessions/day) and **`POST /api/orders-email-webhook`** / `/api/orders_email_webhook` (`server/orders-tickets/email-webhook.ts`, ticket/order replies, ~3/day) — **blocking the pathway drops ALL inbound support email on that channel**, not one sender. Those two routes are configured as **FOUR Inbound Parse entries** in SendGrid (desk2, help.sugarwish.com, desk → `/api/email-webhook`, plus one whose _receiving_ host is `orders.sugarwish.com` but which POSTs cross-host to `desk.sugarwish.com/api/orders-email-webhook` — that cross-host mismatch is **intentional, do not "fix" it**). **Never delete-and-recreate a Parse entry to change it** — edit the Destination URL in place; the gap drops inbound mail with **no retry**. Env changes: desk2/desk3 are `.env` + `pm2 restart --update-env`, but **live/prod needs the var in AWS Secrets Manager followed by a Jenkins `sw-deploy-to-wishdesk-live` rebuild — a plain restart will NOT pick it up.** (WW-2103, 2026-07-28/29: added `?token=` query auth on the Parse URLs read via `SENDGRID_INBOUND_SECRET`, monitored with the `[INBOUND-AUTH] … configured=true valid=true` log line before flipping `INBOUND_WEBHOOK_AUTH_ENFORCE=true`; **SendGrid Inbound Parse and the Signed Event Webhook are different features** — `SENDGRID_WEBHOOK_SECRET` governs the latter and was deferred.) It re-hung within ~1 min of every restart because the app never returned 200, so **SendGrid kept redelivering** (access log: repeated 499s). Hardening (PR merged to `main` 2026-06-29): **(1)** cap HTML body size before parsing, **(2)** bound every quote-stripping regex with `{0,500}`-style limits / timeout guard instead of greedy `.*?`, **(3)** ALWAYS return HTTP 200 from the webhook so SendGrid stops the redelivery retry storm even when parsing fails. Mitigation during the incident was an out-of-band pathway block (no code change), since superseded by the merged fix.

#### WishDesk live-chat "Connection Failed" on desk2 is a socket.io transport-order bug, NOT infra

⚠️ The chat widget (`client/src/hooks/use-chat.ts`) and agent dashboard (`client/src/hooks/useAgentSocket.ts`) configure `transports: ['websocket', 'polling']` — **websocket first**. On the dev/staging tiers, nginx guards `/chat/socket.io` with HTTP Basic Auth (`.htpasswd`; **desk2 = `devuser`/`TimeToTest`**), and a browser **cannot attach cached basic-auth credentials to a native WebSocket upgrade** → nginx 401s the `wss://…/chat/socket.io/?…transport=websocket` handshake. With engine.io-client's `tryAllTransports=false` default, a failed first transport just **retries the same one forever and never falls back to polling** → widget stuck "Connection Failed." XHR/polling _does_ carry the cached basic-auth (why the rest of the site works). **Fix = flip to `transports: ['polling', 'websocket']`** (2 lines, client-only, no infra/Munyr): polling connects and carries auth, then upgrades to websocket where the WS handshake is unguarded. **Prod (`desk`) is unaffected** — it has no basic auth, so it still ends up on a real WebSocket. Nuance: the raw browser WS upgrade actually fails on **both** desk2 AND desk3 (basic-auth on both) — desk3/live only work because their **AWS-ALB/HTTP-2** front lets socket.io settle onto working polling, whereas desk2's **HTTP/1.1 nginx (Hetzner)** leaves it stuck on websocket. (Verified live 2026-07-09: end-to-end polling session driven through desk2's nginx delivered the engine.io 25s ping in real time.)

#### Socket.IO `join_room` was unauthenticated — the `agents` room leaked every live customer chat

⚠️ Until 2026-07-29 (WW-2143) **any anonymous WebSocket** could connect to the `/chat` Socket.IO namespace and emit `join_room { room: 'agents' }` with **no auth whatsoever**, joining the privileged agent broadcast room and streaming every live customer-support conversation — full message content and attachment URLs. Fix (`server/livechat/socket/connection-handler.ts`): agent identity comes **only** from a verified JWT **or a verified cookie session** — client-supplied `data.userId` is ignored entirely — on **both** the `agent-` notification branch and the main `JOIN_SESSION` branch, with a **DB role check** (authorize off the DB role, **not** a JWT claim). The client (`client/src/hooks/useAgentSocket.ts`) must send `externalJwtToken`/`jwtToken` in the `join_room` payload. **Debugging signature:** a client that omits the token is **silently rejected** — it looks like "agents stopped getting live notifications on `/admin/message-center`" with no visible error and no obvious log; check the join payload before chasing infra or transports. **⚠️ THE HOLE REOPENED AND IS STILL OPEN as of 2026-08-03:** the fix shipped as PR #3295 (merged 2026-08-01), but it wedged desk2's boot (see the Redis boot-hang gotcha below) so it was **reverted in PR #3300** (merged 2026-08-01, ~1h later) — reopening the hole, which SWQA re-rated **High** (unauthenticated customer-PII breach; `userId` is an enumerable auto-increment int, and id `1` exists). The re-land is **PR #3305 (still OPEN, base `development`)** — **`development`/desk2 carry the exposure until it merges; merge #3305 before #3306.** Regression coverage on the re-land: `join-room-auth.test.ts` (24 tests) + `session-identity.test.ts` (14). **Test-vacuity footgun found there:** `SECURITY: rejects a forged agent- connection supplying only a userId` **passed against a deliberately vulnerable build** — it emitted `userId: 7` with `storage.getUser` unstubbed, so the attacker was rejected at the _DB role check_ rather than the _credential check_, guarding nothing. Any auth test here must stub `getUser` to resolve a **real** agent row so only the credential check can reject; always verify a security guard **fails with the bug reintroduced**.

#### An unbounded `await` on WishDesk's boot path wedges the process BEFORE `listen()` — pm2 says "online", nginx says 502

⚠️ **Signature:** normal module-init logs (MySQL pools + `SELECT 1`, Slack client, Qdrant OK) then **total silence** — `listen()` never runs, port 5001 never binds, `ss -ltnp` shows no listener, yet **pm2 reports both instances `online`** at 0% CPU with normal memory, and nginx logs `connect() failed (111: Connection refused) -> 127.0.0.1:5001` for every request. Deterministic across boots. This is **not** the DB and **not** the network — it is an `await` during module init that never settles. **Root cause (PR #3295, desk2 outage 2026-07-31):** `server/livechat/socket/agent-announce-gate.ts` did a bare `await redis.connect()` on a `createClient(config)` with **no `socket.connectTimeout` and no `reconnectStrategy`**. node-redis's _default_ reconnect strategy retries **indefinitely**, so against an unreachable Redis that promise **neither resolves nor rejects** — a surrounding `try/catch` is **dead code** for this failure mode, since a catch can only observe a rejection, never a pending promise. Boot chain: `server/index.ts` (`await registerRoutes`) → `server/routes.ts` (`await setupLiveChat`) → `server/livechat/index.ts` (`tryAttachAdapter` → `initAgentAnnounceGate`) → the bare `connect()`. **Diagnostic tell:** _no pending Redis socket at all_ is the clue that distinguishes the two clients — the **adapter's** clients (`socket-redis-adapter.ts` `buildClient()`) were already bounded and failed fast/closed; the **gate's** second client is the one that hung. **Rules:** (1) every client created during module init needs a **bounded** connect — `connectTimeout` + a fail-fast `reconnectStrategy` (return an `Error` before the first successful connect) **plus** a `withTimeout()` race (`server/livechat/with-timeout.ts`) so "boot cannot hang" is a property of _our_ call, not of the library's internals; (2) boot must **fail OPEN** — a Redis outage degrades live chat to the in-memory gate, it must never stop the process binding its port; (3) **never latch an `initialized` flag before the attempt** — doing so made one boot-time blip pin the whole fleet to in-memory gates permanently, silently defeating the documented 30s retry; (4) bound the hot-path (`claimAgentAnnouncement`, 2s) and shutdown (`quit()` with a `disconnect()` fallback, else SIGTERM hangs and pm2 SIGKILLs). **Same class, different route (pre-existing, PR #3306):** `server/swim/mcp-server/index.ts` wraps `app.listen()` in `new Promise(resolve => …)` with **no reject path and no `'error'` listener** — a bind failure (`EADDRINUSE`/`EACCES`) is reported **asynchronously as an `'error'` event**, `listen()` does not throw, so the promise never settles; the retry loop only catches rejections (dead code) and the manager's own `uncaughtException` handler **logs without exiting**, swallowing the crash. Only reachable with `ENABLE_MCP_SERVER=true`. Fix: `new Promise((resolve, reject) => { const s = app.listen(port, host, resolve); s.once('error', reject); })` + a `withTimeout()` on the awaited call. **Repro without desk2:** run the build locally with an unreachable/absent `REDIS_URL` (`REDIS_URL=redis://127.0.0.1:1`), or on the box `PORT=5099 timeout 90 node dist/index.js` and read the last log line before the hang. Related: `server/redis-session-store.ts` logged the **raw `REDIS_URL` twice on boot** into pm2 logs — managed ElastiCache URLs carry inline auth (`rediss://:TOKEN@host`) — now scrubbed through `describeRedisTarget()` (host:port only).
...
| `0` / `NULL`          | no mapping (**~2,264 of 7,515** as of 2026-09-11; was ~1,371)                                                                                    |
...
| **darklaunch**     | SERP's parallel Odoo-shadow validation system. SERP and Odoo run side-by-side, dual-write via Serpy + one-way Odoo→SERP sync, gated for cutover at "<1% drift, stable 2 weeks." Darklaunch history lives in the `serp_test` (staging SERP) DB on Hetzner `5.161.233.240`; production is `serp_app`. Cutover timestamp in `serp_darklaunch_meta.darklaunch_cutover_at` (prod 2026-06-04 09:27:20, staging 2026-06-03 11:52). The worker is **isolated** — never reads Odoo _values_ at runtime (only ID resolution for `odoo_id` stamping). "compare-replica" tooling actually compares darklaunch.                                                                                                                                                                                                                                                                                                                                                                |
...
| **SWIM**           | WishDesk's AI customer-service chatbot, powered by the `kb-v2` Qdrant collection. Generates draft replies (acceptance rate ~1%). **Model backing it: Anthropic `claude-sonnet-4-6`** (the resolved default via `ANTHROPIC_CHAT_MODEL`, unset in `.env` so the code default in `server/swim/shared/config.ts:155` applies) — same model is used for agent-response-suggestion drafts (`server/services/agent-response-service.ts:96`) and SWIM email-processing draft generation (`server/swim/workflows/email-processing/draft-generation.ts:187`). The **Gmail/Gift-Concierge email drafter is a separate stack on OpenAI `gpt-5.1`** (`OPENAI_GENERATION_MODEL`, `server/services/gmail/gmail-ai-draft-generator.ts`) — don't conflate it with SWIM/Ask Wishy. Note: `.env`'s `ANTHROPIC_GENERATION_MODEL=claude-haiku-4-5-20251001` only feeds the release-notes AI drafter, not SWIM/chatbot/drafts — a red herring if searching for the chatbot's model var. |
...
`~/.claude/settings.json` sets `"model": "opus[1m]"`. The 1M window is what _permits_ a
...
control is the _behavioral_ budget (compact ~150k / hand off ~250k), which reduces the same
