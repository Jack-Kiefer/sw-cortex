...
- **Odoo.sh firewall (rolled out 2026-09-21):** direct PostgreSQL access to Odoo (`:5432` — used by SERP pools, n8n, and the `odoo`/`odoo_staging` MCP keys; NOT via SSH) requires the connecting IP to be allowlisted in the Odoo.sh **project-level** Settings → Firewall panel (top-nav Settings, not the branch Settings tab), max 10 IPv4s, no CIDR. An un-allowlisted IP sees `:5432` SYN-dropped (hangs, never refuses) while `:443`/the web UI keep working. A branch's SSH-tab "Allow my IP" only grants 1 hour. If every Odoo read suddenly times out at connect (even `SELECT 1`), check this allowlist before suspecting a query or code regression.
