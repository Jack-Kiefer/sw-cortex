...
- ⚠️ **The publish pattern — the concrete tables Laravel reads instead of joining across the DB boundary (from the 2026-08-03 "SERP Cutover Strategy" + "Usage Inventory" artifacts, the most-current operational plan; the in-repo `serp-odoo-cutover.md` is the engineering half).** SERP owns the computation, **publishes** the answer into `laravel_live`, and Laravel reads it locally — no query ever spans two servers. The three published surfaces:
  ...
  - **The TWO inventory views** are `receiver_product_inventory` (finished goods) **and `component_product_inventory`** (the RM twin — same 9-column contract keyed on `component_id`; **holds & prelist stay literal `0`** so `sellable_qty == available_qty`; demand is real, from `component_orders`). Both live in `laravel_live` (verified 2026-08-03, also in `serp_test`); their bodies are canonical in `EXTRA_VIEWS` in `seeding/scripts/export_manage_schema.py`. ⚠️ **The demand leg still filters on `oddo_synchronized`, NOT `serp_sync` — on BOTH environments, which have NOT diverged (re-verified live 2026-09-10 via `information_schema.VIEWS`).** Both view bodies carry the identical predicate — `` and (`o`.`oddo_synchronized` in (0,2)) `` on the `ec_order` leg and `` and (`p`.`oddo_synchronized` in (0,2)) `` on the `preselect_orders` leg — on `manage` (both `sugarwish_manage_new_2` and `sugarwish_manage_new_3`) and on `laravel_live` (`sugarwish`). No `serp_sync` appears in any of the four view bodies. **A prior entry here claimed `laravel_live` had "already" swapped to `serp_sync = 0` and that the environments diverged (verified-live 2026-08-14) — that is FALSE as of 2026-09-10; neither swapped.** Consequence that matters: the order-queue worker's `serp_sync` writes are **INERT to the demand leg on both stacks** — they advance the worker's own detection cursor only, so a `serp_sync` value cannot yet move `sellable_qty` either way. The swap is a cutover-day predicate change, not a shipped one. Corroborated in-code at `backend/workers/order_processing.py:546-548`. Conversely the **WW-2496 perf indexes are on `manage` ONLY** — `component_orders (order_id, component_id, quantity)` as `idx_co_order_component_qty` and `preselect_orders (created_at)` as `idx_preselect_created` (Manish shipped them as a migration ~2026-08-14); **neither exists on `laravel_live` yet**, which is the normal staging-first steady state, NOT a failed migration. Both views still use a **30-day** demand window in both environments — a proposed narrowing to 14 days has NOT landed anywhere, so do not describe it as current.
  - ⚠️ **The demand leg tests a per-order sync FLAG on `ec_order`, NEVER `serp_sale_order`** — a `NOT EXISTS` against that ~17-row stub matches _every_ order, driving `sellable_qty` negative and auto-disabling the whole catalog. **Today that flag is `oddo_synchronized IN (0,2)` on both stacks** (see the view entry above); `serp_sync` is its planned like-for-like replacement and becomes load-bearing for inventory correctness only once that predicate swaps at cutover. Until then `serp_sync` is the order-queue worker's detection watermark and nothing more. Either way the rule holds: the demand leg keys on a **flag column**, never on a `serp_sale_order` existence test.
...
#### Darklaunch Order Worker (`backend/workers/serp_order_queue_worker.py`)
...
- **Darklaunch Order Worker** (`backend/workers/serp_order_queue_worker.py`): **no queue** — polls live MySQL directly (`ec_order WHERE oddo_synchronized=1`, `POLL_INTERVAL 300s`, `BATCH_LIMIT 50`, single-threaded), dual-writes receiver/preselect rows into `serp_prod_darklaunch` ONLY.
