...
- ⚠️ **There is NO post-cutover SERP→Odoo mirror.** At flip day the seed copies Odoo's history **one-way** into the co-hosted `serp_*` tables in `laravel_live`; after that **nothing writes to Odoo ever again** — it freezes read-only for reference, and decommission is just turning it off (before the ~Oct 2026 v15→v17 forced upgrade). The XML-RPC write service, the `odoo_sync_queue` worker, and the drift monitor **die AT cutover**, not during a wind-down; new records carry **`odoo_id = NULL`**. **⚠️ Cutover-mechanics REVISION Jack is drafting (2026-07-29, still an in-progress plan doc, not final): SERP's tables do NOT get bulk-copied into `laravel_live` — they STAY where they already are (a config flip, no ~84.5 GB / ~3 hr data movement), consolidating into `serp_app` on the same box with `serp_test` becoming the staging snapshot. Because Laravel then CANNOT join across the DB boundary to `serp_sale_order` / `serp_stock_quant`, the integration surface flips from "join directly" to "SERP publishes into Laravel": SERP writes an aggregated stock table into `laravel_live` that the inventory views read (instead of joining `serp_stock_quant`), and the two `serp_sync` columns are PUBLISHED into Laravel rather than Laravel testing whether a `serp_sale_order` row exists. The `/api/odoo` pull-and-callback endpoints are explicitly retired, replaced by the darklaunch order queue reading Laravel's DB directly.**
