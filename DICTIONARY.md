...
- **Live `serp_test` contains ZERO Laravel catalog tables.** Verified 2026-07-30: none of `components`, `receiver_products`, `items`, `ec_order`, `buyer_products`, `inventory_reservations`, `kits`, `component_kits`, `odoo_inventory` exist there. Those mirrors (written by `staging_copier_worker.py`) are only populated in the **local** darklaunch DBs. So any reasoning of the form "the cross-DB `serp_*` ↔ Laravel joins are fine because the copier keeps both sides co-resident" is **false on the live box** — it's a precondition to build, not a current fact. Related live bug: `kits`/`component_kits` are joined in ~10 query sites but are absent from `_CATALOG_TABLES` (`staging_copier_worker.py:278`), so those joins silently return zero rows on the `ENVIRONMENT_SET=serp` path.
