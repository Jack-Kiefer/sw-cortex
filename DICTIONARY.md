...
| `serp_app`           | MySQL      | **LIVE/production SERP DB** — same Hetzner host as `serp_test` (`LIVE_DARKLAUNCH_DB_*`), database `serp_app`. Prod target, but does **not** hold the `serp_*` tables yet (arriving via migrations) |
...
- ⚠️ **The publish pattern — the concrete tables Laravel reads instead of joining across the DB boundary (from the 2026-08-03 "SERP Cutover Strategy" + "Usage Inventory" artifacts, the most-current operational plan; the in-repo `serp-odoo-cutover.md` is the engineering half).** SERP owns the computation, **publishes** the answer into `laravel_live`, and Laravel reads it locally — no query ever spans two servers. The three published surfaces:
  ...
  - ⚠️ **The demand leg tests `ec_order.serp_sync = 0`, NEVER `serp_sale_order`** — a `NOT EXISTS` against that ~17-row stub matches *every* order, driving `sellable_qty` negative and auto-disabling the whole catalog. This makes `serp_sync` **load-bearing for inventory correctness**, not a reporting flag.
