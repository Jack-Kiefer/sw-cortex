...
**Worker process model & hangs:** All workers run in ONE process on ONE asyncio loop, supervised by an asyncio supervisor in `workers_main.py` (added after a May 29 2026 silent odoo-sync death; it restarts individual wedged coroutines). The darklaunch worker once starved the odoo-sync worker this way (live 2026-05-29: odoo-sync stalled ~95 pending, syncing once per ~24-min darklaunch cycle) — since fixed in TWO stages: per-order PROCESSING offloaded via `await asyncio.to_thread(handler, order)` inside `_drain` (May 2026, yields the loop between every order), and DETECTION offloaded via `_run_detection()` in `asyncio.to_thread` (SERP PR #520, July 2026 — the four detectors were blocking PyMySQL on the loop, two vestigially `async` with zero awaits; ~80–110s per pass at ~5.5k in-flight candidates). Blocking network/DB calls with no timeout (asyncpg `pool.acquire`, `xmlrpc.client.ServerProxy`, PyMySQL) run forever on idle-dropped sockets; PM2 keeps the process 'online' so nothing alerts. Fixes: PR #107 bounded `pool.acquire`, PR #108 per-poll `asyncio.wait_for` watchdog, XML-RPC socket timeouts. Recovery needs `pm2 restart`.
...
⚠️ **Darklaunch worker event-loop starvation — FIXED in two stages (don't re-diagnose it as open):** all workers run in ONE process on ONE asyncio loop, and the darklaunch worker once starved odoo-sync (live 2026-05-29: odoo-sync stalled ~95 pending, synced once per darklaunch cycle; disabling darklaunch restored a clean 15–17s cadence). Stage 1 (May 2026): per-order PROCESSING offloaded — every handler runs via `await asyncio.to_thread(handler, order)` inside `_drain`, yielding the loop between orders. Stage 2 (SERP PR #520, July 2026): DETECTION offloaded — the four detectors were still blocking PyMySQL on the loop (two vestigially `async` with zero awaits, ~80–110s per pass at ~5.5k in-flight candidates), now run inside `_run_detection()` via `asyncio.to_thread`, with detection amortized (`DETECTION_LIMIT=5000` for new/cold, one shared `_active_replica_candidates` snapshot per pass, per-phase detect/drain ms in the cycle logs). A **supervisor coroutine** in `workers_main.py` (added after the May 29 2026 silent odoo-sync death) restarts wedged worker coroutines.
...
⚠️ **Darklaunch worker event-loop starvation is FIXED — per-order processing offload shipped May 2026, detection offload shipped SERP PR #520 (July 2026).** All workers run in ONE process on ONE loop; the historical incident (live 2026-05-29: odoo-sync stalled ~95 pending, synced once per darklaunch cycle) was solved by `await asyncio.to_thread(handler, order)` per order in `_drain`, and the remaining gap — the four DETECTORS blocking the loop for ~80–110s/pass at ~5.5k in-flight candidates — by running detection in `asyncio.to_thread` (`_run_detection()`), amortized so one detection feeds the full backlog instead of re-running per 50 orders. Don't cite "wrap processing in to_thread" as a pending fix. Separate incident: odoo-sync "Sync cycle wedged: 3 consecutive timeouts at 180s" is transient/self-recovering (idle-dropped socket), distinct from this starvation.
