...
- **Disable Unreserved V2 — 4-factor formula (live-verified 2026-07-06):** `final_available = Actually Available + prelist_qty − SW Reserved (inventory_reservations.status='active') − Orders Not Imported (items.odoo_sync IN (0,1))`, compared against `effective_drop_level = COALESCE(rp.drop_level, feature_attributes id=1 = 100)`. SKU scope `default_code LIKE 'FG-%'/'SA-%'/'L-B02%'`, `sugarwish_id` 1–800,000. Live workflow renamed **"Disable Unreserved Products V2"** (same live id `VRdmXlm2XTeRbOyT`); its Slack alert now shows `Prelisted:` and `Final:` fields — an alert WITHOUT those fields is the pre-V2 formula. Entry gate: only products with `unreserved_demand > 0` (open customer moves Odoo hasn't reserved) are evaluated at all — it's an **oversell tripwire, not a general low-stock disabler**; only enabled, non-archived PARENT products (`inventory_link IS NULL`) are judged, children get disabled via cascade. It is **disable-only with NO cooldown** (every 1 min), so a manual re-enable is reversed within ~60s while the math stays below drop. ⚠️ **Prelist blind spot (found 2026-07-06, FIXED same day by V2):** the pre-V2 formula ignored `prelist_qty` — and since WW-1667 (2026-07-01) prelisting bumps only that Laravel column and touches NO Odoo inventory, prelisted SKUs (e.g. SA-01-099-A, 100 prelisted, disabled Jul 5 at `actually_available=-1` vs drop 32) got auto-disabled and stayed disabled. V2 adds `prelist_qty` to Node 2's SELECT and `+ prelistQty` to Node 3's formula, so prelisted SKUs correctly stay enabled while overselling within their prelist buffer (verified: 7 prelisted SA SKUs with negative `actually_available` all compute `final_available` far above drop and stay enabled). Still true in V2: deadlock risk on `receiver_products` (multi-statement UPDATE every 1 min vs Laravel/Retool writers, `ER_LOCK_DEADLOCK` seen; onError → Slack, no retry), the `NOT IN ('done','cancel')` seq-scan footgun inside its `move_reservations` CTE (330s Odoo risk), no backoff (cron fires every minute regardless), and a latent scope quirk (Node 2 compares `start_date`/`end_date` to `created_at`, not `NOW()`).
