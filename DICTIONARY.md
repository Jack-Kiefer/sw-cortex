...
⚠️ Until 2026-07-29 (WW-2143) **any anonymous WebSocket** could connect to the `/chat` Socket.IO namespace and emit `join_room { room: 'agents' }` with **no auth whatsoever**, joining the privileged agent broadcast room and streaming every live customer-support conversation — full message content and attachment URLs. Fix (`server/livechat/socket/connection-handler.ts`): a shared **`verifyAgentSocketToken()`** helper — JWT verify **plus a DB role check** (authorize off the DB role, **not** a JWT claim) — now guards **both** `JOIN_SESSION` and the `join_room` `'agents'` branch. The client (`client/src/hooks/useAgentSocket.ts`) must now send `externalJwtToken`/`jwtToken` in the `join_room` payload. **Debugging signature:** a client that omits the token is **silently rejected** — it looks like "agents stopped getting live notifications on `/admin/message-center`" with no visible error and no obvious log; check the join payload before chasing infra or transports. Regression coverage: `join-room-auth.test.ts` (9 tests).
