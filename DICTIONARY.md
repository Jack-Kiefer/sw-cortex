...
- **Inventory-rollup VIEWs `receiver_product_inventory` / `component_product_inventory`** (MySQL views on `laravel_live`/`sugarwish` AND its `serp_test` darklaunch mirror; both are `serp_stock_quant`-based rollups keyed by the Laravel FK on `serp_product_product`). **Rebuilt 2026-07-13** from the old bare `qty_on_hand/qty_reserved/qty_available(=quantity−reserved)` shape into demand-aware columns:
  - **`receiver_product_inventory`** (keyed by `receiver_product_id`): `on_hand_qty` = `SUM(serp_stock_quant.quantity)` physical on-hand ONLY (no deductions); **`available_qty`** = `on_hand − Odoo reserved_quantity − unsynced-order demand − unreserved backorder` (prelist **EXCLUDED**); **`sellable_qty`** = `available_qty + receiver_products.prelist_qty`. The three demand terms: **unsynced orders** = `items` where `odoo_sync IN (0,1) AND deleted_at IS NULL` (joined directly `items.product_id = pp.receiver_product_id`); **unreserved backorder** = the serp_* twin of `unreserved.sql` (outbound internal→customer `serp_stock_move` in state `IN ('confirmed','waiting','partially_available')`, `SUM(product_uom_qty − reserved-from-move-lines)`; `assigned` excluded).
  - **`component_product_inventory`** (keyed by `component_id`): **`on_hand_qty` = `SUM(quantity)` ONLY** — no reserved/available/sellable (component demand isn't derivable without BOM explosion, out of scope).
  - **Design gotchas:** (1) **SW/preselect `inventory_reservations` is DELIBERATELY OMITTED** — that table exists on `laravel_live` but is ABSENT on `serp_test`, and one shared view DDL can't reference a table missing in one DB (view errors on SELECT); Odoo `reserved_quantity` on the quant covers picking allocations. This diverges from `backend/sql/live_products/live/receiver_products.sql` by design. (2) **The unreserved term MUST `FORCE INDEX (idx_serp_sm_state)`** — `serp_stock_move` is ~16M rows and the optimizer otherwise joins locations first and full-scans (verified `type=ALL`); with the force it's a ~2,275-row range scan. It's a pre-aggregated derived table (once per receiver_product_id), never correlated per-quant. (3) `items` full-scans on live (15M rows, no `odoo_sync` index) — the one heavy branch, bounded (single scan, ~446 unsynced rows). **Three code copies must stay in sync:** `seeding/scripts/export_manage_schema.py` `EXTRA_VIEWS`, `seeding/snapshots/manage_schema.json` `_views` (drives serp_test on reseed AND CI via `apply_manage_schema.py::apply_views`), and the hand-run `migrations/20260713_inventory_views_demand_prelist.sql`. Live `laravel_live` update must run as `swrelease` (views are `SECURITY DEFINER`, `DEFINER=swrelease@'%'`) or the DEFINER re-stamps. `component_inventory` (no `_product_`) is a SEPARATE legacy BASE TABLE — not one of these. No live SQL/Python SELECTs FROM either view (forecast moved off them), so reshaping columns is safe.
