...
| `serp_app`           | MySQL      | **LIVE/production SERP DB** — same Hetzner host as `serp_test` (`LIVE_DARKLAUNCH_DB_*`), database `serp_app`. Now **holds the `serp_*` tables** (migrations have landed — both DBs at `_migrations` 078 as of 2026-08-10; the earlier "does not hold them yet" is stale) |
...
- ⚠️ **The publish pattern — the concrete tables Laravel reads instead of joining across the DB boundary (from the 2026-08-03 "SERP Cutover Strategy" + "Usage Inventory" artifacts, the most-current operational plan; the in-repo `serp-odoo-cutover.md` is the engineering half).** SERP owns the computation, **publishes** the answer into `laravel_live`, and Laravel reads it locally — no query ever spans two servers. The three published surfaces:
  - **`serp_published_stock`** (in `laravel_live`) — one row per stocked product, **PK `product_id` = the raw `sugarwish_id`** (NOT `serp_product_product.id` — corrected WW-2529, PR #903 merged 2026-08-20, and live-verified: freshest rows have `product_id == receiver_product_id`). The sugarwish*id is DECODED into **both** bridge FKs (`receiver_product_id` INT for finished goods; for RM a `800`-prefix + len>3 strips to `component_id` BIGINT — exactly one set is filled). Columns `on_hand_qty`/`reserved_qty`/`unreserved_outbound_qty` (all `DECIMAL(16,4)`, **`on_hand_qty` is SIGNED** — negative is real) + `computed_at`. **No FKs** (referents live in another DB). Replaces the view legs that used to join `serp_stock_quant`/`serp_stock_move` across the boundary. **TWO writers converge on the same `product_id`=sugarwish_id key:** (1) the interim Laravel publisher `OdooInventoryService::publishStock` (WW-2496), and (2) a NEW **hourly SERP cron `reconcile-published-stock`** (WW-2529, PR #903) that computes the table **entirely from Odoo** via `odoo_routed_pool` — env-routed: **Odoo staging on serp-test/dev, live Odoo in prod** — grouped by `product_template.sugarwish_id`, clear+rebuild in one transaction (skip-on-empty, zeros included, sign preserved); the internal-location filter (`stock_quant` over `usage='internal'`, CASE-guarded) lives in that cron. (Note: PR #899's earlier serp*\*-MySQL-sourced version was WRONG — `manage` has `serp_product_product` but zero `serp_stock_quant`, so every `on_hand` published 0; #903 superseded it. Live row count fluctuates with the hourly rebuild — treat the exact count as volatile.)
  - **`serp_published_kits`** (~1,039 rows, keyed `buyer_product_id`+`component_id`) — replaces Laravel's four `serp_*` Eloquent models + the cross-schema join inside `SerpBomService`.
