...
- **Column-deprecation workflow (decided 2026-07-21 SERP Touchbase, Anna):** deprecated Laravel/inventory columns are NOT dropped at the flip. After the new system goes live, each is **renamed with a `(deleted)` / "deleted" suffix** and **left live in the DB for ~several weeks–a month**, then physically dropped. The grace window lets any stray reader surface (erroring loudly on the renamed column) before the hard removal, so migration day never trips on a column drop. This is the same `(deleted)`-suffix pattern already deployed on `components` (`inventory_quantity`/`status`/`drop_level`/`mi_inventory_qty`/`location_2_status`), now adopted as the STANDARD cutover deprecation strategy for the frozen `inventory_qty`/`odoo_inventory`/sync-flag columns. Follow-up removal tickets track the eventual drop. (Also decided Jul 21: investigate whether Ops still uses the Laravel receiver-product admin page before deciding to migrate it to SERP; component inventory needs no handling change now — the view was widened to mirror the receiver view but nothing depends on it for sales logic.)
- **Column-deprecation workflow (decided 2026-07-21 SERP Touchbase, Anna):** deprecated Laravel/inventory columns are NOT dropped at the flip. After the new system goes live, each is **renamed with a `(deleted)` / "deleted" suffix** and **left live in the DB for ~several weeks–a month**, then physically dropped. The grace window lets any stray reader surface (erroring loudly on the renamed column) before the hard removal, so migration day never trips on a column drop. This is the same `(deleted)`-suffix pattern already deployed on `components` (`inventory_quantity`/`status`/`drop_level`/`mi_inventory_qty`/`location_2_status`), now adopted as the STANDARD cutover deprecation strategy for the frozen `inventory_qty`/`odoo_inventory`/sync-flag columns. Follow-up removal tickets track the eventual drop. (Also decided Jul 21: investigate whether Ops still uses the Laravel receiver-product admin page before deciding to migrate it to SERP; component inventory needs no handling change now — the view was widened to mirror the receiver view but nothing depends on it for sales logic.)
...
- ⚠️ **Auto-disable ownership decided at the 2026-07-21 SERP Touchbase (Seth + Anna, stability): Laravel is primary — and it already IS.** The real Laravel disabler is **`ReceiverProduct::disableIfCanBe()`** (`app/Models/ReceiverProduct.php:149`), which fires **synchronously on every order placement** (checkout controllers, `PreselectOrderPlacement`, `SweetShoppeOrderPlacement`, the queued `PrepickOrderJob`, `ReceiverOrderService` inside the order txn — NOT a cron), disabling when `inventory_qty ≤ drop_level` (own or `inventory_link` parent's; else global `candy-inventory-drop-threshold`=100) and cascading disable to parent + child via `inventory_link`. It reads ONLY `inventory_qty` vs `drop_level` (no prelist/reservations/odoo_inventory) and is **disable-only** (never re-enables). So the Jul 21 decision is NOT new work — it is to **repoint that existing check** from raw `inventory_qty` to the view's **`sellable_qty`** (`sellable_qty ≤ drop_level`), in the SAME change that removes the co-located decrement writes (post-freeze `inventory_qty` is a dead number). The SERPY/n8n "network node" disabler (n8n `Disable Unreserved`, the known deadlock source; the Gemini notes' "NAN" = that SERPY/node path, not a separate system) stays **BACKUP only, not primary** — too unstable to lead. (Two other disablers exist today and both retire: the `SynchronizeOdooInventory` Python sidecar auto-disables during the Odoo sync — dies with that sync; and n8n `Disable Unreserved` — demoted to backup.) Manual inventory corrections still route through **SERPY** (draft→approve).
- ⚠️ **Auto-disable ownership decided at the 2026-07-21 SERP Touchbase (Seth + Anna, stability):** post-cutover the PRIMARY auto-disable check moves **into Laravel**, reading the view's **`sellable_qty`** (`receiver_product_inventory.sellable_qty ≤ drop_level`) — NOT the SERPY/n8n automated "network node" disabler, which stays a **BACKUP only, not the primary system** (it's the known `Disable Unreserved` deadlock source, judged too unstable to lead). The Gemini notes' "NAN" is a transcription of that SERPY/node disable path, not a separate system. Jack's follow-up: rewrite the Laravel product-disable function to use `sellable_qty` instead of the network node. Manual inventory corrections still route through **SERPY** (draft→approve); only the automatic disable ownership is pinned to Laravel-primary.
