...
| `serp_test`          | MySQL      | **Live PROD darklaunch** on Hetzner `5.161.233.240:3306`; canonical/most-current write target (the old `live_darklaunch_db` key was removed — same DB)                               |
| `serp_app`           | MySQL      | **Main/live SERP app DB** — same Hetzner host as `serp_test` (`LIVE_DARKLAUNCH_DB_*`), database `serp_app`; the prod DB the SERP app reads/writes — NOT the darklaunch shadow mirror |
...
- **Production darklaunch MySQL DB**: Hetzner at **5.161.233.240**, database `serp_test` (MCP key `serp_test`) — this is the **REAL live production darklaunch mirror**, NOT a throwaway test DB.
...
| `serp_test`                             | MySQL      | **Live PRODUCTION darklaunch mirror** on Hetzner `5.161.233.240:3306` (old `live_darklaunch_db` key removed) |
...
- **Darklaunch DBs**: replica PLUS Odoo overlay (normal BOMs, MOs, SVL, POs). `serp_prod_darklaunch` is future production. `serp_test` on Hetzner is the canonical prod write target and runs consistently ahead (~35K stock_moves vs `serp_prod_darklaunch`'s ~34K).
...
- **`serp_test`** (MCP key) = MySQL `serp_test` on **Hetzner `5.161.233.240:3306`** — the REAL live PRODUCTION darklaunch mirror (NOT a throwaway test DB), the canonical prod write target for the worker. Consistently _ahead_ of `serp_prod_darklaunch` (~35K moves vs ~34K). TZ = `America/Denver`. (The old `live_darklaunch_db` MCP key was removed — same DB.)
...
| `serp_test`(Hetzner`5.161.233.240`) | **REAL live PRODUCTION darklaunch mirror** (not a throwaway), MCP key `serp_test`, consistently ~ahead of `serp_prod_darklaunch` | —                            |
