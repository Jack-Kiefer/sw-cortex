...
- Inventory has **no single source of truth**: sellable (SA) = Laravel `receiver_products.inventory_qty` **(until cutover — then the `receiver_product_inventory` view's `sellable_qty`, `serp_stock_quant`-backed)**; raw material (RM) = Odoo only; accounting/valuation = Odoo `stock_quant`/SVL **(all three consolidate into the `serp_*` tables in `laravel_live` at cutover)**. Manual inventory corrections go through **SERPY** draft→approve post-cutover; the Laravel admin inventory-transaction UI retires.
