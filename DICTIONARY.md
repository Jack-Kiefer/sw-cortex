...
- **`check_sql` honors the heavy-query rule:** POSITIVE state list `state IN ('draft','confirmed','waiting','partially_available','assigned')`, never a negated `NOT IN`; add an indexed window + LIMIT. On the SERP MySQL mirror use the **4-state** list — the seeder collapses `partially_available` to `assigned`; the 5-state list is Odoo-PG-only. `text()`/asyncmy landmines apply: `%`→`%%`, no `:word`, no reserved-word aliases.
...
- **Bin-level `location_id` divergence** (3 of 9 in-window worker stock_move_lines write parent loc 8 where Odoo refines to a bin like 1285/1358/1366/3927). ⚠️ **Cause clause retracted 2026-09-11:** this previously read "SERP doesn't replicate Odoo putaway/removal strategy", which is no longer supported — `_gather` _does_ expand to child bins and stamps the line at the gathered quant's bin. Note the observation here (writing the **parent**, loc 8) is the _opposite_ of the expansion symptom, so it may be a distinct phenomenon; **the cause is currently unknown — re-derive it.** Do **not** add `location_id` to `WORKER_ROW_DIVERGENT_COLUMNS['stock_move_line']` until then: suppressing it could mask the parent→child self-cancel signal. (`stock_move_line.move_id` FK id-space and `reference`=NULL are separate and still fine to suppress.)
