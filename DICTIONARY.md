...
> ⚠️ **The four DB NAMES below are the RETIRED local Docker layout** (`serp_prod_replica`/`serp_staging_replica`/`serp_prod_darklaunch`/`serp_staging_darklaunch`). **SERP PR #46 consolidated the local Docker DBs 5 → 3** — the current local DBs on `serp-mysql` (`127.0.0.1:3307`, `devuser`) are **`serp_local_prod`**, **`serp_local_staging`**, and **`laravel_local`** (the 13 Laravel catalog tables). The live darklaunch history lives in Hetzner **`serp_test`** (staging) and production is **`serp_app`** — see "The 14-Database Landscape". The replica-vs-darklaunch *concept* (clean Laravel mirror vs Odoo-overlaid) is still worth understanding, but these are no longer live local DB names.
...
- Missing AUTO_INCREMENT → INSERT 1048 errors: **`serp_stock_location` and `serp_stock_picking_type` are now FIXED** (both `id` columns carry `auto_increment` as of 2026-08-10); **`serp_res_currency.id` still lacks it** (verified live 2026-08-10 — `extra` empty), so that one still 1048s on a bare INSERT. M2M rel tables missing `serp_` prefix (`purchase_order_stock_picking_rel`, `stock_move_line_consume_rel`).
...
- `inventory_qty` (Englewood/current SA), `mi_inventory_qty` (Taylor), `odoo_inventory` (synced cache, can be NEGATIVE -122/-44/-325, stale — use `inventory_qty` for decisions), `prelist_qty` (int NOT NULL, default 0 — **prelisting**: qty Sophie wants sellable before an MO is packed; added 2026-07-01; drain wiring LIVE as of 2026-07-13 (SERPY subtracts on MO, logging each drain to the **`prelist_qty_log`** audit table); the Odoo→Laravel inventory sync cron ADDS it into `inventory_qty`, SERPY SUBTRACTS from it — floored at 0 — when it MOs the product; reporting must SUBTRACT `prelist_qty` to avoid double-counting; does NOT touch Odoo, so costing stays correct), `inventory_link` (parent product_id for cascading disables — set NULL on combo-line children in Apr-2026 migration), `location_id` (FK locations; **ALL ~5,150 rows NULL — field unused**), `vendor_id`, `vendor_cost` (decimal 8,2), `product_id` (= `sugarwish_id`, join to `product_template.sugarwish_id`), `product_type`, `tango_utid`, `odoo_id`. **Duplicate SKUs exist** (SA-01-026-A as product_ids 1706+4499; 13 component SKUs duplicated) — use `product_id`/`id` for joins, NOT SKU.
