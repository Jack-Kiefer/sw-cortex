...
- **Both `serp_app` migration tracks still land in `serp_test` TODAY** — prod sets `SERP_APP_DB_NAME=serp_test` (`.env.example:189`) and `serp_app/run-serp-app-migrations.js` falls back to `serp_test`, so `migrate:serp-app` writes there, not to the `serp_app` DB. Verified live 2026-07-30: `serp_test._migrations` is at **069**, `serp_app._migrations` at **067** (068/069 exist ONLY on `serp_test`). Migration **`069_baseline_serp_erp_schema.sql`** has begun moving the ERP schema onto the serp_app track (creating those tables empty on `serp_app`), so one-DB-per-environment is the **target**, not the current state. ❌ Do not "correct" a doc that says the runner targets `serp_test` — that is what actually happens right now.
