...
| 🔴 Duplicate Receiver Orders **BROKEN**       | `EfCt429Vle67ckY8`                                                                                 | every 10 min                                    | ⚠️ **Undocumented and failing on EVERY run** (verified 2026-09-11, latest failures minutes apart): `Unknown column 'eo.is_printed'` — `laravel_live.ec_order` has no such column, so it has **never** produced a result. Active but useless; either fix the column or deactivate it.                                                                                                  |
