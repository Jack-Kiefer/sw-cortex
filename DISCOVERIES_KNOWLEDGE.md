...
- **Deploy is MANUAL.** GitHub Actions CI runs on push but does **NOT deploy**.
  - ❌ AI assumes SERP auto-deploys from `main` via Jenkins → ✅ Reality: deployment is a manual SSH step: `ssh -i ~/.ssh/id_ed25519 ubuntu@34.203.231.65 'cd /opt/SERP && bash deploy.sh'`. A push/merge to `main` is NOT live until `deploy.sh` runs.
...
**2. Darklaunch Order Worker** (`darklaunch_order_worker.py`). **NO queue at all** — it POLLS live MySQL directly (`ec_order WHERE oddo_synchronized=1`, `POLL_INTERVAL=300s`, `BATCH_LIMIT=50`, single-threaded). It dual-writes receiver-order + preselect-order rows into `serp_prod_darklaunch` ONLY (never the main DB), shadow-processing Odoo-sourced orders (`order_type IN ('receiver-order','preselect-order') AND inventory_source='odoo'`), reading read-only against Odoo/live MySQL for seeding, never pushing back to Odoo.
...
- DETECTOR BUG: `detect_shipped_orders_odoo`/`detect_cancelled_orders_odoo` had a vestigial `ORDER BY ... DESC LIMIT 50` that permanently strands the oldest tail when in-flight candidates exceed 50 (~883 observed). Fix is oldest-first ordering (by replica `serp_stock_picking.create_date`) + a per-cycle cap.
...
- **`oddo_synchronized`** tinyint (TYPO 'oddo' — the REAL Odoo-sync flag): **0=not started (~77K), 1=synced (~3.88M), 2=in-flight/partial (~64-166), 3=stuck/errored, 5=vendor/bypass (~17K)**. (`value 3` exists per some reports, NOT-exists per others — verify per env.) Plus `ship_date_odoo_synchronized`, `component_imported`, `vendor_imported`.
...
⚠️ **Two SERP servers, not one:** SERP app server is **AWS EC2 `34.203.231.65`** (`/opt/SERP`); darklaunch MySQL is a **separate Hetzner machine `5.161.233.240` / `serp_test`**.
